...
// harness-contract: gui.renderer-no-privileges
test("renderer model has no Node or Electron privileged surface", () => {
  ...
  assert.deepEqual(
    rendererNavigation.map((item) => item.id),
    ["workspace", "board", "list", "detail", "doc-viewer", "review-queue", "execution-evidence", "graph"],
  );
});
