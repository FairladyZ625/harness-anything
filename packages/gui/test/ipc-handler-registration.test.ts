...
test("main process registers one IPC handler for each preload allowlist method", async () => {
  ...
  const localPayload = (method: string): object | null => method === "listRuntimeInstances" ? null : method === "createRuntimeInstance" ? { instanceId: "codex-review", name: "Codex Review", kindId: "codex", installationId: "installation-codex", providerId: "openai", model: "gpt-5.6-sol", codex: {}, authMode: "api-key", apiKey: "sk-typed-in-form" } : ["signInRuntimeInstance", "reauthRuntimeInstance", "signOutRuntimeInstance"].includes(method) ? { repoId: "repo-a", instanceId: "codex-review", idempotencyKey: "once" } : method === "updateRuntimeInstance" ? { instanceId: "codex-review", enabled: true } : { instanceId: "codex-review" };
  for (const method of invokeMethods) { const payload = localMainPreloadMethods.includes(method as never) ? localPayload(method) : method === "getTaskDispatches" ? { repoId: "repo-a", taskIds: ["task-a"] } : routeByMethod.get(method)?.requiresRepo ? { repoId: "repo-a" } : null; assert.deepEqual(await handlers.get(`harness:${method}`)?.(trustedEvent, payload), { ok: true, method, payload }); }
  ...
  assert.throws(() => assertPreloadPayload("createRuntimeInstance", { instanceId: "codex-review", name: "Codex Review", kindId: "codex", installationId: "installation-codex", providerId: "openai", model: "gpt", authMode: "subscription", apiKey: "subscription-cannot-carry-a-key" }), /invalid/iu);
