...
test("main process registers one IPC handler for each preload allowlist method", async () => {
  ...
  for (const method of invokeMethods) { const payload = routeByMethod.get(method)?.requiresRepo ? { repoId: "repo-a" } : null; assert.deepEqual(await handlers.get(`harness:${method}`)?.(trustedEvent, payload), { ok: true, method, payload }); }
