...
//   看板不读任何三元切面(徽章由行内 placement 派生);决策摘要/事实切面归 ⌘K
//   (或任务详情);完整投影归渲染它的视图。台账 cut 变化只重取挂载中的查询。
//   这里直接挂真实的 App,断言桥上真正发出的读面,而不是断言某个内部函数被调用过。
...
describe("三元读取按挂载域分层", () => {
  ...
  it("任务看板不读任何关系切面,也不为关闭的 ⌘K 读决策摘要", async () => {
    ...
    // 徽章改为行内 placement.spawningDecisionIds 派生后,看板不再有 derives
    // 边切面读(此前每次 cut 变化约 258 KB,fix #21/B6)。
    expect(relationGraphCalls(calls())).toEqual([]);
    expect(decisionCalls(calls())).toEqual([]);
  });

  it("台账 cut 前进时,看板不重取任何三元切面,完整投影不被重取", async () => {
    const { calls, mark, advanceLedger } = await mountApp({ view: "board" });
    const atBoard = mark();
    await advanceLedger();
    const since = calls().slice(atBoard);
    expect(relationGraphCalls(since)).toEqual([]);
