...
// plus the operator-shell next action.
test("system status carries a build-skew verdict only when both commits are known and differ", async () => {
  const own = "0123456789abcdef0123456789abcdef01234567",
    staleCommit = "fedcba9876543210fedcba9876543210fedcba98";
  const target = async () => ({
    repoId: "skew-repo",
    socketPath: "/dev/null",
    userRoot: "/tmp/ha-skew-user",
    daemonId: "skew",
  });
  const statusFor = async (daemon: Daemon): Promise<Overlay> => {
    const inner: GuiServiceBridge = {
      invoke: async () => ({
        schema: "gui-system-status/v1",
        ok: true,
        observedAt: "2026-08-21T00:00:00.000Z",
        daemon: { daemonId: "skew", pid: 1, startedAt: "2026-08-21T00:00:00.000Z", ...daemon },
        repos: [],
      }),
  ...
  assert.deepEqual(
    stale.buildStale,
    { daemonCommit: staleCommit, clientCommit: own },
    "a differing daemon commit must surface the skew verdict",
  );
