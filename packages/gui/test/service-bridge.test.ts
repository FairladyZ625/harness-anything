...
test("GUI entity write channel validates then installs an Agent and preserves a Squad roster", async () => {
  ...
  try {
    ...
    assert.equal(existsSync(path.join(fixture.rootDir, "harness/agents/gui-created-agent.json")), true); assert.equal(existsSync(path.join(fixture.rootDir, "harness/squads/gui-created-squad.json")), true); assert.equal(existsSync(path.join(fixture.rootDir, ".harness/agents")), false); assert.equal(existsSync(path.join(fixture.rootDir, ".harness/squads")), false);
