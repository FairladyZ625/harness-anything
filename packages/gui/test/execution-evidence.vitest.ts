...
const ACTOR = {
  ...
  executor: { kind: "agent" as const, id: "fact-execution-migration" },
};
...
describe("execution evidence model", () => {
  it("uses projected origin and joins witnesses only through the exact execution cut", () => {
    ...
    expect(execution.reviews.map(({ reviewId }) => reviewId)).toEqual([
      "review-dismissed",
      "review-unselected",
      "review-matching",
    ]);
    expect(execution.consents.map(({ consentId }) => consentId)).toEqual(["consent-matching"]);
    expect({ reviewId: execution.selectedReviewId, consentId: execution.selectedConsentId }).toEqual({
      reviewId: "review-matching",
      consentId: "consent-matching",
    });
      ...
      executionEvidence: [
        {
          ...
          origin: "archival",
          outputs: [{ checkerReceiptRef: null, checkerResult: "unknown" }],
        },
      ],
      snapshot: {
        ...source.snapshot,
        executions: [
          {
            ...source.snapshot.executions[0]!,
            submission: {
              ...source.snapshot.executions[0]!.submission!,
              outputs: ["artifacts/unprojected.txt"],
            },
          },
        ],
      },
  ...
  it("adapts a TaskRow-shaped source (透传字段) the same as a full projection row", () => {
    ...
    // rawTask embeds the full TaskV2 only when a full row is passed; the adapted
  ...
  it("copies task, execution, output originals, projected fields, receipt, and copy time", () => {
    ...
    const context = buildExecutionEvidenceContext(
      executionRow,
      executionRow.outputs[0]!,
  ...
  const base: TaskSnapshotProjectionRow = {
    ...
    snapshot: {
      ...
      task: {
        ...
        iteration: 0,
        createdBy: ACTOR,
        completionGateIds: ["build"],
        presetSnapshotDigest: null,
        pinned: false,
      },
      executions: [execution("execution-1", 1)],
      reviews: [
        review("review-dismissed", SHA_A, 0, "dismissed"),
        review("review-unselected", SHA_A, 0),
        review("review-matching", SHA_A, 0),
        review("review-other-cut", SHA_B, 1),
      ],
    ...
    placement: {
      ...
      provenance: [{ kind: "canonical-event", ref: `task/${taskId}` }],
    },
    executionEvidence: [
      {
        executionId: "execution-1",
        ...
        outputs: [
          ...
          {
            evidenceId: `evidence_${"2".repeat(24)}`,
...
function execution(executionId: string, minute: number) {
  return {
    ...
    actor: ACTOR,
    claimedAt: `2026-08-14T08:${String(minute).padStart(2, "0")}:00.000Z`,
    submittedAt: `2026-08-14T08:${String(minute).padStart(2, "0")}:30.000Z`,
    closedAt: null,
    submission: {
      completionClaim: "Evidence delivered",
      deliverables: ["result"],
      outputs: ["artifacts/result.txt", "https://example.test/log"],
...
function review(reviewId: string, commitSha: string, iteration: 0 | 1, verdict: "approved" | "dismissed" = "approved") {
  return {
    ...
    reason: "verified",
    evidenceChecked: ["artifacts/result.txt"],
    commitSha,
    iteration,
    contentDigest: DIGEST,
    reviewedAt: "2026-08-14T08:10:00.000Z",
  };
}

function consent(consentId: string, reviewId: string) {
  return {
    schema: "review-consent/v1" as const,
    consentId,
    taskId: "task-evidence",
    executionId: "execution-1",
    reviewId,
    ...
    consentedAt: "2026-08-14T08:11:00.000Z",
  };
}

function gate(witnessId: string, commitSha: string, iteration: 0 | 1) {
  return {
    schema: "completion-gate-witness/v1" as const,
    witnessId,
    receiptId: witnessId === "gate-matching" ? "gate-receipt-matching" : "gate-receipt-other",
    checkerId: "build",
    gateId: "build",
    ...
    actor: ACTOR,
    source: "local" as const,
    verifiedAt: "2026-08-14T08:12:00.000Z",
  };
}
