...
describe("main-only runtime instance credential broker", () => {
  it("never returns the native secret and gives daemon only an opaque reference", async () => {
    ...
    const receipt = await controller.create({ instanceId: "codex-review", name: "Codex Review", kindId: "codex", installationId: "installation-codex", providerId: "openai", model: "gpt-5.6-sol", codex: {}, authMode: "api-key" });
  ...
  it("fails closed before daemon create when native secure input is unavailable", async () => {
    ...
    const receipt = await controller.create({ instanceId: "claude-review", name: "Claude Review", kindId: "claude", installationId: "installation-claude", providerId: "anthropic", model: "claude-opus", claude: {}, authMode: "api-key" });
