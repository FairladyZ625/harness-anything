...
import { credentialPort, runCredentialCommand } from "@harness-anything/daemon/client";
...
const appliedReceipt = (instance: Record<string, unknown> = {}) => ({
  schema: "command-receipt/v2",
  ok: true,
  command: "runtime-instance-create",
  outcome: "applied",
  opId: "runtime-instance-create:1",
  instance: { instanceId: "codex-sidecar", authMode: "api-key", authState: "configured", ...instance },
  evidence: JSON.stringify({ instanceId: "codex-sidecar", authMode: "api-key" }),
  summary: "runtime-instance-create: codex-sidecar",
  nextAction: null,
});
...
describe("main-only runtime instance credential controller", () => {
  it("stores the typed key in the vault and gives the daemon only an opaque reference", async () => {
    ...
    const controller = createRuntimeInstanceCredentialController({
      port: {
  ...
  it("fails closed before daemon create when the vault refuses to store", async () => {
    const create = vi.fn(),
      controller = createRuntimeInstanceCredentialController({
        port: {
          issue: () => "credential:v1:issued-ref",
          store: async () => {
            throw Object.assign(new Error("vault refused"), { code: "runtime_credential_unavailable" });
          },
          resolve: async () => "",
        },
        create,
  ...
  it("rejects an api-key create that arrives without a key instead of creating a dead instance", async () => {
    const create = vi.fn(),
      controller = createRuntimeInstanceCredentialController({
        port: { issue: () => "credential:v1:issued-ref", store: async () => undefined, resolve: async () => "" },
        create,
      });
  ...
  it("keeps the Electron main event loop responsive while the credential backend waits", async () => {
    const port = credentialPort("darwin", () =>
      runCredentialCommand({ file: process.execPath, args: ["-e", "setTimeout(() => process.exit(0), 200)"] }),
    );
    ...
    try {
      expect(await controller.create({ ...base, authMode: "api-key", apiKey: secret })).toMatchObject({ ok: true });
    } finally {
