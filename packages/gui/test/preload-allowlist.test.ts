...
test("preload exposes only the approved API methods", () => {
  ...
  // 40 explicit actions plus the complete declaration read are the 41 editing-facing facets.
  const editingFacets = [
    ...daemonGuiActionMethods.map(({ guiBridgeMethod }) => guiBridgeMethod),
    "readVerticalDeclaration",
  ];
  assert.equal(editingFacets.length, 41);
  ...
  assert.equal(daemonGuiActionMethods.length, 40);
  assert.equal(
