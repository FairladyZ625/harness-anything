...
function decisionRow(overrides: Partial<DecisionRow> = {}): DecisionRow {
  return {
    ...
    judgmentConsents: [{ schema: "decision-judgment-consent/v1", consentId: "djc_1", decisionId: "dec_1", action: "accept", targetState: "in_effect", machineDigest: "sha256:ab", actor: { principal: { personId: "person-ceo" }, executor: null }, source: "local", consentedAt: "2026-08-01T00:00:00.000Z" }],
    proposedAt: "2026-08-01T00:00:00.000Z", decidedAt: "2026-08-02T00:00:00.000Z",
    vertical: "software-coding", preset: "plt-gui", decisionClass: "ordinary",
    workspaceRevision: 7, appliesTo: { modules: ["gui"], productLines: ["platform"] },
    proposedBy: { kind: "human", id: "person-ceo" }, arbiter: { kind: "human", id: "person-ceo" },
...
function showReceipt(body: string | null, status: "ready" | "pending" = "ready") {
  return {
    schema: "command-receipt/v2", ok: true, command: "decision-show", outcome: status === "ready" ? "applied" : "pending", opId: "read:decision-show", revision: 7,
    evidence: JSON.stringify({
      status, watermark: 7, sourceRevision: 7,
      decision: {
        schema: "decision-row/v1", decisionId: "dec_1", path: "decisions/decision-dec_1/decision.md",
        state: "in_effect", title: "暴露投影", question: "走哪条读径?", riskTier: "low", urgency: "low",
        vertical: "software-coding", preset: "plt-gui", decisionClass: "ordinary",
        appliesTo: { modules: ["gui"], productLines: ["platform"] },
        proposer: { principal: { personId: "person-ceo" }, executor: null }, arbiter: { principal: { personId: "person-ceo" }, executor: null },
        proposedAt: "2026-08-01T00:00:00.000Z", decidedAt: "2026-08-02T00:00:00.000Z", workspaceRevision: 7,
        chosen: [], rejected: [], claims: [], provenance: [], judgmentConsents: [],
        body: body === null ? null : { path: "decisions/decision-dec_1/decision.md", blobSha256: `sha256:${"a".repeat(64)}`, size: body.length, mediaType: "text/markdown", body, workspaceRevision: 7 },
      },
    }),
    visibility: "center", proof: { committedRevision: 7, appliedCut: 7, durable: true, canonicalVisible: status === "ready", worktreeVisible: null },
...
afterEach(async () => {
  await act(async () => { for (const { root, client } of queryMounted.splice(0)) { root.unmount(); client.clear(); } });
  vi.unstubAllGlobals();
});
...
async function mountDecisionView(decision: DecisionRow | null, props: Record<string, unknown> = {}) {
  ...
  await act(async () => {
    root.render(createElement(QueryClientProvider, { client }, createElement(DecisionDetailView, {
      repoId: "repo-a", decisionId: decision?.decisionId ?? "dec_1", decisions: decision ? [decision] : [], relations,
      loading: false, onBack: () => undefined, projectName: "Harness", fromViewLabel: "决策池",
      onNavigateDecision: () => undefined, onNavigateEntity: () => undefined, ...props,
    })));
  });
...
describe("DecisionDetailView", () => {
  ...
  it("投影未返回正文时说出原因,不显示空白(负向不变量)", async () => {
    ...
    expect(div.querySelector("[data-testid='decision-body-unavailable']")?.textContent).toContain("投影未返回该决策的正文");
  ...
  it("投影追赶(pending)与读取失败各自显式说明", async () => {
    ...
    expect(pending.querySelector("[data-testid='decision-body-pending']")?.textContent).toContain("Retry decision-show after projection catch-up.");

    await act(async () => { for (const { root, client } of queryMounted.splice(0)) { root.unmount(); client.clear(); } });
    vi.stubGlobal("window", { harness: { showDecision: vi.fn(async () => ({ schema: "command-receipt/v2", ok: false, command: "decision-show", outcome: "op_rejected", opId: "op-x" })) } });
  ...
  it("长正文分批渲染并上报剩余量(规模不变量)", async () => {
    const longBody = Array.from({ length: 30 }, (_, index) => `第 ${index} 节内容,验证长正文分批显形。`).join("\n\n");
    vi.stubGlobal("window", { harness: { showDecision: vi.fn(async () => showReceipt(longBody)) } });
    const div = await mountDecisionView(decisionRow());

    expect(div.querySelectorAll("[data-testid='decision-body-block']").length).toBe(12);
    const more = div.querySelector<HTMLButtonElement>("[data-testid='decision-body-more']")!;
    expect(more.textContent).toContain("还有 18 块");

    await act(async () => { more.click(); });
    expect(div.querySelectorAll("[data-testid='decision-body-block']").length).toBe(24);
    expect(div.querySelector("[data-testid='decision-body-more']")?.textContent).toContain("还有 6 块");
  });
  ...
  it("身份条与四个分页签齐备;池/图出口带决策 ID", async () => {
    ...
    const onOpenPool = vi.fn(), onFocusGraph = vi.fn();
    const div = await mountDecisionView(decisionRow(), { onOpenPool, onFocusGraph });

    expect([...div.querySelectorAll('[role="tab"]')].map((tab) => tab.textContent?.trim())).toEqual(["正文", "概况", "承重与裁决", "关系"]);
  ...
  it("概况/承重与裁决/关系分页签渲染决策结构信息", async () => {
    ...
    expect(div.querySelector("[data-testid='decision-panel-claims']")?.textContent).toContain("正文必须经单体 read 取回");
