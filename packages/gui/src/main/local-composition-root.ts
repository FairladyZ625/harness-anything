...
export function createLocalGuiServiceBridge(rootDir: string, _layoutOverrides?: { readonly authoredRoot?: string }): GuiServiceBridge {
  const root = path.resolve(rootDir); validateProjectPath(root, "."); return createGuiServiceBridgeForDaemon((route, payload) => request(root, route, payload), async (route, payload, emit) => {
...
async function request(rootDir: string, route: ShippedGuiRoute, payload: unknown): Promise<JsonObject> { try {
  ...
  const result = await daemon.requestLocalDaemonJsonRpcForTarget(target, route.rpcMethod, params, 200); return (isDaemonGuiActionMethod(route.rpcMethod) ? parseDaemonGuiActionResponse(route.rpcMethod, result) : route.rpcMethod === "daemon.gui.control.receipt" ? parseDaemonGuiReadResult(route.rpcMethod, result) : parseDaemonGuiReadResponse(route.rpcMethod, result)) as unknown as JsonObject;
...
function globalTarget(daemon: DaemonClient): { readonly socketPath: string } { const userRoot = daemon.daemonUserRoot(), daemonId = daemon.daemonIdFromEnv(); return { socketPath: daemon.localUserDaemonEndpoint(userRoot, daemonId) }; }
