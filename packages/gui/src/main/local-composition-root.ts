...
async function request(rootDir: string, route: ShippedGuiRoute, _payload: unknown): Promise<JsonObject> { try {
  ...
  return parseDaemonGuiReadResponse(route.rpcMethod, await daemon.requestLocalDaemonJsonRpcForTarget(target, route.rpcMethod, { repo: { repoId: target.repoId } }, 200)) as unknown as JsonObject;
