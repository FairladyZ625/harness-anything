...
export function addLocalMainControls(input: { readonly bridge: GuiServiceBridge; readonly target: (repoId?: string) => Promise<Target>; readonly packaged?: PackagedRuntime; readonly credentialPort?: CredentialPort }): GuiServiceBridge {
  ...
  const runtimeRpc = async (operation: string, payload: Record<string, unknown>) => requestDaemonJsonRpcAt((await input.target()).socketPath, `daemon.runtimeInstance.${operation}`, { payload } as never, ["create", "list"].includes(operation) || payload.probe === true ? 12_000 : 2_000), credentialController = createRuntimeInstanceCredentialController({ ...(input.credentialPort ? { port: input.credentialPort } : {}), create: (payload) => runtimeRpc("create", payload) });
  ...
  return { stream: input.bridge.stream, invoke: async (method, payload) => {
    if (method === "listRuntimeInstances") { const listed = asRecord(await runtimeRpc("list", { all: true })), rows = Array.isArray(listed.instances) ? listed.instances.map(asRecord) : []; if (listed.ok !== true) return listed; const checked = await Promise.allSettled(rows.map(async (instance) => asRecord(asRecord(await probeRuntimeInstance(String(instance.instanceId))).instance))); for (const result of checked) if (result.status === "rejected") consumeKnownError(result.reason); return { ...listed, instances: rows.map((instance, index) => { const result = checked[index]; return { ...instance, ...(result?.status === "fulfilled" && result.value.authReadiness ? { authReadiness: result.value.authReadiness } : {}) }; }) }; }
