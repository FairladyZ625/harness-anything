...
export function addLocalMainControls(input: { readonly bridge: GuiServiceBridge; readonly target: (repoId?: string) => Promise<Target>; readonly packaged?: PackagedRuntime; readonly credentialPort?: CredentialPort }): GuiServiceBridge {
  ...
  return { stream: input.bridge.stream, invoke: async (method, payload) => {
    if (method === "listRuntimeInstances") { const listed = asRecord(await runtimeRpc("list", { all: true })), rows = Array.isArray(listed.instances) ? listed.instances.map(asRecord) : []; if (listed.ok !== true) return listed; const checked = await Promise.all(rows.map(async (instance) => asRecord(asRecord(await probeRuntimeInstance(String(instance.instanceId))).instance))); return { ...listed, instances: rows.map((instance, index) => ({ ...instance, ...(checked[index]?.authReadiness ? { authReadiness: checked[index]!.authReadiness } : {}) })) }; }
