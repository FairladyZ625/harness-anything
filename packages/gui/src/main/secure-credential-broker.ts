...
type Kind = "claude" | "codex"; type CreatePayload = { readonly instanceId: string; readonly name: string; readonly kindId: Kind; readonly installationId: string; readonly providerId: string; readonly model: string; readonly claude?: { readonly baseUrl?: string }; readonly codex?: { readonly reasoningEffort?: string; readonly baseUrl?: string; readonly wireApi?: string; readonly requiresOpenAiAuth?: boolean; readonly httpHeaders?: Readonly<Record<string, string>> }; readonly authMode: "subscription" | "api-key"; readonly apiKey?: string }; type Receipt = Readonly<Record<string, unknown>>;
export function createRuntimeInstanceCredentialController(input: { readonly port?: CredentialPort; readonly create: (payload: Record<string, unknown>) => Promise<Receipt> }) { const port = input.port ?? credentialPort(); return { create: async (payload: CreatePayload): Promise<Receipt> => {
  if (payload.authMode === "subscription") return input.create(payload);
