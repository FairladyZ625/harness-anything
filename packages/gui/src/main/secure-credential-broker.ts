...
import { credentialPort, type CredentialPort } from "@harness-anything/daemon/client";
import { runtimeKindForId, type RuntimeKindId } from "@harness-anything/daemon/protocol";
...
export function createRuntimeInstanceCredentialController(input: {
  ...
  return {
    create: async (payload: CreatePayload): Promise<Receipt> => {
      ...
      if (!runtimeKindForId(payload.kindId).auth.modes.some((mode) => mode === "api-key"))
        return unavailable(
          "runtime_auth_mode_mismatch",
          `${payload.kindId} runtime instances do not support API keys.`,
        );
