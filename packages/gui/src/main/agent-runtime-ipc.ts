...
} function checkedEnvelope(value: unknown): { readonly subscriptionId: string; readonly payload: DaemonGuiStreamPayloadMap[keyof DaemonGuiStreamPayloadMap] } { if (!record(value) || typeof value.subscriptionId !== "string" || !record(value.payload)) throw new Error("Stream envelope is invalid."); return value as { subscriptionId: string; payload: DaemonGuiStreamPayloadMap[keyof DaemonGuiStreamPayloadMap] }; }
function checkedSubscription(value: unknown): string { if (!record(value) || typeof value.subscriptionId !== "string") throw new Error("Agent runtime subscription id is required."); return value.subscriptionId; } function record(value: unknown): value is Record<string, unknown> { return value !== null && typeof value === "object" && !Array.isArray(value); }
