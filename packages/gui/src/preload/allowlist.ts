...
function validRuntimeKindConfig(value: Record<string, unknown>): boolean { const field = value.kindId === "codex" ? "codex" : "claude", other = field === "codex" ? "claude" : "codex", config = value[field]; return value[other] === undefined && isPreloadPayloadRecord(config) && closed(config, field === "codex" ? ["reasoningEffort", "baseUrl"] : ["baseUrl"]) && Object.values(config).every((item) => typeof item === "string"); }
