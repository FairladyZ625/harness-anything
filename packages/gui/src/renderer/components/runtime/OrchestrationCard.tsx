    ...
    <Card dashed>
      ...
      <Sect title={t("agentRuntime.traceChain")} desc={t("agentRuntime.traceChainDesc")}>
        {chain.length === 0 ? <Empty>{t("agentRuntime.noDispatches")}</Empty> : <div className="grid gap-2.5 lg:grid-cols-3">
          ...
          <Column testId="orchestration-dispatches" label={t("agentRuntime.dispatches")} count={counts.dispatches}>{chain.map((row) => <button type="button" key={`d-${row.dispatchId}`} data-testid={`orchestration-session-${row.dispatchId}`} data-session={row.ledger?.runtimeSessionId ?? null} onClick={() => { if (row.dispatchRecord) setSelected({ ...row.dispatchRecord, kind: "dispatch" }); if (row.ledger) onFocusSession(row.ledger.runtimeSessionId); }} className={`w-full rounded border px-2 py-1.5 text-left ${chainTone(selected, row.dispatchId, selected?.kind === "dispatch")}`}>
