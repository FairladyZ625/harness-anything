  ...
  return (
    <aside className="sticky top-4 hidden self-start rounded-lg border border-border bg-surface px-3 py-3 lg:block">
      <div className="flex items-center justify-between gap-2">
        <span className={SECTION_LABEL}>Registry 上下文</span>
      ...
      <div className="mt-3 border-b border-border pb-3">
        ...
        <div className="mt-1 flex gap-1">
          ...
          <span className={CHIP}>{focusedAdapter.writable ? "writable" : "read-only"}</span>
        </div>
      </div>

      <div className="border-b border-border py-3">
        <div className="font-mono text-[17px] font-semibold text-text">{projectedCount}</div>
        <div className="text-[10px] text-text-faint">当前投影中的任务</div>
      </div>
      ...
      <div className="pt-3">
        <div className="text-[10px] text-text-faint">真实注册项</div>
        <div className="mt-1.5 flex flex-col gap-1">
          {adapters.map((adapter) => (
            <div key={adapter.engine} className="flex items-center gap-2 text-[11px]">
              ...
              <span className="ml-auto font-mono text-text-faint">{adapter.capabilities.length} caps</span>
            </div>
          ))}
        </div>
      </div>
