  ...
  return <section data-testid="orchestration-panel" aria-label={t("components.orchestration.title")} className="border-b border-border bg-bg px-4 py-4">
    ...
    {taskId === "" ? <p className="text-[11px] text-text-faint">{t("components.orchestration.noTasks")}</p> : <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
      <div className="grid gap-3 lg:grid-cols-3">
        <ChainColumn testId="orchestration-missions" label={t("components.orchestration.missions")} count={chain.filter((row) => row.mission).length}>{chain.map((row) => <ChainCell key={`m-${row.dispatchId}`} row={row} pick={row.mission} selected={selected} onSelect={setSelected}>{row.dispatchId}</ChainCell>)}</ChainColumn>
        <ChainColumn testId="orchestration-dispatches" label={t("components.orchestration.dispatches")} count={chain.length}>{chain.map((row) => <button type="button" key={`d-${row.dispatchId}`} onClick={() => { setSelected(row.dispatchRecord); if (row.ledger) onFocusSession(row.ledger.runtimeSessionId); }} className={`w-full rounded border px-2 py-1.5 text-left ${selected?.dispatchId === row.dispatchId && selected.path.endsWith(".json") ? "border-accent bg-accent/10" : "border-transparent hover:bg-surface-raised"}`}>
        ...
        <ChainColumn testId="orchestration-reports" label={t("components.orchestration.reports")} count={chain.filter((row) => row.report).length}>{chain.map((row) => <ChainCell key={`r-${row.dispatchId}`} row={row} pick={row.report} selected={selected} onSelect={setSelected}>{row.report ? row.dispatchId : `${row.dispatchId} · ${t("components.orchestration.pending")}`}</ChainCell>)}</ChainColumn>
      </div>
