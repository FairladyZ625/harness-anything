...
export const MOCK_RELATIONS: RelationEdge[] = [
  ...
  { from: "fact/KER-106/F-perf-v2", to: "fact/KER-106/F-perf-baseline", kind: "supersedes-fact", provenance: "local-document", rationale: "重测 450ms 推翻 <200ms 旧基线,测量口径不同" },
  ...
  // Implicit produces edges connecting Tasks to Facts
  { from: "task/KER-101", to: "fact/KER-101/F-lesson-drift", kind: "produces", provenance: "local-document" },
  { from: "task/KER-106", to: "fact/KER-106/F-perf-baseline", kind: "produces", provenance: "local-document" },
  { from: "task/KER-106", to: "fact/KER-106/F-perf-v2", kind: "produces", provenance: "local-document" },
  { from: "task/STO-210", to: "fact/STO-210/F-journal-idem", kind: "produces", provenance: "local-document" },
  { from: "task/KER-109", to: "fact/KER-109/F-pty-bench", kind: "produces", provenance: "local-document" },
  { from: "task/GUI-403", to: "fact/GUI-403/F-csp-audit", kind: "produces", provenance: "local-document" },

  // ===== 信号灯样本(DEC-109~112)的 supports 边 =====
  // DEC-109 chosen 引用已失效的 F-perf-baseline(被 F-perf-v2 invalidated)→ evidence 活性灯黄
  { from: "decision/DEC-109/CH1", to: "fact/KER-106/F-perf-baseline", kind: "evidenced-by", provenance: "local-document", rationale: "旧性能基线曾支撑深度上限论证,但已被重测推翻" },
  // DEC-110 chosen → 活 fact(覆盖度可达,evidence 活性绿;但 applies_to 漂移黄)
  { from: "decision/DEC-110/CH1", to: "fact/KER-109/F-pty-bench", kind: "evidenced-by", provenance: "local-document", rationale: "PTY 基准说明锁竞争在并发下可观测" },
  // DEC-112 chosen → 活 fact(覆盖度可达;但冲突标记红)
  { from: "decision/DEC-112/CH1", to: "fact/KER-109/F-pty-bench", kind: "evidenced-by", provenance: "local-document", rationale: "daemon 并发基准证明 service 同面写的吞吐可行" },
