...
export function compatibleDispatchInstances(runtimeType: string, instances: readonly AgentRuntimeInstanceDto[]): readonly AgentRuntimeInstanceDto[] { return instances.filter((instance) => instance.enabled && instance.kindId === runtimeType); }
export function requireCompatibleDispatchInstance(request: Pick<DispatchRequest, "subject" | "workerId" | "runtimeInstanceId">, instances: readonly AgentRuntimeInstanceDto[]): AgentRuntimeInstanceDto { const executor = dispatchExecutorRef(request), instance = instances.find((row) => row.instanceId === request.runtimeInstanceId); if (!executor) throw new Error("dispatch_executor_missing"); if (!instance || !instance.enabled || instance.kindId !== executor.runtimeType) throw new Error("dispatch_runtime_type_mismatch"); return instance; }
export function buildDispatchSpawnInput(request: DispatchRequest, instances: readonly AgentRuntimeInstanceDto[]): RuntimeSpawnInput {
  const executor = dispatchExecutorRef(request); requireCompatibleDispatchInstance(request, instances); if (!executor) throw new Error("dispatch_executor_missing"); const squadRouting = request.subject.kind === "squad" ? { agentId: request.subject.leader.agentId, targetAgentId: executor.agentId } : { agentId: executor.agentId };
  return { runtimeInstanceId: request.runtimeInstanceId, ...squadRouting, ...(request.model ? { model: request.model } : {}), ...(request.effort ? { effort: request.effort } : {}), cwd: request.cwd, prompt: request.mission, taskId: request.taskId, idempotencyKey: request.idempotencyKey };
}
