...
export function DecisionsView({ decisions, tasks, relations, facts, onCallAgent, onJudge, mutationFeedback, onCheckReceipt, relationState = "ready", onNavigateDecision, onNavigateTask, onFocusGraph, coverageRows = [] }: {
  ...
  return <div className="flex h-full flex-col">
    ...
    <div className="flex min-h-0 flex-1"><div className="flex min-w-0 flex-1 flex-col"><div className="flex-1 overflow-auto p-4">
      ...
      {history.length > 0 && <section className="mt-4 rounded-lg border border-border bg-surface p-3"><h2 className="font-mono text-[11px] font-semibold uppercase tracking-wide text-text-faint">Canonical judgment history</h2><ul className="mt-1.5 space-y-1">{history.slice(0, 12).map(({ decision, consent }) => { const receipt = mutationFeedback?.(decision.decisionId)?.receipt; return <li key={consent.consentId} className="text-[11px] leading-relaxed"><span className="font-mono text-text-muted">{consent.action} · {decision.decisionId} · {consent.consentId}</span><span className="ml-2 break-all text-text-faint">{receipt?.path ?? decision.path ?? "path unknown"} · commit {receipt?.commitSha?.slice(0, 10) ?? "not in current session"}</span></li>; })}</ul></section>}
    </div>
