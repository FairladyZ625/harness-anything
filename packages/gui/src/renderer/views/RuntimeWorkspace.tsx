...
export function RuntimeWorkspace({ repoId, tasks }: { readonly repoId: string; readonly tasks: readonly { readonly taskId: string; readonly title: string }[] }) {
  ...
  const [busy, setBusy] = useState(false), [settlement, setSettlement] = useState<RuntimeSpawnSettlement | null>(null), [credential, setCredential] = useState<RuntimeCredentialReceipt | null>(null), [revision, setRevision] = useState(0), [error, setError] = useState<string | null>(null);
  ...
  const configure = async (kindId: "claude" | "codex", baseUrl?: string) => { if (busy) return; setBusy(true); setError(null); try { const receipt = await runtimeCommandClient.configureCredential(kindId, baseUrl); setCredential(receipt); if (receipt.ok) await reread(); } catch (cause) { consumeKnownError(cause); setError(message(cause)); } finally { setBusy(false); } };
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden">{error && <p role="alert" className="border-b border-border bg-status-blocked/10 px-4 py-2 text-xs text-status-blocked">{error}</p>}<RuntimeControlPanel overview={overview.data} tasks={tasks} busy={busy} settlement={settlement} credentialReceipt={credential} onSpawn={spawn} onConfigureCredential={configure}/><AgentRuntimeView key={`${repoId}:${revision}`} repoId={repoId}/></div>;
}
