...
export const terminalClient = {
  ...
  input: async (repoId: string, sessionId: string, clientSeq: number, utf8: string): Promise<number> => {
    ...
    if (!record(result) || result.schema !== "terminal-input-ack/v1" || result.ok !== true || !Number.isSafeInteger(result.acceptedThrough)) throw new Error(hint(result, "Terminal input was not acknowledged."));
  ...
  detach: async (repoId: string, sessionId: string, attachmentId: string): Promise<unknown> => {
    ...
    if (!record(result) || result.schema !== "terminal-detach-ack/v1" || result.ok !== true || result.state !== "detached") throw new Error(hint(result, "Terminal detach was not acknowledged."));
    return result;
  },
...
function sessionList(value: unknown): TerminalSessionList {
  if (!record(value) || value.schema !== "terminal-session-list/v1" || value.ok !== true || typeof value.repoId !== "string" || !Number.isSafeInteger(value.daemonGeneration) || !Array.isArray(value.sessions)) throw new Error(hint(value, "Terminal session list is invalid."));
  return value as unknown as TerminalSessionList;
}
function control(value: unknown): TerminalControlReceipt {
  if (!record(value) || value.schema !== "terminal-control-receipt/v1" || typeof value.operationId !== "string" || !["applied", "rejected"].includes(String(value.outcome))) throw new Error(hint(value, "Terminal control receipt is invalid."));
...
function attachValue(value: unknown): TerminalAttachInitial | TerminalStreamFrame {
  if (!record(value)) throw new Error("Terminal stream returned an invalid frame.");
