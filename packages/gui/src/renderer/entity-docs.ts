...
export const KERNEL_ENTITY_CONTRACT = Object.freeze({
  ...
  "runtime-instance": {
    ...
    statuses: [
      ...
      {
        field: "kindId",
        words: ["claude", "codex", "agy", "zcode", "devin"],
      },
    ],
...
const runtimeInstanceDoc: EntityKindDoc = {
  ...
  definition:
    "可创建、停用和删除的 provider 配置实例。kind 是实例属性而不是独立实体:claude / codex / agy / zcode / devin " +
  ...
  fields: [
    ...
    field("kindId", true, "enum", "内置适配器词表:claude / codex / agy / zcode / devin。"),
