/**
 * 前向截断历史栈的泛型核心(经典浏览器语义;移植老 main 线同名模块)。
 *
 * 图内焦点历史(focusHistory.ts,条目=实体 id)与应用位置历史
 * (viewHistory.ts,条目=AppLocation)只在两处不同:条目类型,以及
 * 「两个条目算不算同一个」的判据。迁移逻辑不各写一遍,这里只存在一份。
 *
