...
export function reduceTerminalStream(tab: TerminalTab, frame: TerminalStreamFrame): TerminalTab {
  ...
  if (frame.kind === "gap") return { ...tab, lastSeq: frame.seq, notice: `Daemon backpressure dropped output through sequence ${frame.droppedThrough ?? "unknown"}; reattach for bounded replay.` };
