...
export const localGitWorktreeSettlement = Object.freeze({
  readNode,
  settle: (repoRoot: string, files: readonly { readonly target: string; readonly body: string; readonly mode?: "100644" | "120000" }[], hooks: { readonly whileFilesSync?: () => void; readonly beforeRename?: () => void; readonly afterRename?: () => void } = {}): number => { const directories = new Set<string>(), pending = files.map((file, index) => { const target = path.join(repoRoot, ...file.target.split("/")), directory = path.dirname(target), temporary = path.join(directory, `.ha-settle-${process.pid}-${index}`), mode = file.mode ?? "100644"; mkdirSync(directory, { recursive: true }); directories.add(directory); removeNode(temporary); return { target, temporary, body: file.body, mode, logical: file.target }; }), regular = pending.filter(({ mode }) => mode === "100644"), links = pending.filter(({ mode }) => mode === "120000"), fileSync = beginDurableSettlement({ files: regular }); try { hooks.whileFilesSync?.(); awaitDurableSettlement(fileSync); } catch (error) { try { awaitDurableSettlement(fileSync); } catch (syncError) { consumeKnownError(syncError); } for (const item of regular) removeNode(item.temporary); throw error; } for (const item of regular) { hooks.beforeRename?.(); renameSync(item.temporary, item.target); hooks.afterRename?.(); } const indexInput = pending.map((file) => `${file.mode} ${gitBlobOid(file.body)}\t${file.logical}\0`).join(""); if (files.length) localGitProcesses += 1; awaitDurableSettlement(beginDurableSettlement({ index: files.length ? { repoRoot, input: indexInput } : undefined })); for (const item of links) { hooks.beforeRename?.(); runGit(repoRoot, "checkout-index", "--force", "--", item.logical); hooks.afterRename?.(); } awaitDurableSettlement(beginDurableSettlement({ directories: [...directories] })); return files.length + directories.size; },
  preserveConflict: (repoRoot: string, target: string, logical: string, commit: string): string => { const node = readNode(target); if (!node) throw new Error(`conflicting worktree node disappeared at ${logical}`); const extension = path.extname(target), stem = target.slice(0, target.length - extension.length), id = hash("sha256", `${logical}\0${commit}\0${node.mode}\0${hash("sha256", node.body)}`).slice(0, 8), scratch = `${stem}.conflict-${id}${extension}`, relative = path.relative(repoRoot, scratch).split(path.sep).join("/"); ensureConflictExclude(repoRoot); if (!readNode(scratch)) { if (node.mode === "120000") checkoutDetachedSymlink(repoRoot, relative, node.body); else durableWrite(scratch, Buffer.from(node.body)); } return relative; }
});
function readNode(target: string): { readonly mode: "100644" | "120000"; readonly body: string } | null { try { const info = lstatSync(target); return info.isSymbolicLink() ? { mode: "120000", body: readlinkSync(target) } : info.isFile() ? { mode: "100644", body: readFileSync(target, "utf8") } : null; } catch (error) { consumeKnownError(error); return null; } }
