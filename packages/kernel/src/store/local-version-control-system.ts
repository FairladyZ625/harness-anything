...
export const localGitWorktreeSettlement = Object.freeze({
  ...
  settle: (repoRoot: string, files: readonly { readonly target: string; readonly body: string }[], hooks: { readonly whileFilesSync?: () => void; readonly beforeRename?: () => void; readonly afterRename?: () => void } = {}): number => { const directories = new Set<string>(), pending = files.map((file, index) => { const target = path.join(repoRoot, ...file.target.split("/")), directory = path.dirname(target), temporary = path.join(directory, `.ha-settle-${process.pid}-${index}`); mkdirSync(directory, { recursive: true }); directories.add(directory); return { target, temporary, body: file.body }; }), fileSync = beginDurableSettlement({ files: pending }); try { hooks.whileFilesSync?.(); awaitDurableSettlement(fileSync); } catch (error) { try { awaitDurableSettlement(fileSync); } catch (syncError) { consumeKnownError(syncError); } for (const item of pending) if (existsSync(item.temporary)) unlinkSync(item.temporary); throw error; } for (const item of pending) { hooks.beforeRename?.(); renameSync(item.temporary, item.target); hooks.afterRename?.(); } const indexInput = files.map((file) => `100644 ${gitBlobOid(file.body)}\t${file.target}\0`).join(""); if (files.length) localGitProcesses += 1; awaitDurableSettlement(beginDurableSettlement({ directories: [...directories], index: files.length ? { repoRoot, input: indexInput } : undefined })); return files.length + directories.size; },
