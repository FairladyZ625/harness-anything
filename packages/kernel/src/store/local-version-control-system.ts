...
function runGitWithEnvironment(repoRoot: string, env: NodeJS.ProcessEnv, args: readonly string[], input?: Uint8Array): string { localGitProcesses += 1; const invocation = gitInvocation(repoRoot, args); try { return execFileSync(invocation.command, invocation.args, { env, input, encoding: "utf8", maxBuffer: gitMaxBuffer, stdio: [input ? "pipe" : "ignore", "pipe", "pipe"], windowsHide: true, ...(process.platform === "win32" ? { windowsVerbatimArguments: true } : {}) }); } catch (error) { throw new VcsCommandError({ command: args[0] ?? "command", cwd: repoRoot, exitCode: commandErrorCode(error), signal: commandErrorSignal(error), stderrSummary: commandErrorSummary(error) }); } }
