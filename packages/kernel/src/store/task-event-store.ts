...
export interface EventRecoveryReceipt {
  readonly status: "none" | "cleared" | "committed" | "already_committed" | "indeterminate";
...
export interface TaskEventStore {
  ...
  readonly append: (event: TaskEventV1) => TaskEventAppendReceipt;
  readonly recover: () => EventRecoveryReceipt;
  readonly rebuildHead: () => EventHead | null;
}
...
export function makeTaskEventStore(options: {
  ...
  return {
    ...
    append: (event) => {
      ...
      const existing = readEvent(event.opId);
      if (existing !== null) {
    ...
    rebuildHead: () => deriveEventHead(readTaskEventFiles(eventsRoot, undefined).events)
  };
}
...
function sameHead(left: EventHead | null, right: EventHead | null): boolean {
  if (left === null || right === null) return left === right;
  return serializeEventHead(left) === serializeEventHead(right);
}
...
function readLooseRef(refPath: string): string | null {
  if (!localEventFileSystem.exists(refPath)) return null;
  const sha = localEventFileSystem.readText(refPath).trim();
...
function deriveEventHead(events: readonly TaskEventV1[]): EventHead | null {
  const event = events.at(-1);
  if (event === undefined) return null;
  return { revision: event.workspaceRevision, opId: event.opId,
    eventDigest: `sha256:${sha256Text(serializeTaskEvent(event))}` };
}

function eventPath(eventsRoot: string, opId: string): string {
  if (opId === "head" || opId.length === 0 || /[\\/]/u.test(opId) || opId === "." || opId === "..") {
    throw new TaskEventStoreError("invalid_store", "event opId is not a safe filename");
  }
...
function shapeError(value: unknown, message: string): TaskEventStoreError {
  ...
  return new TaskEventStoreError(legacy ? "legacy_shape" : "invalid_store", legacy
