...
import {
  assertAgentEntityWritePlan,
  isAgentEntityEvent,
} from "../domain/agent-entity-event.ts";
import {
...
import {
  ...
  type LedgerGitLayout,
} from "./ledger-git-layout.ts";
import {
...
export function validateCanonicalWriteBundle(
  bundle: CanonicalWriteBundle,
): void {
  assertBundle(bundle);
}
export function canonicalEventContentClaims(
...
type GitFileMode = "100644" | "120000";
interface PublicationWrite {
  readonly target: string;
  readonly body: string;
  readonly mode: GitFileMode;
}
interface PublicationRename {
  readonly from: string;
  readonly to: string;
}
interface PublicationDelete {
  readonly delete: string;
}
type PublicationFile = PublicationWrite | PublicationRename | PublicationDelete;
...
export function canonicalEventWritePlan(
  event: CanonicalEventV1,
  projection: string,
  key: string,
): FrozenWritePlan {
...
export function makeTaskEventStore(options: {
  ...
  if (!refs.authored)
    throw new TaskEventStoreError(
      "publication_indeterminate",
      "registered authored branch cannot be resolved",
    );
  ...
  let initialPrepared: readonly (readonly [string, string])[] | null =
    refs.prepared;
  ...
  if (!refs.canonical)
    updateRef(repoRoot, CANONICAL_EVENT_REF, canonicalCommit);
  const currentCommit = () => ledgerCommitSha(repoId, canonicalCommit);
  const readHead = () => {
    if (canonicalHead === undefined)
      canonicalHead = readHeadAt(ledger, canonicalCommit);
  ...
  const readContentBlob = (sha256: string) =>
    recentContent.get(sha256) ?? readBlobAt(ledger, canonicalCommit, sha256);
  const publish = (
    bundle: CanonicalWriteBundle,
    ...
    const writeLayout: LedgerObjectLayout = isLedgerLayoutMigrationEvent(event)
      ...
      : layoutState === undefined
        ? (layoutState = writeLayoutAt(ledger, parent.sha))
        : layoutState === "mixed"
          ? (() => {
              throw mixedLayoutError(scanEventsRoot(ledger, parent.sha)!);
            })()
          : layoutState;
    const existing =
      recentEvents.get(event.opId) ??
      (knownOpIds !== null && !knownOpIds.has(event.opId)
        ? null
        : readEventAt(ledger, parent.sha, event.opId));
    if (existing !== null) {
      rememberEvents([existing]);
      if (serializeCanonicalEvent(existing) !== eventBytes)
        throw new TaskEventStoreError(
          "op_conflict",
          `opId ${event.opId} already names different event bytes`,
        );
    ...
    if (isDecisionEvent(event)) {
      const current = showText(
        repoRoot,
        parent.sha,
        ledgerGitPath(ledger, event.payload.decisionDocumentClaim.path),
      );
      const base = current === null ? null : sha256Text(current);
      if (event.payload.baseDocumentSha256 !== base)
        throw new TaskEventStoreError(
          "revision_conflict",
          `Decision ${event.decisionId} document base changed`,
        );
    }
    ...
    assertAuthorizedReplacement(ledger, parent.sha, event);
    if (
      isDocEvent(event) &&
      stableStringify(event.payload.baseLedgerSha) !==
        stableStringify(canonicalLedgerCut(repoId, previousHead))
    )
      throw new TaskEventStoreError(
        "repo_mismatch",
    ...
    const files: PublicationFile[] = [
      {
        target: eventObjectPath(ledger, event.opId, writeLayout),
        body: eventBytes,
        mode: "100644",
      },
      {
        target: ledgerGitPath(ledger, "events/head.json"),
        body: serializeEventHead(head),
        mode: "100644",
      },
      ...additionalFiles,
    ];
    const uncached = blobs
      .filter((blob) => !recentContent.has(blob.sha256))
      .map((blob) => blob.sha256);
    const existingBlobs = readBlobsAt(ledger, parent.sha, uncached);
    for (const claim of blobs) {
      const existingBlob =
        recentContent.get(claim.sha256) ??
        existingBlobs.get(claim.sha256) ??
        null;
      if (existingBlob !== null) {
        if (
          sha256Text(Buffer.from(existingBlob).toString("utf8")) !==
          claim.sha256
        )
          throw new TaskEventStoreError(
            "invalid_store",
            `reachable content blob ${claim.sha256} is corrupt`,
          );
      } else {
        files.push({
          target: blobObjectPath(ledger, claim.sha256, writeLayout),
          body: claim.body,
          mode: "100644",
        });
      }
    ...
    for (const claim of canonicalDocumentClaims(event)) {
      ...
      if (!blob)
        throw new TaskEventStoreError(
          "invalid_write_plan",
          `authored file ${claim.path} has no content input`,
        );
      files.push({
        target: ledgerGitPath(ledger, claim.path),
        body: blob.body,
        mode: documentMode(event, claim.path),
      });
    }
    for (const retirement of canonicalDocumentRetirements(event))
      files.push({ delete: ledgerGitPath(ledger, retirement.path) });
    const changedPaths = files
      ...
      .map((target) =>
        workspacePath(layout.rootDir, layout.authoredRoot, ledger, target),
      )
      .sort();
    ...
    const preparedSha = prepareCommit(
      repoRoot,
      preparedRef,
      parent.sha,
      files,
    ...
    try {
      nodeSyncs = settleFiles(
        ...
        () => {
          const finalize = () => {
            ...
            assertAuthorizedReplacement(ledger, parent.sha, event);
            try {
              finalizeRefs(repoRoot, authoredRef, preparedSha, parent.sha);
            } catch (error) {
              try {
                deleteRef(repoRoot, preparedRef);
              } catch (cleanupError) {
                throw new TaskEventStoreError(
                  "publication_indeterminate",
              ...
              throw new TaskEventStoreError(
                "publication_indeterminate",
                `ledger ${authoredRef} must point at the last published event commit ${parent.sha}, but a commit was made outside the daemon. Recover with: git -C ${repoRoot} update-ref ${authoredRef} ${parent.sha} — this moves only the branch pointer and leaves every file in place. Then run ha daemon stop and retry. Cause: ${messageOf(error)}`,
              );
            }
            canonicalCommit = preparedSha;
            authoredCommit = preparedSha;
            canonicalHead = head;
            rememberEvents([event]);
            if (blobs.length)
              recentContent = new Map(
                blobs.map((blob) => [blob.sha256, Buffer.from(blob.body)]),
              );
            options.killpoint?.("after_git_commit");
          };
          (options.withAppendFence
            ? () => options.withAppendFence!(finalize)
            : finalize)();
        },
      );
    } catch (error) {
      ...
      if (
        !(error instanceof TaskEventStoreError) ||
    ...
    return receipt(
      event,
      ledgerCommitSha(repoId, preparedSha),
      started,
      changedPaths,
      nodeSyncs,
    );
  ...
  const migrateLayout: CanonicalEventStore["migrateLayout"] = (migration) => {
    assertPublicationCut(repoRoot, authoredRef, canonicalCommit);
    const started = gitObjects.processCount();
    let parent = currentCommit();
    const previousHead = readHead();
    let flat = flatLayoutAt(ledger, parent.sha);
    if (flat.events.length === 0 && flat.blobs.length === 0) {
      const event = previousHead ? readEvent(previousHead.opId) : null;
      if (event && isLedgerLayoutMigrationEvent(event)) {
        const before = commitParent(repoRoot, parent.sha);
        auditLayoutMigration(ledger, before, parent.sha, event);
        layoutState = "sharded-sha256-2/v1";
        return receipt(event, parent, started, []);
      }
      throw new TaskEventStoreError(
        "invalid_store",
        "ledger has no flat layout to migrate",
      );
    }
    if (flat.hasShardedEvents || flat.hasShardedBlobs) {
      normalizeMixedLayout(parent.sha, migration.occurredAt);
      parent = currentCommit();
      flat = flatLayoutAt(ledger, parent.sha);
      if (flat.hasShardedEvents || flat.hasShardedBlobs)
        throw new TaskEventStoreError(
          "invalid_store",
          "mixed ledger normalization left sharded entries behind",
        );
    }
    if (flat.events.length !== (previousHead?.revision ?? 0))
      throw new TaskEventStoreError(
        "invalid_store",
        "flat event count does not match the canonical head revision",
      );
    ...
    const renames: PublicationRename[] = [
      ...flat.events.map(({ name }) => ({
        from: ledgerGitPath(ledger, `events/${name}`),
        to: eventObjectPath(ledger, name.slice(0, -5)),
      })),
      ...flat.blobs.map(({ name }) => ({
        from: ledgerGitPath(ledger, `objects/sha256/${name}`),
        to: blobObjectPath(ledger, name),
      })),
    ];
    const appended = publish(
      { event, plan: ledgerLayoutMigrationWritePlan(event), blobs: [] },
      renames,
    );
    auditLayoutMigration(ledger, parent.sha, appended.commitSha!.sha, event);
    layoutState = "sharded-sha256-2/v1";
    return appended;
  };
  ...
  const normalizeMixedLayout = (parent: string, occurredAt: string): string => {
    const events = scanEventsRoot(ledger, parent),
      content = scanContentRoot(ledger, parent);
    if (events === null)
      throw new TaskEventStoreError(
        "invalid_store",
        "flat ledger has no events tree",
      );
    const shardedEvents = eventEntriesFromShards(ledger, events.shards),
      shardedBlobs = contentEntriesFromShards(ledger, content?.shards ?? []);
    const flatEvents = new Map(
        events.flat.map((entry) => [entry.name, entry.oid] as const),
      ),
      flatBlobs = new Map(
        (content?.flat ?? []).map((entry) => [entry.name, entry.oid] as const),
      );
    const renames: PublicationRename[] = [],
    ...
    for (const { name, oid } of shardedEvents) {
      ...
      if (twin === undefined)
        renames.push({ from, to: ledgerGitPath(ledger, `events/${flatName}`) });
      else if (twin !== oid)
        throw new TaskEventStoreError(
          "invalid_store",
    ...
    for (const { name, oid } of shardedBlobs) {
      ...
      if (twin === undefined)
        renames.push({
          from,
          to: ledgerGitPath(ledger, `objects/sha256/${flatName}`),
        });
      else if (twin !== oid)
        throw new TaskEventStoreError(
          "invalid_store",
    ...
    if (renames.length + deletions.length === 0)
      throw new TaskEventStoreError(
        "invalid_store",
        "mixed ledger has no sharded entries to normalize",
      );
    assertPublicationCut(repoRoot, authoredRef, parent);
    const ref = `refs/ha-layout-normalized/${sha256Text(parent)}`,
      sha = prepareCommit(
        repoRoot,
        ref,
        parent,
        [...renames, ...deletions],
        `harness layout normalize ${parent}`,
        occurredAt,
      );
    settleFiles(repoRoot, sha, [...renames, ...deletions]);
    finalizeRefs(repoRoot, authoredRef, sha, parent);
    deleteRef(repoRoot, ref);
    canonicalCommit = sha;
    authoredCommit = sha;
    return sha;
  };
  const recover = (): EventRecoveryReceipt => {
    ...
    const prepared = initialPrepared ?? preparedRefs(repoRoot);
    initialPrepared = null;
    if (options.rejectPreparedRecovery && prepared.length > 0) {
      for (const [ref] of prepared) deleteRef(repoRoot, ref);
      return {
        status: "none",
        publications: 0,
    ...
    try {
      const changed = changedPublication(ledger, sha);
      validatePrepared(ledger, sha, changed.files, changed.head);
      if (canonicalCommit === sha && authoredCommit === sha) {
        ...
        canonicalHead = changed.head;
        rememberEvents([changed.event]);
        settleFiles(repoRoot, sha, changed.files);
        deleteRef(repoRoot, ref);
        return {
      ...
      if (
        canonicalCommit === changed.parent &&
        authoredCommit === changed.parent
      ) {
        assertAuthorizedReplacement(ledger, changed.parent, changed.event);
        settleFiles(repoRoot, sha, changed.files);
        finalizeRefs(repoRoot, authoredRef, sha, changed.parent, [ref, sha]);
        canonicalCommit = sha;
        authoredCommit = sha;
        canonicalHead = changed.head;
        rememberEvents([changed.event]);
  ...
  return {
    ...
    currentCommit,
    currentCut: () => canonicalLedgerCut(repoId, readHead()),
    publication: (event) => ({
      commitSha: currentCommit(),
      cut: canonicalEventCut(repoId, event),
    }),
    readHead,
    readEvent,
    readTaskEvent,
    ...
    layout: () => {
      if (layoutState === undefined) {
        const scan = scanEventsRoot(ledger, canonicalCommit);
        layoutState =
    ...
    revisionAt: (commit) => {
      if (commit.repoId !== repoId)
        throw new TaskEventStoreError(
          "repo_mismatch",
          `ledger commit belongs to repo ${commit.repoId}, not ${repoId}`,
        );
      return canonicalRevisionAt(ledger, canonicalCommit, commit.sha);
    },
    read: () => {
      const stream = readStream(ledger, canonicalCommit, readHead());
      knownOpIds = new Set(stream.events.map((event) => event.opId));
      rememberEvents(stream.events);
      return stream;
    },
    readBatch: (cursor, maxItems) => {
      ...
      knownOpIds = new Set(
        cachedBatchEventEntries(ledger, canonicalCommit).map((entry) =>
          path.posix.basename(entry.name).slice(0, -5),
        ),
      );
      rememberEvents(batch.events);
      return batch;
    },
    materialize: () =>
      materialize(ledger, repoId, canonicalCommit, readHead(), authoredRef),
    append: (bundle) => publish(bundle),
    migrateLayout,
    recover,
    drain: async () => {},
  };
}
function assertBundle(bundle: CanonicalWriteBundle): void {
  const { event, plan, blobs } = bundle;
  assertPublishableOpId(event.opId);
  if (!isFrozenWritePlan(plan))
    throw new TaskEventStoreError(
      "invalid_write_plan",
      "canonical write bundle requires one frozen write plan",
    );
  if (isDocEvent(event)) {
    if (validateCurrentDocEvent(event).length)
      throw new TaskEventStoreError(
        "invalid_write_plan",
        "doc event write requires the current cut identity",
      );
  ...
  if (isAgentEntityEvent(event))
    try {
      assertAgentEntityWritePlan(
        event,
        plan as FrozenWritePlan<"AgentEntityWrite">,
      );
    } catch {
      throw new TaskEventStoreError(
        "invalid_write_plan",
  ...
  if (isSnapshotUpgradeEvent(event))
    try {
      ...
      throw new TaskEventStoreError(
        "invalid_write_plan",
        "snapshot upgrade inputs or plan are invalid",
      );
    }
  ...
  if (
    required.some(
      (target) =>
        !plan.targets.some(
          (candidate) => stableStringify(candidate) === stableStringify(target),
        ),
    )
  )
    throw new TaskEventStoreError(
      "invalid_write_plan",
      "canonical write bundle must declare its event and head",
    );
  assertContentInputs(claims, blobs, "canonical bundle");
  const declaredAuthored = plan.targets.filter(
      (target) => target.kind === "authored_file",
    ),
    expectedAuthored: WriteTarget[] = canonicalDocumentClaims(event).map(
      (claim) => ({
        kind: "authored_file",
        path: claim.path,
        operation: "replace",
...
function assertDocWritePlan(
  ...
  assertContentInputs(
    event.payload.changes.flatMap(({ candidate }) =>
...
function assertBootstrapInputs(
  ...
  try {
    assertTaskBootstrapWritePlan(
      event,
      plan as FrozenWritePlan<"TaskBootstrap">,
    );
  } catch {
    throw new TaskEventStoreError(
      "invalid_write_plan",
  ...
  if (!claim || !("digest" in claim))
    throw new TaskEventStoreError(
      "invalid_write_plan",
      "task bootstrap snapshot claim is missing",
    );
  const blob = blobs.find((candidate) => candidate.sha256 === claim.sha256);
  ...
  try {
    ...
    throw new TaskEventStoreError(
      "invalid_write_plan",
      "task bootstrap snapshot claim must be JSON",
    );
  }
  const { digest, ...snapshot } = value;
  if (
    digest !== claim.digest ||
    claim.digest !== `sha256:${sha256Text(stableStringify(snapshot))}`
  )
    throw new TaskEventStoreError(
      "invalid_write_plan",
...
function validatePrepared(
  ...
  const body = files.find(
    (file): file is PublicationWrite =>
      "target" in file &&
      (eventObjectPaths(ledger, head.opId) as readonly string[]).includes(
        file.target,
      ),
  )?.body;
  ...
  if (
    ...
    head.eventDigest !== `sha256:${sha256Text(body)}`
  )
    throw new Error("prepared event/head mismatch");
}
export function canonicalEventCut(
  repoId: string,
  event: CanonicalEventV1,
): CanonicalEventCut {
...
export function canonicalLedgerCut(
  repoId: string,
  head: EventHead | null,
): LedgerCutIdentity {
  return {
...
function readStream(
  ...
  if (head === null)
    return { schema: "canonical-event-stream/v1", revision: 0, events: [] };
  const events = readEventsAt(
    ledger,
    commit,
    eventEntries(ledger, commit),
  ...
  for (const [index, event] of events.entries()) {
    if (event.workspaceRevision !== index + 1)
      throw new TaskEventStoreError(
        "invalid_store",
        `event revision ${event.workspaceRevision} is not contiguous`,
      );
    if (opIds.has(event.opId))
      throw new TaskEventStoreError(
        "op_conflict",
        `duplicate event opId ${event.opId}`,
      );
  ...
  if (events.length !== head.revision || events.at(-1)?.opId !== head.opId)
    throw new TaskEventStoreError(
      "invalid_store",
      "event head does not match committed events",
    );
  return {
...
function readBatch(
  ...
  const entries = cachedBatchEventEntries(ledger, commit),
    ...
    events = readEventsAt(ledger, commit, selected).filter(
      (event) => event.workspaceRevision <= sourceRevision,
    );
  return {
    sourceRevision,
    events,
...
function readEventsAt(
  ...
  for (const entry of entries) {
    const headerEnd = output.indexOf(10, cursor),
      size = Number(
        output.subarray(cursor, headerEnd).toString("utf8").split(" ").at(-1),
      ),
      start = headerEnd + 1,
    ...
    if (event.opId !== path.posix.basename(entry.name).slice(0, -5))
      throw new TaskEventStoreError(
        "invalid_store",
        "event object names do not match canonical event bytes",
      );
    events.push(event);
    cursor = start + size + 1;
  }
...
function readEventAt(
  ledger: LedgerGitLayout,
  commit: string,
  opId: string,
): CanonicalEventV1 | null {
  safeOpId(opId);
  const bytes = readFirstPathAt(
    ledger.rootDir,
    commit,
    eventObjectPaths(ledger, opId),
  );
  ...
  if (event.opId !== opId)
    throw new TaskEventStoreError(
      "invalid_store",
      `event object does not match opId ${opId}`,
    );
  validateEventBlobs(ledger, commit, event);
  return event;
}
function validateEventBlobs(
  ledger: LedgerGitLayout,
  commit: string,
  event: CanonicalEventV1,
  ...
  for (const claim of contentClaims(event)) {
    const changed = files.find(
        (file) => file.target === blobObjectPath(ledger, claim.sha256),
      ),
      blob = changed
        ? Buffer.from(changed.body)
        : readBlobAt(ledger, commit, claim.sha256);
    assertBlobExact(blob, claim);
  }
}
function prefetchEventContent(
  ledger: LedgerGitLayout,
  commit: string,
  events: readonly CanonicalEventV1[],
): ReadonlyMap<string, Uint8Array | null> {
  const claims = [
      ...new Map(
        events
          .flatMap((event) => contentClaims(event))
          .map((claim) => [claim.sha256, claim] as const),
      ).values(),
    ],
    blobs = readBlobsAt(
      ledger,
  ...
  for (const claim of claims)
    assertBlobExact(blobs.get(claim.sha256) ?? null, claim);
  return blobs;
}
function validateStreamContent(
  ledger: LedgerGitLayout,
  commit: string,
  events: readonly CanonicalEventV1[],
): void {
  ...
  for (const claim of claims) {
    if (chunk.length >= 512 || bytes + claim.size > 64 * 1024 * 1024)
      validate();
    chunk.push(claim);
    bytes += claim.size;
  }
...
function assertBlobExact(
  ...
  if (
    ...
    throw new TaskEventStoreError(
      "invalid_store",
      `event content blob ${claim.sha256} is not reachable and exact`,
    );
}
function readHeadAt(ledger: LedgerGitLayout, commit: string): EventHead | null {
  const body = showText(
    ledger.rootDir,
    commit,
  ...
  if (serializeEventHead(head) !== body)
    throw new TaskEventStoreError(
      "invalid_store",
      "event head is not canonical",
    );
  return head;
}
function readBlobAt(
  ...
  if (!/^[0-9a-f]{64}$/u.test(sha256))
    throw new TaskEventStoreError(
      "invalid_store",
      "content blob hash is invalid",
    );
  return readFirstPathAt(
    ledger.rootDir,
    commit,
    blobObjectPaths(ledger, sha256),
  );
}
function readBlobsAt(
  ledger: LedgerGitLayout,
  commit: string,
...
function canonicalRevisionAt(
  ...
  if (
    ...
    !gitObjects.isAncestor(ledger.rootDir, commit, current)
  )
    return null;
...
interface GitTreeEntry extends EventEntry {
...
>();
function cachedBatchEventEntries(
  ledger: LedgerGitLayout,
  commit: string,
): readonly EventEntry[] {
...
function scanEventsRoot(
  ledger: LedgerGitLayout,
  commit: string,
): LedgerRootScan | null {
  const tree = treeObjectAt(
    ledger.rootDir,
    `${commit}:${ledgerGitPath(ledger, "events")}`,
  );
  ...
  for (const entry of tree.entries) {
    ...
    throw new TaskEventStoreError(
      "invalid_store",
      `events root contains non-sharded entry ${entry.name}`,
    );
  }
  return {
    treeSha: tree.oid,
...
function scanContentRoot(
  ...
  const tree = treeObjectAt(
    ledger.rootDir,
    `${commit}:${ledgerGitPath(ledger, "objects/sha256")}`,
  );
  if (tree === null) return null;
  const flat: GitTreeEntry[] = [],
    shards: GitTreeEntry[] = [];
...
function mixedLayoutError(scan: LedgerRootScan): TaskEventStoreError {
...
function contentEntriesFromShards(
  ledger: LedgerGitLayout,
...
function eventEntriesFromShards(
  ledger: LedgerGitLayout,
  shards: readonly GitTreeEntry[],
): readonly EventEntry[] {
  if (shards.length === 0) return [];
  ...
  for (const shard of shards) {
    ...
    for (const entry of parsed.entries) {
      ...
      if (
        entry.mode !== "100644" ||
        !opId ||
        eventObjectShard(opId) !== shard.name
      )
        throw new TaskEventStoreError(
          "invalid_store",
...
function treeObjectAt(
  ...
  if (!oid || type !== "tree")
    throw new TaskEventStoreError("invalid_store", `${spec} is not a tree`);
...
function checkedEventBytes(event: CanonicalEventV1): string {
  try {
...
function eventObjectPaths(
  ledger: LedgerGitLayout,
  opId: string,
): readonly [string, string] {
  return [
    eventObjectPath(ledger, opId),
    eventObjectPath(ledger, opId, "flat/v1"),
  ];
}
function blobObjectPaths(
  ledger: LedgerGitLayout,
  sha256: string,
): readonly [string, string] {
  return [
    blobObjectPath(ledger, sha256),
    blobObjectPath(ledger, sha256, "flat/v1"),
  ];
}
function readFirstPathAt(
  repoRoot: string,
  commit: string,
  ...
  const output = gitObjects.batch(
    repoRoot,
    `${targets.map((target) => `${commit}:${target}`).join("\n")}\n`,
  );
  let cursor = 0;
...
function safeOpId(opId: string): void {
  if (
    !opId ||
    opId === "head" ||
    /[\\/]/u.test(opId) ||
    ...
    throw new TaskEventStoreError(
      "invalid_store",
      "event opId is not a safe object name",
    );
}
...
function prepareCommit(
  ...
  try {
    ...
    throw new TaskEventStoreError(
      "publication_indeterminate",
      `Git object import failed: ${messageOf(error)}`,
    );
  }
  ...
  if (!/^[0-9a-f]{40}$/u.test(sha))
    throw new TaskEventStoreError(
      "publication_indeterminate",
      "Git object import returned no commit",
    );
  return sha;
}
...
function parseRefs(body: string): readonly (readonly [string, string])[] {
  return body.trim()
    ? body
        .trim()
        .split(/\r?\n/u)
...
export function canonicalDocumentClaims(
  event: CanonicalEventV1,
): readonly {
  ...
  return isDocEvent(event)
    ...
    : isAgentEntityEvent(event)
      ? [event.payload.declarationDocumentClaim]
      : isTaskEvent(event)
        ? [
            ...(event.payload.documentClaims ?? []),
            ...(event.payload.carriedDocumentClaims ?? []).map(
              ({ path: target, candidate }) => ({ path: target, ...candidate }),
            ),
          ]
...
export function canonicalDocumentRetirements(
  ...
  return isDocEvent(event)
    ? event.payload.changes.flatMap(
        ({ path: target, baseBlobSha256, candidate }) =>
          candidate === null && baseBlobSha256 !== null
            ? [{ path: target, baseBlobSha256 }]
            : [],
      )
    : [];
}
function contentClaims(
  event: CanonicalEventV1,
): readonly {
  readonly sha256: string;
  readonly size: number;
  ...
  const claims = isDocEvent(event)
    ? event.payload.changes.flatMap((change) =>
        change.candidate === null ? [] : [change.candidate],
      )
    : isAgentEntityEvent(event)
      ? [event.payload.declarationDocumentClaim]
      : isTaskEvent(event)
...
function targetShape(targets: readonly WriteTarget[]): string {
  return stableStringify(targets.map(stableStringify).sort());
}
function flatLayoutAt(
  ledger: LedgerGitLayout,
  commit: string,
): {
  ...
  const events = eventsTree.entries.filter(
    ({ mode, name }) =>
      mode === "100644" && name !== "head.json" && name.endsWith(".json"),
  );
  const hasShardedEvents = eventsTree.entries.some(
    ({ mode, name }) => mode === "40000" && /^[0-9a-f]{2}$/u.test(name),
  );
  for (const entry of eventsTree.entries)
    if (
      ...
      throw new TaskEventStoreError(
        "invalid_store",
        `events root contains invalid migration source ${entry.name}`,
      );
  const blobsTree = treeObjectAt(
    ledger.rootDir,
    `${commit}:${ledgerGitPath(ledger, "objects/sha256")}`,
  );
  const blobs =
    blobsTree?.entries.filter(
      ({ mode, name }) => mode === "100644" && /^[0-9a-f]{64}$/u.test(name),
    ) ?? [];
  const hasShardedBlobs =
    blobsTree?.entries.some(
      ({ mode, name }) => mode === "40000" && /^[0-9a-f]{2}$/u.test(name),
    ) ?? false;
  for (const entry of blobsTree?.entries ?? [])
    if (
      !blobs.includes(entry) &&
      !(entry.mode === "40000" && /^[0-9a-f]{2}$/u.test(entry.name))
    )
      throw new TaskEventStoreError(
        "invalid_store",
...
function auditLayoutMigration(
  ...
  if (
    ...
    throw new TaskEventStoreError(
      "invalid_store",
      "migration event does not describe its parent layout",
    );
  const beforeCommitCount = Number(
    localGitText(ledger.rootDir, "rev-list", "--count", parent).trim(),
  );
  const afterCommitCount = Number(
    localGitText(ledger.rootDir, "rev-list", "--count", commit).trim(),
  );
  if (afterCommitCount !== beforeCommitCount + 1)
    throw new TaskEventStoreError(
      "invalid_store",
      "migration must add exactly one canonical commit",
    );
  if (!gitObjects.isAncestor(ledger.rootDir, parent, commit))
    throw new TaskEventStoreError(
      "invalid_store",
      "pre-migration head is not an ancestor of the migrated head",
    );
  ...
  if ([...beforeObjects].some((oid) => !afterObjects.has(oid)))
    throw new TaskEventStoreError(
      "invalid_store",
      "migration lost an object reachable from its parent",
    );
  const eventEntries = eventEntriesFromShards(
    ledger,
    scanEventsRoot(ledger, commit)?.shards ?? [],
  );
  if (eventEntries.length !== event.payload.eventCount + 1)
    throw new TaskEventStoreError(
      "invalid_store",
      "migrated event count does not match the migration event",
    );
  const eventsRoot = treeObjectAt(
    ledger.rootDir,
    `${commit}:${ledgerGitPath(ledger, "events")}`,
  );
  if (
    !eventsRoot ||
    eventsRoot.entries.some(
      ({ name }) => name.endsWith(".json") && name !== "head.json",
    )
  )
    throw new TaskEventStoreError(
      "invalid_store",
      "migrated events root still contains direct event JSON",
    );
  if (countShardedBlobs(ledger, commit) !== event.payload.blobCount)
    throw new TaskEventStoreError(
      "invalid_store",
      "migrated blob count does not match the migration event",
    );
}
function countShardedBlobs(ledger: LedgerGitLayout, commit: string): number {
  const root = treeObjectAt(
    ledger.rootDir,
    `${commit}:${ledgerGitPath(ledger, "objects/sha256")}`,
  );
  if (!root) return 0;
  const shards = root.entries.filter(
    ({ mode, name }) => mode === "40000" && /^[0-9a-f]{2}$/u.test(name),
  );
  if (shards.length !== root.entries.length)
    throw new TaskEventStoreError(
      "invalid_store",
      "migrated content object root contains a flat entry",
    );
...
function reachableObjects(
  ...
  return new Set(
    localGitText(repoRoot, "rev-list", "--objects", "--no-object-names", commit)
      .trim()
      .split(/\r?\n/u)
      .filter(Boolean),
  );
}
function commitParent(repoRoot: string, commit: string): string {
  const parent = localGitText(repoRoot, "rev-parse", `${commit}^`).trim();
  if (!/^[0-9a-f]{40}$/u.test(parent))
    throw new TaskEventStoreError(
      "invalid_store",
      "prepared commit has no parent",
    );
  return parent;
}
function stripLedgerPrefix(ledger: LedgerGitLayout, target: string): string {
  const prefix = ledger.authoredPrefix ? `${ledger.authoredPrefix}/` : "";
  if (prefix && !target.startsWith(prefix))
    throw new TaskEventStoreError(
      "invalid_store",
      `prepared path is outside the ledger: ${target}`,
    );
  return prefix ? target.slice(prefix.length) : target;
}
function changedPublication(
  ledger: LedgerGitLayout,
  commit: string,
): {
  ...
  if (!eventBody)
    throw new TaskEventStoreError(
      "invalid_store",
      "prepared commit has no changed event",
    );
  ...
  if (isLedgerLayoutMigrationEvent(event)) {
    ...
    for (const { status, target } of changes) {
      ...
      if (!to || !added.delete(to))
        throw new TaskEventStoreError(
          "invalid_store",
          `prepared migration has unmatched deletion ${target}`,
        );
    ...
    if (
      changes.some(
        ({ status, target }) => status === "A" && target === headPath,
      )
    )
      added.delete(headPath);
    if (
      added.size > 0 ||
      renames.length !== event.payload.eventCount + event.payload.blobCount ||
      changes.some(
        ({ status, target }) =>
          !["A", "D", "M"].includes(status) ||
          (status === "M" && target !== headPath),
      )
    )
      throw new TaskEventStoreError(
        "invalid_store",
        "prepared migration change set does not match its event payload",
      );
    return {
      parent,
      head,
  ...
  const expectedDeletes = new Set(
      canonicalDocumentRetirements(event).map(({ path: target }) =>
        ledgerGitPath(ledger, target),
      ),
    ),
    actualDeletes = changes
  ...
  if (
    ...
    throw new TaskEventStoreError(
      "invalid_store",
      "ordinary prepared publication contains an undeclared deletion",
    );
  ...
  return {
    ...
    event,
    files: [
      ...publicationModes(ledger, writes, event),
      ...actualDeletes.map((target) => ({ delete: target })),
    ],
  };
}
...
function assertAuthorizedReplacement(
  ledger: LedgerGitLayout,
  parent: string,
  event: CanonicalEventV1,
  ...
  if (isDocEvent(event)) {
    for (const retirement of canonicalDocumentRetirements(event)) {
      const committed = committedNode(
        ledger.rootDir,
        parent,
        ledgerGitPath(ledger, retirement.path),
      );
      if (
        committed?.nodeKind !== "file" ||
        committed.sha256 !== retirement.baseBlobSha256
      )
        throw new TaskEventStoreError(
          "revision_conflict",
  ...
  const entity = event.payload.entity,
    ...
    same = (node: typeof committed, shape: typeof expected) =>
      node !== null &&
      node.nodeKind === shape.nodeKind &&
      node.sha256 === shape.sha256 &&
  ...
  if (
    ...
    !localNode ||
    (!same(localNode, expected) &&
      !(acceptPublished && same(localNode, published)))
  )
    throw new TaskEventStoreError(
      "revision_conflict",
...
function settleFiles(
  ...
  for (const file of files) {
    const logical =
      "target" in file ? file.target : "delete" in file ? file.delete : null;
    if (
      logical === null ||
      ("target" in file && !/\.(?:md|txt)$/u.test(logical))
    )
      continue;
    ...
    if (
      ...
      ("target" in file &&
        ...
        sha256Text(local.body) === sha256Text(file.body))
    )
      continue;
...
function materialize(
  ...
  const latest = new Map<
    string,
    { readonly hash: string; readonly mode: GitFileMode }
  >();
  ...
  for (const [logical, { hash, mode }] of [...latest].sort(([left], [right]) =>
    ...
    if (!bytes)
      throw new TaskEventStoreError(
        "invalid_store",
        `materialization blob ${hash} is missing`,
      );
    const body = Buffer.from(bytes).toString("utf8"),
    ...
    if (local !== null)
      conflicts.push(
        worktree.preserveConflict(ledger.rootDir, target, logical, commit),
      );
...
export function canonicalDocumentMode(
  event: CanonicalEventV1,
  documentPath: string,
): GitFileMode {
...
const documentMode = canonicalDocumentMode;
...
function workspacePath(
  ...
  const authored = path
      .relative(workspaceRoot, authoredRoot)
      .split(path.sep)
      .join("/"),
...
function shapeError(body: string, detail: string): TaskEventStoreError {
  let schema = "unknown";
  try {
