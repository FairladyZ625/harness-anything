...
interface LocalVersionControlCommandAdapter {
  ...
  readonly execute: (repoRoot: string, ...args: ReadonlyArray<string>) => string;
  readonly executeAs: (repoRoot: string, author: VcsCommitAuthor | undefined, ...args: ReadonlyArray<string>) => string;
}

export function makeLocalVersionControlCommands(adapter: LocalVersionControlCommandAdapter): VersionControlSystem {
  ...
  return {
    ...
    isIgnored: (repoRoot, relativePath) => {
      try {
        execute(repoRoot, "check-ignore", "--no-index", "-q", "--", relativePath);
        return true;
      } catch {
        return false;
      }
    ...
    add: (repoRoot, input) => {
      ...
      execute(repoRoot, "add", "-A", ...(input.force ? ["-f"] : []), "--", ...input.paths);
    },
    workingTreeFiles: (repoRoot, paths) => execute(repoRoot, "status", "--porcelain", "-uall", "--", ...paths),
    stagedFiles: (repoRoot, paths) => execute(repoRoot, "diff", "--cached", "--name-only", "--", ...paths),
    ...
    currentBranch: (repoRoot) => {
      try {
        const name = execute(repoRoot, "rev-parse", "--abbrev-ref", "HEAD").trim();
    ...
    originHeadBranch: (repoRoot) => {
      try {
        const ref = execute(repoRoot, "symbolic-ref", "--quiet", "--short", "refs/remotes/origin/HEAD").trim();
    ...
    sessionBranches: (repoRoot) =>
      execute(repoRoot, "for-each-ref", "--sort=creatordate", "--format=%(refname:short)", "refs/heads/sessions")
        .split(/\r?\n/u)
