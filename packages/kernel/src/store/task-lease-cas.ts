...
import { validateWriteSource, type WriteSource } from "../domain/write-chain.contract.ts";
import { localRuntimeStateFileSystem } from "../local/local-layout-file-system.ts";
import type { LeaseCasPayload, WriteOp } from "../ports/write-coordinator.ts";
import { defaultLifecycleTaskProjectionPath } from "../projection/task-projection.ts";
interface LeaseRow {
  readonly task_id: string; readonly execution_id: string; readonly actor_json: string; readonly source_json: string;
  readonly phase: LeaseV1["phase"]; readonly expires_at: string; readonly ttl_ms: number; readonly version: number;
}
...
export function validateLeaseCasWrite(op: WriteOp): LeaseCasPayload {
  ...
  if (!(["reserve", "activate", "renew", "release"] as const).includes(p.operation) || !text(p.taskId) || !text(p.executionId)
    || validateActorAxes(p.actor).length > 0 || validateWriteSource(p.source).length > 0 || !text(p.now) || p.capacity !== TASK_LEASE_BROKER_CONTRACT.capacity) {
    throw reject("invalid_lease_cas", "lease_cas identity, operation, timestamp, or capacity is invalid");
  }
  if (p.operation === "reserve" && (!text(p.expiresAt) || !Number.isInteger(p.ttlMs) || p.ttlMs! < 1)) throw reject("invalid_lease_cas", "reserve requires expiry and ttlMs");
...
export function applyLeaseCasWrite(root: HarnessLayoutInput, op: WriteOp): void {
  ...
  try {
    db.exec("PRAGMA journal_mode = DELETE; CREATE TABLE IF NOT EXISTS lease_cas (task_id TEXT PRIMARY KEY, execution_id TEXT NOT NULL, actor_json TEXT NOT NULL, source_json TEXT NOT NULL, phase TEXT NOT NULL CHECK (phase IN ('reserving', 'active', 'orphaned', 'released')), expires_at TEXT NOT NULL, ttl_ms INTEGER NOT NULL, version INTEGER NOT NULL)");
...
function mutate(db: DatabaseSync, p: LeaseCasPayload): void {
  ...
  const sourceJson = JSON.stringify(canonicalizeContractValue(p.source));
  if (p.operation === "reserve") {
    const effective = current !== null && current.phase !== "released" && current.expires_at > p.now;
    if (effective && current.execution_id === p.executionId && current.actor_json === actorJson && current.source_json === sourceJson) return;
    ...
    const lease = checked({ schema: "lease/v1", taskId: p.taskId, executionId: p.executionId, actor: p.actor, source: p.source,
      phase: "reserving", expiresAt: p.expiresAt!, ttlMs: p.ttlMs!, version: current === null ? 0 : current.version + 1 });
    db.prepare("INSERT INTO lease_cas (task_id, execution_id, actor_json, source_json, phase, expires_at, ttl_ms, version) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(task_id) DO UPDATE SET execution_id=excluded.execution_id, actor_json=excluded.actor_json, source_json=excluded.source_json, phase=excluded.phase, expires_at=excluded.expires_at, ttl_ms=excluded.ttl_ms, version=excluded.version")
      .run(lease.taskId, lease.executionId, actorJson, sourceJson, lease.phase, lease.expiresAt, lease.ttlMs, lease.version);
    return;
  }
  if (current === null || current.execution_id !== p.executionId || current.actor_json !== actorJson || current.source_json !== sourceJson) throw stale(p);
  const phase = p.operation === "release" ? "released" : "active", expiry = p.expiresAt ?? current.expires_at, ttlMs = p.ttlMs ?? current.ttl_ms;
  if (current.version === p.version! + 1 && current.phase === phase && current.expires_at === expiry && current.ttl_ms === ttlMs) return;
  ...
  const result = db.prepare("UPDATE lease_cas SET phase = ?, expires_at = ?, ttl_ms = ?, version = ? WHERE task_id = ? AND execution_id = ? AND actor_json = ? AND source_json = ? AND version = ? AND phase = ?")
    .run(phase, expiry, ttlMs, current.version + 1, p.taskId, p.executionId, actorJson, sourceJson, p.version!, current.phase);
...
function fromRow(value: LeaseRow | null): LeaseV1 | null {
  return value === null ? null : checked({ schema: "lease/v1", taskId: value.task_id, executionId: value.execution_id,
    actor: JSON.parse(value.actor_json) as ActorAxes, source: JSON.parse(value.source_json) as WriteSource,
    phase: value.phase, expiresAt: value.expires_at, ttlMs: value.ttl_ms, version: value.version });
}
