...
function renderExecution(value: ExecutionV1, snapshot: TaskLifecycleSnapshot): string {
  const packet = value.submission,
    ...
    results = gateResults(snapshot, undefined, value.executionId, packet, value.iteration),
    checkerGateIds = results
      .filter(({ gateId, status }) => gateId !== "code-doc-reconciliation" && status !== "not_applicable")
      .map(({ gateId }) => gateId),
    ...
    codeDocRequired = results.some(
      ({ gateId, status }) => gateId === "code-doc-reconciliation" && status !== "not_applicable",
    ),
