...
export function lifecycleDocumentPaths(
  event: TaskEventV1,
  packagePath: string,
): readonly string[] {
  if (event.type === "task_created" || event.type === "lease_renewed")
    return [];
  ...
  if (!("execution" in event.payload)) {
    ...
    return plan !== null &&
      event.payload.documentClaims?.some((claim) => claim.path === plan)
      ? [...base, plan]
      : base;
  }
  const paths = [
    `${packagePath}/INDEX.md`,
    `${packagePath}/executions/${event.payload.execution.executionId}.md`,
  ];
  if (
...
export function lifecycleDocumentFetchPaths(
  event: TaskEventV1,
  packagePath: string,
): readonly string[] {
  if (event.type === "task_created" || event.type === "lease_renewed")
  ...
  const base = mutationDocumentPaths(event, packagePath);
  if ("execution" in event.payload)
    return lifecycleDocumentPaths(event, packagePath);
  const plan = retitledTaskPlanPath(event, packagePath);
  return plan !== null && !base.includes(plan) ? [...base, plan] : base;
}
function mutationDocumentPaths(
  event: TaskEventV1,
  packagePath: string,
): readonly string[] {
  return [
    ...
    ...(event.payload.task.metadata?.moduleKey
      ? [`${packagePath}/module.md`]
      : []),
  ];
}
export function retitledTaskPlanPath(
  event: TaskEventV1,
  packagePath: string,
): string | null {
  return event.type === "task_amended" &&
    event.payload.mutation.fields.includes("title")
...
export function compileTaskLifecycleWrite(input: {
  ...
  const sourceEvent = {
    ...
    planPath = input.packagePath
      ? retitledTaskPlanPath(sourceEvent, input.packagePath)
      : null,
    invariantPaths = input.packagePath
      ? lifecycleDocumentPaths(sourceEvent, input.packagePath)
      : [],
    planBase =
      planPath === null
        ? null
        : (input.currentDocuments.find(
            (document) => document.path === planPath,
          ) ?? null),
    paths =
      planPath !== null &&
      planBase !== null &&
      !invariantPaths.includes(planPath)
        ? [...invariantPaths, planPath]
        : invariantPaths,
    current = new Map(
      input.currentDocuments.map((value) => [value.path, value.body]),
    ),
    bodies = paths.map((path) => ({
      path,
      body: renderLifecycleDocument(
        sourceEvent,
        snapshot,
        path,
        current.get(path) ?? null,
      ),
    })),
...
export function taskLifecycleWritePlan(event: TaskEventV1): FrozenWritePlan {
  ...
  if (event.type === "execution_started")
    targets.push(
      lease(event.taskId, "reserve"),
      lease(event.taskId, "activate"),
      lease(event.taskId, "release"),
    );
...
export function assertTaskLifecycleWritePlan(
  event: TaskEventV1,
  ...
  if (
    ...
    shape(plan) !== shape(taskLifecycleWritePlan(event))
  )
    throw new Error(
...
export function renderLifecycleDocument(
  ...
  if (path.endsWith("/INDEX.md"))
    return renderIndex(event, snapshot, path, base);
  if (path.endsWith("/task-contract.json"))
    return renderContract(snapshot, base);
  if (path.endsWith("/module.md")) return renderModule(snapshot);
  if (
    path.endsWith("/task_plan.md") &&
    retitledTaskPlanPath(event, path.slice(0, -"/task_plan.md".length)) !== null
  )
    return renderTaskPlan(snapshot, path, base);
  if (
    path.endsWith("/code-doc-anchors.json") &&
  ...
  if (path.includes("/reviews/")) {
    const review =
      ...
      event.type === "review_consent_recorded"
        ...
        : snapshot.reviews.find((value) =>
            path.endsWith(`/${value.reviewId}.md`),
          );
    return renderReview(
      review as ReviewV1,
      snapshot.consents.find((value) => value.reviewId === review?.reviewId) ??
        null,
    );
  }
  const execution =
    "execution" in event.payload
      ...
      : snapshot.executions.find((value) =>
          path.endsWith(`/${value.executionId}.md`),
        );
...
function renderTaskPlan(
  ...
  if (base === null)
    throw new Error(`cannot retitle an unpublished task plan: ${path}`);
...
function renderIndex(
  ...
  const task = snapshot.task!,
    current =
      "execution" in event.payload
        ...
        : snapshot.executions.find(
            (value) =>
              value.iteration === task.iteration && value.state === "submitted",
          ),
    ...
    selected = current?.submission
      ...
      : undefined,
    consentReviewId =
      approved.length === 1 ? approved[0]!.reviewId : "<review-id>",
    gateStatus = (gateId: string) =>
      gateId === "code-doc-reconciliation"
        ? snapshot.codeDocWitnesses.some(
            (value) => value.executionId === executionId,
          )
        : snapshot.gateWitnesses.some(
            (value) =>
              value.executionId === executionId && value.gateId === gateId,
          ),
    ...
    next =
      task.status === "active"
        ...
        : task.status === "in_review" && !approved.length
          ? `Run \`ha task review-execution ${task.taskId} --execution-id <id> --review-id <id> --from-file <review.json>\`.`
          : task.status === "in_review" && !selected
            ? `Run \`ha task review-consent ${task.taskId} --execution-id <id> --review-id ${consentReviewId} --consent-id <id>\`.`
            : missingGate === "ci"
              ? `Run \`ha task complete ${task.taskId} --execution-id <id> --ci passed\`.`
              : missingGate === "code-doc-reconciliation"
                ? `Run \`ha task code-doc reconcile ${task.taskId} --execution-id <id> --commit-sha <sha> --iteration <n> --path <path>\`.`
                : task.status === "done"
                  ? "Task complete."
    ...
    gates = task.completionGateIds.length
      ? task.completionGateIds
          .map(
            (gateId) =>
              `- ${gateId}: ${gateStatus(gateId) ? "pass" : "blocked"}`,
          )
          .join("\n")
      : "- none",
...
function renderContract(
  ...
  return `${JSON.stringify({ ...current, schema: "task-contract/v1", contractVersion: task.contractVersion ?? 1, taskId: task.taskId, title: task.title, taskClass: task.taskClass, pinned: task.pinned ?? false, metadata, relations }, null, 2)}\n`;
}
...
function renderExecution(
  ...
  const packet = value.submission,
    reviews = snapshot.reviews.filter(
      (candidate) => candidate.executionId === value.executionId,
    ),
    selected = packet
      ? consentedApprovedReview(
          snapshot.reviews,
...
function lease(
  taskId: string,
  operation: "reserve" | "activate" | "release",
): WriteTarget {
