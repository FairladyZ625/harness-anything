...
export function validateArchivedExecutionV0(value: unknown): readonly ContractValidationIssue[] { const fields = ["schema", "generation", "migratedFrom", "executionId", "taskId", "nodeId", "iteration", "state", "actor", "claimedAt", "submittedAt", "closedAt", "sessionBindings", "outputs", "submission", "archivedSubmission"]; if (!isRecord(value) || !hasOnlyFields(value, fields) || value.schema !== "archived-execution/v1" || value.generation !== "v0" || !isNonEmptyString(value.migratedFrom) || !isNonEmptyString(value.executionId) || !isNonEmptyString(value.taskId) || value.nodeId !== "implementation" || value.iteration !== 0 && value.iteration !== 1 || !(executionStates as readonly unknown[]).includes(value.state) || validateActorAxes(value.actor).length || !timestamp(value.claimedAt) || value.submittedAt !== null && !timestamp(value.submittedAt) || value.closedAt !== null && !timestamp(value.closedAt) || !Array.isArray(value.sessionBindings) || !value.sessionBindings.every(isRecord) || !Array.isArray(value.outputs) || !value.outputs.every(archivedOutput) || value.submission !== null || value.archivedSubmission !== null && !archivedSubmission(value.archivedSubmission)) return [{ code: "invalid_execution", message: "archived execution fields or migration provenance are invalid" }]; return []; }
...
export function validateLeaseHolder(value: unknown): readonly ContractValidationIssue[] {
  if (!isRecord(value) || !hasOnlyFields(value, LEASE_HOLDER_FIELDS) || !isNonEmptyString(value.taskId) || !isNonEmptyString(value.executionId)
    || validateActorAxes(value.actor).length > 0 || validateWriteSource(value.source).length > 0) return [{ code: "invalid_lease_holder", message: "lease holder requires task, execution, actor, and source" }];
  return [];
}
