...
import { reviewDigest } from "./task-lifecycle.contract.ts";
...
export function completionBlockers(snapshot: TaskLifecycleSnapshot, executionId: string, context: CompletionReadinessContext): readonly CompletionBlocker[] {
  ...
  const review = snapshot.reviews.find((value) => value.executionId === executionId && value.verdict === "approved" && value.commitSha === execution.submission?.commitSha && value.iteration === execution.iteration);
  if (!review) return one("review_missing", "review", `ha task review-execution ${task.taskId} --execution-id ${executionId} --review-id <id> --from-file <review.json>`, "Record one independent approved Execution Review.");
  const consent = snapshot.consents.find((value) => value.executionId === executionId && value.reviewId === review.reviewId && value.reviewDigest === reviewDigest(review) && value.contentDigest === review.contentDigest);
  if (!consent) return one("consent_missing", "consent", `ha task review-consent ${task.taskId} --execution-id ${executionId} --review-id ${review.reviewId} --consent-id <id> --from-file <consent.json>`, "Record content-pinned owner consent for the approved Review.");
  for (const gateId of task.completionGateIds) {
    if (gateId === "code-doc-reconciliation" && !snapshot.codeDocWitnesses.some((value) => value.executionId === executionId && value.commitSha === execution.submission?.commitSha && value.iteration === execution.iteration)) return one("code_doc_missing", gateId, `ha task code-doc reconcile ${task.taskId} --execution-id ${executionId} --commit-sha ${execution.submission.commitSha} --iteration ${execution.iteration} --path <path>`, "Publish a typed code-doc witness for this execution cut.");
    if (gateId !== "code-doc-reconciliation" && !snapshot.gateWitnesses.some((value) => value.gateId === gateId && value.executionId === executionId && value.commitSha === execution.submission?.commitSha && value.iteration === execution.iteration)) return one(gateId === "ci" ? "ci_missing" : "gate_witness_missing", gateId, gateId === "ci" ? `ha task complete ${task.taskId} --execution-id ${executionId} --ci passed` : `ha task complete ${task.taskId} --execution-id ${executionId}`, `Publish a canonical ${gateId} checker witness for this execution cut.`);
  }
