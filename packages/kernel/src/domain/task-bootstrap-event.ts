...
function validSnapshotClaim(value: unknown): value is PresetSnapshotClaim { return isRecord(value) && hasOnlyFields(value, ["digest", "sha256", "size", "mediaType"]) && /^sha256:[0-9a-f]{64}$/u.test(String(value.digest)) && validBootstrapStoredClaim(value) && value.mediaType === "application/json"; }
function validDocumentClaim(value: unknown): value is InitialDocumentClaim { if (!isRecord(value) || !hasOnlyFields(value, ["path", "sha256", "size", "mediaType", "owner", "policyId"]) || !validBootstrapStoredClaim(value) || value.owner !== "machine" && value.owner !== "doc-sync" || value.owner === "machine" && value.policyId !== "typed-machine-writer/v1" || value.owner === "doc-sync" && (value.policyId !== "markdown-body-replaceable/v1" || value.mediaType !== "text/markdown" && value.mediaType !== "text/plain") || !["application/json", "text/markdown", "text/plain"].includes(String(value.mediaType))) return false; try { return normalizeRelativeDocumentPath(String(value.path)) === value.path; } catch { return false; } }
