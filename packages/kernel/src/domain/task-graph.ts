...
export function validateTaskGraph(value: unknown): readonly GraphValidationIssue[] {
  if (!record(value) || value.template !== "replay/v1" || value.maxIterations !== 1 || !Array.isArray(value.nodes) || !Array.isArray(value.edges)) return [issue("invalid_graph_shape", "graph must declare canonical replay/v1 nodes and edges")];
  if (JSON.stringify(value.nodes) !== JSON.stringify(REPLAY_TASK_GRAPH.nodes)) return [issue("invalid_node_set", "replay/v1 requires implementation and review in order")];
  if (value.edges.some((edge) => !record(edge) || !["forward", "return"].includes(String(edge.kind)))) return [issue("invalid_graph_shape", "replay/v1 edges require a declared kind")];
  const returns = value.edges.filter((edge) => record(edge) && edge.kind === "return"), forwards = value.edges.filter((edge) => record(edge) && edge.kind === "forward");
  if (returns.length !== 1 || !sameEdge(returns[0], REPLAY_TASK_GRAPH.edges[1])) return [issue("invalid_return_edge", "review changes_requested is the only return edge")];
  const outgoing = new Map<unknown, number>(); for (const edge of forwards) if (record(edge)) outgoing.set(edge.from, (outgoing.get(edge.from) ?? 0) + 1);
  if ([...outgoing.values()].some((count) => count > 1)) return [issue("forward_fan_out", "replay/v1 forward nodes cannot fan out")];
  if (forwards.some((edge) => record(edge) && edge.from === edge.to)) return [issue("forward_cycle", "replay/v1 forward edges must form a DAG")];
  if (forwards.length !== 1 || !sameEdge(forwards[0], REPLAY_TASK_GRAPH.edges[0])) return [issue("invalid_forward_path", "submit must be the sole implementation to review edge")];
  return [];
}
function record(value: unknown): value is Record<string, unknown> { return typeof value === "object" && value !== null; }
function sameEdge(value: unknown, expected: GraphEdgeDefinition): boolean { return record(value) && ["id", "from", "to", "on", "actorRole", "kind"].every((key) => value[key] === expected[key as keyof GraphEdgeDefinition]); }
function issue(code: GraphValidationIssue["code"], message: string): GraphValidationIssue { return { code, message }; }
