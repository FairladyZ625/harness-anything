...
import { isTaskBootstrapEvent, validateTaskBootstrapEvent, type TaskBootstrapEventV1 } from "./task-bootstrap-event.ts";
...
declare const ledgerCommitShaBrand: unique symbol, docClaimRefBrand: unique symbol, docByteLengthBrand: unique symbol; export interface LedgerCommitSha { readonly repoId: string; readonly sha: string; readonly [ledgerCommitShaBrand]: true } export type DocClaimRef = string & { readonly [docClaimRefBrand]: true }; export type DocByteLength = number & { readonly [docByteLengthBrand]: true };
...
export type CanonicalEventV1 = TaskEventV1 | DocEventV1 | AgentRuntimeEventV1 | TaskBootstrapEventV1;
...
export const canonicalEventSchemas = Object.freeze([{ schema: "task-event/v1", validate: (value: unknown) => validateTaskEvent(value).map((issue) => issue.message) },
  { schema: "doc-event/v1", validate: validateDocEvent }, { schema: "agent-runtime-event/v1", validate: validateAgentRuntimeEvent }, { schema: "task-bootstrap-event/v1", validate: validateTaskBootstrapEvent }] as const);
...
export function decideDocWrite(input: DocWriteDecisionInput): DocWriteDecision {
  ...
  if (input.intent.baseLedgerSha.repoId !== input.currentLedgerSha.repoId || input.intent.baseLedgerSha.sha !== input.currentLedgerSha.sha) return reject("base_ledger_changed", "refresh status/base and resubmit with a new opId");
