...
import { freezeDeclaredWritePlan, hasOnlyFields, isFrozenWritePlan, isNonEmptyString, isRecord, serializeEventEnvelope,
...
declare const ledgerCommitShaBrand: unique symbol, docClaimRefBrand: unique symbol, docByteLengthBrand: unique symbol; export type LedgerCommitSha = string & { readonly [ledgerCommitShaBrand]: true }; export type DocClaimRef = string & { readonly [docClaimRefBrand]: true }; export type DocByteLength = number & { readonly [docByteLengthBrand]: true };
export interface ContentClaim { readonly ref: DocClaimRef; readonly sha256: string; readonly size: DocByteLength; readonly mediaType: "text/markdown" | "text/plain" }
...
export interface DocWriteIntent { readonly schema: "doc-write-intent/v1"; readonly executionId: string; readonly baseLedgerSha: LedgerCommitSha; readonly changes: readonly DocWriteChange[] }
...
export function parseDocWriteIntent(value: unknown): DocWriteIntent { const errors = validateDocWriteIntent(value); if (errors.length) throw new DocSyncContractError(errors.join("; ")); const raw = value as { readonly schema: "doc-write-intent/v1"; readonly executionId: string; readonly baseLedgerSha: string; readonly changes: readonly { readonly path: string; readonly baseBlobSha256: string | null; readonly policyId: string; readonly candidate: { readonly ref: string; readonly sha256: string; readonly size: number; readonly mediaType: "text/markdown" | "text/plain" } | null }[] }; return { ...raw, baseLedgerSha: ledgerCommitSha(raw.baseLedgerSha), changes: raw.changes.map((change) => ({ ...change, path: documentPath(change.path), candidate: change.candidate === null ? null : { ...change.candidate, ref: docClaimRef(change.candidate.ref), size: docByteLength(change.candidate.size) } })) }; }
...
export function resolveDocRoute(path: PortableDocumentPath): { readonly allowed: boolean; readonly requiredRoute: string } { const denied = docRouteRegistry.find((route) => path === route.prefix || path.startsWith(route.prefix)); return denied ? { allowed: false, requiredRoute: denied.requiredRoute } : { allowed: true, requiredRoute: "doc-sync" }; }
...
function validClaim(value: unknown): boolean { if (value === null) return true; if (!isRecord(value) || !hasOnlyFields(value, ["ref", "sha256", "size", "mediaType"]) || !validStoredClaim(value)) return false; try { docClaimRef(String(value.ref)); return true; } catch { return false; } }
function validStoredClaim(value: unknown): boolean { return isRecord(value) && /^[0-9a-f]{64}$/u.test(String(value.sha256)) && Number.isSafeInteger(value.size) && (value.size as number) >= 0 && (value.mediaType === "text/markdown" || value.mediaType === "text/plain"); }
...
function safePath(value: unknown): value is string { try { return typeof value === "string" && normalizeRelativeDocumentPath(value) === value; } catch { return false; } }
...
function additiveProof(path: PortableDocumentPath, base: string, candidate: string, mediaType: string): { readonly proofs: readonly RegionProof[]; readonly differences: readonly DocSyncDifference[]; readonly unresolved: readonly DocSyncUnresolvedTouch[] } {
