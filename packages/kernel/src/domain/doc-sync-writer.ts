...
export function decideDocWrite(input: DocWriteDecisionInput): DocWriteDecision {
  ...
  for (const [index, change] of input.intent.changes.entries()) {
    ...
    if (current === null && decisionDocumentPath(change.path)) {
      unresolvedTouches.push(
        touch(change.path, null, "Decision documents must be proposed through the typed route", "ha decision --help"),
      );
      continue;
    }
    ...
    if (change.policyId !== OPAQUE_TEXTUAL_POLICY_ID && body.includes("\r")) {
      unresolvedTouches.push(touch(change.path, null, "claim is not canonical LF text", "canonical-utf8-prose"));
      continue;
    }
    const semantic =
      change.policyId === OPAQUE_TEXTUAL_POLICY_ID
        ? opaqueProof()
        : additiveProof(change.path, current?.body ?? "", body, change.candidate.mediaType, current === null);
...
export function resolveDocRoute(path: PortableDocumentPath): {
  ...
  const denied = docRouteRegistry.find((route) => path === route.prefix || path.startsWith(route.prefix));
  return denied
