 ...
 * mandatory human signoff. `admitPreservedVerdict` lets replay honor accepted historical gaps.
 */
export function judgeGateWitnesses(
  ...
  const cut = witnesses.filter(
    ...
    judge = (witness: CompletionGateWitnessV1 | undefined): GateWitnessJudgment => {
      ...
      if (admitPreservedVerdict && isPreservedVerdictWitness(witness))
        return witness.result === "pass" ? { status: "passed" } : { status: "missing" };
      const judgment =
  ...
  if (human?.override !== undefined && judge(human).status === "passed") {
    ...
    if (waivable && human.override.waivedReceiptId === waivable.receiptId)
      return {
        status: "waived",
        detail:
    ...
    if (requirement?.allowOverride === true && human.override.waivedReceiptId === null && automated === undefined)
      return {
        status: "waived",
        detail:
  ...
  const signoff = human !== undefined && human.override === undefined ? judge(human) : { status: "missing" as const };
