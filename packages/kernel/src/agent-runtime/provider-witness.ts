...
export function parseProviderWitness(value: unknown): ProviderWitnessV1 {
  if (!isRecord(value) || !hasOnlyFields(value, fields) || value.schema !== "agent-runtime-witness/v1" || !["claude-compatible", "codex"].includes(String(value.protocolFamily)) || !["wrapper", "hook"].includes(String(value.channel)) || ![...agentRuntimeEventTypes, "heartbeat"].includes(value.type as AgentRuntimeEventType | "heartbeat") || !isRecord(value.payload)) throw new Error("provider witness envelope is invalid");
