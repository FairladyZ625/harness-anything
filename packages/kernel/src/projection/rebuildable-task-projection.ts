...
type EventContentPrefetch = (
  events: readonly CanonicalEventV1[],
) => ReadonlyMap<string, Uint8Array | null>;
const batchContentPrefetchers = new WeakMap<
  EventStreamPort,
  EventContentPrefetch
>();
...
export function makeTaskProjection(options: {
  ...
  const projectionPath =
    ...
    defaultLifecycleTaskProjectionPath(options.rootDir);
  ...
  if (!Number.isInteger(limit) || limit < 1 || limit > 4096)
    throw new Error(
      "task projection catch-up limit must be between 1 and 4096",
    );
  if (localRuntimeStateFileSystem.exists(projectionPath))
    withDatabase(projectionPath, readHead, (db) =>
      transaction(db, () => {
        if (markRuntimeSessionsUnknown(db) > 0)
  ...
  return {
    ...
    apply: (event, plan) => {
      ...
      if (
        isTaskEvent(event) &&
        ((event.payload.documentClaims?.length ?? 0) > 0 ||
          (event.payload.carriedDocumentClaims?.length ?? 0) > 0)
      )
      ...
      if (isPresetSnapshotUpgradeEvent(event))
        assertPresetSnapshotUpgradeWritePlan(
          event,
          plan as FrozenWritePlan<"PresetSnapshotUpgrade">,
        );
      ...
      const receipt = withDatabase(projectionPath, readHead, (db) =>
        reduceBatch(
          ...
          [event],
          limit,
          options.eventStore.readContentBlob,
          readHead()?.revision ?? 0,
        ),
      );
    ...
    readStateDigest: () =>
      withDatabase(projectionPath, readHead, readStateDigest),
    read: (taskId) =>
      readProjection(
        projectionPath,
        readHead,
    ...
    readTaskRelations: () =>
      withDatabase(projectionPath, readHead, (db) => {
        const round = catchUpRound(db, options.eventStore, limit),
    ...
    readOperation: (opId) =>
      withDatabase(projectionPath, readHead, (db) => {
        const row = db
          .prepare("SELECT event_json FROM event_index WHERE op_id = ?")
          .get(opId) as { readonly event_json: string } | undefined;
        return row === undefined
          ? null
          : { event: parseEventJson(row.event_json), watermark: watermark(db) };
      }),
    readRelationTruth: () =>
      withDatabase(projectionPath, readHead, (db) => {
        ...
        return {
          ...
          coverageRows: decisions.coverageRows.map((row) => ({
            ...row,
            fulfillment:
              row.fulfillment === "standing_policy"
                ? ("standing-policy" as const)
                : row.fulfillment,
          })),
        };
      }),
    ...
    readTaskCompletion: (taskId, executionId) =>
      withDatabase(projectionPath, readHead, (db) => {
        catchUpRound(db, options.eventStore, limit);
        const row = queryRows(
          db,
    ...
    readRuntimeDispatch: (runtimeSessionIdValue, definitionSnapshotRef) =>
      withDatabase(projectionPath, readHead, (db) => {
        ...
        if (!row) return null;
        const event = parseEventJson(String(row.event_json));
        return isAgentRuntimeEvent(event) &&
    ...
    readRuntimeSessionEvents: (runtimeSessionIdValue, afterRevision, limit) =>
      withDatabase(projectionPath, readHead, (db) => {
        if (
          ...
          throw new Error(
            "runtime session event page requires a non-negative revision and a positive limit",
          );
        return queryRows(
          db,
          ...
          .map((row) => parseEventJson(String(row.event_json)))
          .filter((event): event is AgentRuntimeEventV1 =>
            isAgentRuntimeEvent(event),
          );
      }),
    ...
    readReplicaBasis: (afterRevision) => {
      if (
        ...
        throw new Error(
          "replica basis revision must be a non-negative integer or null",
        );
      const sourceRevision = options.eventStore.readHead()?.revision ?? 0;
        ...
        (db) =>
          (
            db
              ...
              .get(documentPath, documentPath) as
    ...
    readProgress: (taskId) =>
      withDatabase(projectionPath, readHead, (db) => {
        ...
        return {
          ...
          rows: rows.map(
            (row) =>
              parseEventJson(String(row.event_json)) as TaskProgressEventV1,
          ),
          watermark: current,
          sourceRevision: round.sourceRevision,
        };
    ...
    readRuntimeInstallation: (installationId) =>
      withDatabase(projectionPath, readHead, (db) =>
        readRuntimeInstallation(db, installationId),
      ),
    readRuntimeInstallations: () =>
      withDatabase(projectionPath, readHead, readRuntimeInstallations),
    readRuntimeSession: (runtimeSessionIdValue) =>
      withDatabase(projectionPath, readHead, (db) =>
        readRuntimeSession(db, runtimeSessionIdValue),
      ),
    readRuntimeSessions: () =>
      withDatabase(projectionPath, readHead, readRuntimeSessions),
    readRuntimeSessionsForTask: (taskId) =>
      withDatabase(projectionPath, readHead, (db) =>
        readRuntimeSessions(db).filter((session) =>
          session.taskBindings.some((binding) => binding.taskId === taskId),
        ),
      ),
    readLeaseIntervals: (taskId) =>
      withDatabase(projectionPath, readHead, (db) => readIntervals(db, taskId)),
    currentLease: (taskId, at) =>
      withDatabase(projectionPath, readHead, (db) =>
        effectiveLease(db, taskId, at ?? now()),
      ),
  ...
  return withDatabase(projectionPath, readHead, (db) => {
    ...
    if (
      ...
      return {
        ...
        sourceRevision: round.sourceRevision,
        warnings:
          !existed && round.sourceRevision > 0 ? ["projection_missing"] : [],
      };
    }
...
function readPresetSnapshot(
  ...
  return withDatabase(projectionPath, readHead, (db) => {
    const round = catchUpRound(db, eventStore, limit),
      current = watermark(db),
      row = db
        .prepare("SELECT value_json FROM preset_snapshot WHERE digest = ?")
  ...
  return withDatabase(projectionPath, readHead, (db) => {
    ...
    return {
      ...
      packagePath:
        (
          db
            .prepare("SELECT package_path FROM task_package WHERE task_id = ?")
...
function rebuildProjection(
  ...
  let transactions = 0,
    ...
    maxBatchItems = 0;
  for (;;) {
    const round = withDatabase(projectionPath, readHead, (db) =>
  ...
  const result = withDatabase(projectionPath, readHead, (db) =>
    transaction(db, () => {
      ...
      if (digest === null)
        throw new Error(
          "projection rebuild did not reach a source-complete state digest",
        );
...
const projectionDatabaseOwners = new WeakMap<
  EventStreamPort["readHead"],
  Map<string, ProjectionDatabaseOwner>
>();
...
export function closeTaskProjectionsUnder(rootDir: string): void {
  ...
  for (const projectionPath of [...projectionClosers.keys()]) {
    if (projectionPath !== resolvedRoot && !projectionPath.startsWith(prefix))
      continue;
...
function projectionDatabaseOwner(
  ...
  const initialize = () => {
    open();
    if (
      projectionSchemaVersion(db!) !== null &&
      projectionSchemaVersion(db!) !== taskProjectionSchemaVersion
    ) {
      discard();
      open();
    }
...
function matchesLedgerIdentity(
  db: DatabaseSync,
  ...
  if (
    Number(row.watermark) > sourceRevision ||
    Number(row.scanned_revision) > sourceRevision
  )
    return false;
  return (
    Number(row.scanned_revision) !== sourceRevision ||
    row.head_digest === (head?.eventDigest ?? null)
  );
}
...
function projectionSchemaVersion(db: DatabaseSync): number | null {
  if (
    queryRows(
      db,
...
function reduceBatch(
  ...
  return transaction(db, () => {
    ...
    const state = db
      .prepare(
        "SELECT scan_cursor, scanned_revision FROM projection_meta WHERE singleton = 1",
      )
      .get() as {
...
function catchUpRound(
  ...
  const shouldScan =
    state.scan_cursor !== null || state.scanned_revision < sourceRevision;
  const batch = shouldScan
    ? eventStore.readBatch(state.scan_cursor, limit)
    : null;
  if (batch?.prefetchContent !== undefined)
    batchContentPrefetchers.set(eventStore, batch.prefetchContent);
  const hasDeferred =
    db
      .prepare(
        "SELECT 1 AS present FROM event_source WHERE workspace_revision = ?",
      )
      .get(watermark(db) + 1) !== undefined;
  if (batch === null && !hasDeferred) {
    ...
    const digest = transaction(db, () =>
      refreshStateDigestAtSourceCut(db, sourceRevision),
    );
    if (digest === null)
      throw new Error("projection digest refresh lost its source-complete cut");
    return {
      sourceRevision,
      watermark: current,
      reducedItems: 0,
  ...
  let prefetch =
    batch?.prefetchContent ?? batchContentPrefetchers.get(eventStore);
  if (prefetch === undefined && hasDeferred) {
    prefetch = eventStore.readBatch(null, 1).prefetchContent;
    if (prefetch !== undefined)
      batchContentPrefetchers.set(eventStore, prefetch);
  }
  if (replayEvents.length > 0 && prefetch === undefined)
    throw new Error(
      "event stream must provide a verified batch content prefetch",
    );
  const prefetchedContent = replayEvents.length
    ? prefetch!(replayEvents)
  ...
  const reducedItems = transaction(db, () => {
    if (batch !== null) {
      ...
      if (batch.done)
        ...
        runSql(
          db,
          "UPDATE projection_meta SET scan_cursor = ? WHERE singleton = 1",
          batch.cursor,
        );
    }
    const reduced = drainDeferred(
      db,
      limit,
      (sha256) => prefetchedContent.get(sha256) ?? null,
    );
    refreshStateDigestAtSourceCut(db, sourceRevision);
    return reduced;
  });
...
function readyDeferredEvents(
  ...
  for (const row of queryRows(
    ...
    candidates.set(
      Number(row.workspace_revision),
      parseEventJson(String(row.event_json)),
    );
  for (const event of batch)
    if (event.workspaceRevision <= current + limit)
      candidates.set(event.workspaceRevision, event);
...
function stageEvent(db: DatabaseSync, event: CanonicalEventV1): void {
  ...
  if (applied !== undefined) {
    if (applied.event_json !== eventJson)
      throw new Error(`projection opId ${event.opId} names different bytes`);
    return;
  }
  const staged = db
    .prepare(
    ...
    .get(event.opId, event.workspaceRevision) as
    { readonly event_json: string } | undefined;
  if (staged !== undefined) {
    if (staged.event_json !== eventJson)
      throw new Error(
        `projection revision or opId ${event.opId} names different bytes`,
      );
    return;
  }
  runSql(
...
function drainDeferred(
  ...
  while (reduced < limit) {
    const row = db
      ...
      .get(next + 1) as { readonly event_json: string } | undefined;
    if (row === undefined) break;
    const event = parseEventJson(row.event_json);
    applyEvent(db, event, row.event_json, readBlob);
    runSql(
      db,
      "DELETE FROM event_source WHERE workspace_revision = ?",
      event.workspaceRevision,
    );
  ...
  runSql(
    db,
    "UPDATE projection_meta SET watermark = ? WHERE singleton = 1",
    next,
  );
  return reduced;
}
...
function applyEvent(
  ...
  if (isAgentEntityEvent(event)) {
    ...
    if (!bytes || bytes.byteLength !== claim.size)
      throw new Error(
        `agent entity declaration blob ${claim.sha256} is unavailable`,
      );
    let body: string;
    try {
      ...
      throw new Error(
        `agent entity declaration blob ${claim.sha256} is not UTF-8`,
      );
    }
    if (sha256Text(body) !== claim.sha256)
      throw new Error(
        `agent entity declaration blob ${claim.sha256} hash mismatch`,
      );
  ...
  if (isAgentRuntimeEvent(event)) {
    const taskId =
      event.type === "runtime_session_task_bound" ? event.payload.taskId : null;
    runSql(
      db,
    ...
    const installation = reduceRuntimeInstallation(
      event.type === "runtime_installation_observed"
        ? readRuntimeInstallation(db, event.payload.installationId)
        : null,
      event,
    );
    ...
    if (sessionId !== null) {
      const session = reduceRuntimeSession(
        readRuntimeSession(db, sessionId),
        event,
      );
      if (session !== null)
        runSql(
  ...
  if (isDocEvent(event)) {
    ...
    for (const change of event.payload.changes) {
      const previous = db
        ...
        base = previous
          ? (JSON.parse(previous.value_json) as DocumentState)
          : null;
      ...
      if (!bytes || bytes.byteLength !== change.candidate.size)
        throw new Error(
          `document blob ${change.candidate.sha256} is unavailable`,
        );
      let body: string;
      try {
        body = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
        ...
        throw new Error(
          `document blob ${change.candidate.sha256} is not UTF-8`,
        );
      }
  ...
  if (isPresetSnapshotUpgradeEvent(event)) {
    ...
    if (
      ...
      throw new Error(
        `preset snapshot upgrade basis mismatch for ${event.taskId}`,
      );
    const changed = {
    ...
    if (
      ...
      throw new Error(
        `preset snapshot upgrade changed immutable task fields for ${event.taskId}`,
      );
    const snapshot = JSON.parse(
        new TextDecoder("utf-8", { fatal: true }).decode(snapshotBytes),
      ),
      contractBody = new TextDecoder("utf-8", { fatal: true }).decode(
        contractBytes,
      ),
  ...
  if (isTaskBootstrapEvent(event)) {
    ...
    if (
      ...
      throw new Error(
        `preset snapshot blob ${event.payload.presetSnapshotClaim.sha256} is unavailable`,
      );
    const snapshotBody = new TextDecoder("utf-8", { fatal: true }).decode(
        snapshotBytes,
      ),
    ...
    runSql(
      db,
      "INSERT INTO task_package(task_id, package_path) VALUES (?, ?)",
      event.taskId,
    ...
    runSql(
      db,
      "INSERT OR IGNORE INTO task_generation VALUES (?, 'v1')",
      event.taskId,
    );
    if (
      Number(
      ...
      throw new Error(
        `preset snapshot digest ${event.payload.presetSnapshotClaim.digest} names different bytes`,
      );
    for (const claim of event.payload.initialDocumentClaims) {
      const bytes = readBlob(claim.sha256);
      if (!bytes || bytes.byteLength !== claim.size)
        throw new Error(`document blob ${claim.sha256} is unavailable`);
  ...
  const packagePath = queryRows(
    db,
    "SELECT package_path FROM task_package WHERE task_id = ?",
    event.taskId,
  )[0]?.package_path;
  refreshTaskRelationProjection(
    db,
  ...
  if (lifecycleClaims.length) {
    ...
    for (const claim of lifecycleClaims) {
      ...
      if (!bytes || bytes.byteLength !== claim.size)
        throw new Error(
          `lifecycle document blob ${claim.sha256} is unavailable`,
        );
      const body = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
      const document: DocumentState = {
        path: claim.path as DocumentState["path"],
  ...
  for (const change of event.payload.carriedDocumentClaims ?? []) {
    const previous = queryRows(
        db,
        "SELECT value_json FROM document WHERE path = ?",
        change.path,
      )[0],
      base = previous
        ? (JSON.parse(String(previous.value_json)) as DocumentState)
        : null,
      bytes = readBlob(change.candidate.sha256);
    if (!bytes || bytes.byteLength !== change.candidate.size)
      throw new Error(
        `carried document blob ${change.candidate.sha256} is unavailable`,
      );
    let body: string;
    try {
      body = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
      ...
      throw new Error(
        `carried document blob ${change.candidate.sha256} is not UTF-8`,
      );
    }
    if (
      change.baseBlobSha256 !== (base?.blobSha256 ?? null) ||
      !verifyDocEventChange(change, base?.body ?? "", body)
    )
...
function projectMigration(
  ...
  if (entity.kind === "task") {
    ...
    runSql(
      db,
      "INSERT INTO task_package VALUES (?, ?)",
      entity.task.taskId,
      entity.packagePath,
    );
    runSql(
      db,
      "INSERT INTO task_generation VALUES (?, 'v0')",
      entity.task.taskId,
    );
    refreshTaskRelationProjection(
      db,
  ...
  if (entity.kind === "decision") {
    ...
    refreshDecisionDocumentSearch(
      db,
      migrationDocument(db, entity.documentClaim.path)!,
    );
    return;
  }
  if (entity.kind === "fact") {
  ...
  if (entity.kind === "execution") {
    ...
    if (
      ...
      throw new Error(
        `migration execution owner or identity mismatch for ${value.executionId}`,
      );
    const next = {
      ...snapshot,
      revision: event.workspaceRevision,
  ...
  if (entity.kind === "task-document") {
    if (!readSnapshot(db, entity.taskId).task)
      throw new Error(
        `migration task document owner is missing for ${entity.taskId}`,
      );
    storeMigrationDocument(db, event, entity.documentClaim, readBlob);
    return;
  }
...
function storeMigrationDocument(
  db: DatabaseSync,
  event: MigrationImportEventV1,
  ...
  const body = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
  if (sha256Text(body) !== claim.sha256)
    throw new Error(`migration document blob ${claim.sha256} hash mismatch`);
  const document: DocumentState = {
    path: claim.path as DocumentState["path"],
    blobSha256: claim.sha256,
    body,
...
function projectProgress(
  ...
  if (
    event.payload.baseDocumentSha256 !== (base?.blobSha256 ?? null) ||
    !bytes ||
    bytes.byteLength !== claim.size
  )
    throw new Error(
      `progress document base or blob mismatch for task ${taskId}`,
    );
  const body = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
  if (sha256Text(body) !== claim.sha256)
    throw new Error(`progress append proof mismatch for task ${taskId}`);
  const document: DocumentState = {
    path: claim.path as DocumentState["path"],
    blobSha256: claim.sha256,
    body,
  ...
  for (const change of event.payload.carriedDocumentClaims ?? []) {
    ...
    try {
      carriedBody = new TextDecoder("utf-8", { fatal: true }).decode(
        carriedBytes,
      );
    } catch {
      throw new Error(
...
function projectFact(
  ...
  const packagePath = queryRows(
    ...
    bytes = readBlob(claim.sha256);
  if (
    (packagePath && claim.path !== `${packagePath}/facts.md`) ||
    !bytes ||
    bytes.byteLength !== claim.size
  )
    throw new Error(
      `fact document path or blob mismatch for task ${event.taskId}`,
    );
  reduceFactEvent(db, event);
  const body = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
  if (sha256Text(body) !== claim.sha256)
    throw new Error(
      `fact document projection mismatch for task ${event.taskId}`,
    );
  const document: DocumentState = {
    path: claim.path as DocumentState["path"],
    blobSha256: claim.sha256,
...
function projectDecision(
  ...
  const claim = event.payload.decisionDocumentClaim,
    previousRow = queryRows(
      db,
      "SELECT value_json FROM document WHERE path = ?",
      claim.path,
    )[0],
    previous = previousRow
      ? (JSON.parse(String(previousRow.value_json)) as DocumentState)
      : null,
    bytes = readBlob(claim.sha256);
  if (
    ...
    throw new Error(
      `decision document path, base, or blob mismatch for ${event.decisionId}`,
    );
  reduceDecisionEvent(db, event);
  const state = readDecisionDocumentState(db, event.decisionId);
  if (!state)
    throw new Error(`decision projection missing for ${event.decisionId}`);
  const body = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
  if (sha256Text(body) !== claim.sha256)
    throw new Error(
      `decision document projection mismatch for ${event.decisionId}`,
    );
  const document: DocumentState = {
    path: claim.path as DocumentState["path"],
    blobSha256: claim.sha256,
...
function replayClaim(
  ...
  db.prepare(
    ...
    event.payload.previousHolder === null
      ? null
      : canonicalJson(event.payload.previousHolder),
    event.payload.leaseExpiresAt,
    event.payload.reason,
  );
}

function replayRenew(
  db: DatabaseSync,
  event: Extract<TaskEventV1, { readonly type: "lease_renewed" }>,
): void {
...
function readSnapshot(
  ...
  const lease =
    now === undefined
      ? storedLease(db, taskId)
      : effectiveLease(db, taskId, now);
  ...
  const decisionRelations = (
    queryRows(
      db,
      "SELECT row_json FROM relation_edge WHERE target_ref = ?",
      `task/${taskId}`,
    ) as readonly { readonly row_json: string }[]
  ).map((edge) => {
...
function readIntervals(
  ...
  const rows = db
    .prepare(
      "SELECT * FROM lease_interval WHERE task_id = ? ORDER BY acquired_revision",
    )
  ...
  return rows.map((row) => ({
    ...
    releasedRevision:
      row.released_revision === null ? null : Number(row.released_revision),
    holder: JSON.parse(String(row.holder_json)) as LeaseHolder,
    previousHolder:
      row.previous_holder_json === null
        ? null
        : (JSON.parse(String(row.previous_holder_json)) as LeaseHolder),
...
function changeLease(
  ...
  runSql(
    db,
    "UPDATE lease_cas SET lease_json = ? WHERE task_id = ?",
    canonicalJson(next),
    next.taskId,
  );
  return next;
...
function effectiveLease(
  ...
  return current.phase === "reserving"
    ? null
    : checkedLease({ ...current, phase: "orphaned" });
}

function storedLease(db: DatabaseSync, taskId: string): LeaseV1 | null {
  const row = queryRows(
    db,
    "SELECT lease_json FROM lease_cas WHERE task_id = ?",
    taskId,
  )[0];
  return row === undefined
    ? null
    : checkedLease(JSON.parse(String(row.lease_json)) as LeaseV1);
}
function readRuntimeInstallation(
  db: DatabaseSync,
  installationId: string,
): RuntimeInstallation | null {
  const row = queryRows(
    db,
    "SELECT value_json FROM runtime_installation WHERE installation_id = ?",
    installationId,
  )[0];
  return row
    ? (JSON.parse(String(row.value_json)) as RuntimeInstallation)
    : null;
}
function readRuntimeInstallations(db: DatabaseSync): RuntimeInstallation[] {
  return queryRows(
    db,
    "SELECT value_json FROM runtime_installation ORDER BY installation_id",
  ).map((row) => JSON.parse(String(row.value_json)) as RuntimeInstallation);
}
function readRuntimeSession(
  ...
  const row = queryRows(
    db,
    "SELECT value_json FROM runtime_session WHERE runtime_session_id = ?",
    sessionId,
  )[0];
  return row ? (JSON.parse(String(row.value_json)) as RuntimeSession) : null;
}
function readRuntimeSessions(db: DatabaseSync): RuntimeSession[] {
  return queryRows(
    db,
    "SELECT value_json FROM runtime_session ORDER BY runtime_session_id",
  ).map((row) => JSON.parse(String(row.value_json)) as RuntimeSession);
}
function markRuntimeSessionsUnknown(db: DatabaseSync): number {
...
function readStateDigest(db: DatabaseSync): `sha256:${string}` | null {
  const row = db
    .prepare("SELECT state_digest FROM projection_meta WHERE singleton = 1")
    .get() as { readonly state_digest: string | null };
  return row.state_digest === null
    ? null
    : (row.state_digest as `sha256:${string}`);
}

function isAtSourceCut(db: DatabaseSync, sourceRevision: number): boolean {
  const state = db
    .prepare(
      "SELECT watermark, scan_cursor, scanned_revision FROM projection_meta WHERE singleton = 1",
    )
    .get() as {
    readonly watermark: number;
    readonly scan_cursor: string | null;
...
function refreshStateDigestAtSourceCut(
  db: DatabaseSync,
  sourceRevision: number,
): `sha256:${string}` | null {
  ...
  runSql(
    db,
    "UPDATE projection_meta SET state_digest = ? WHERE singleton = 1",
    value,
  );
  return value;
}
...
function prepareQuery(db: DatabaseSync, sql: string) {
...
function queryRow(
  ...
  return prepareQuery(db, sql).get(...values) as
    Record<string, unknown> | undefined;
}
function queryRows(
  db: DatabaseSync,
  ...
  return prepareQuery(db, sql).all(...values) as unknown as readonly Record<
    string,
    unknown
  >[];
}
...
function parseEventJson(value: string): CanonicalEventV1 {
