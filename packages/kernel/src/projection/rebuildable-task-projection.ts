...
import { assertDocSyncWritePlan, docByteLength, isDecisionEvent, isDocEvent, isFactEvent, isTaskEvent, parseCanonicalEvent, serializeCanonicalEvent, verifyDocEventChange, type CanonicalEventV1, type DocumentState } from "../domain/doc-sync.contract.ts"; import type { FrozenWritePlan } from "../domain/write-chain.contract.ts";
...
export interface DecisionProjectionSearchRead { readonly status: "ready" | "pending"; readonly decisions: readonly DecisionProjectionRow[]; readonly watermark: number; readonly sourceRevision: number }
...
export function readLifecycleRelationTruth(rootDir: string): EventBackedRelationTruth { const projectionPath = defaultLifecycleTaskProjectionPath(rootDir); if (!localRuntimeStateFileSystem.exists(projectionPath)) return { factAnchors: [], decisionAnchors: [], edges: [], coverageRows: [] }; const db = new DatabaseSync(projectionPath, { readOnly: true }); try { const facts = readFactGraphRows(db), decisions = readDecisionGraphRows(db); return { factAnchors: facts.factAnchors, decisionAnchors: decisions.decisionAnchors, edges: [...facts.edges, ...decisions.edges], coverageRows: decisions.coverageRows }; } finally { db.close(); } }
export function makeTaskProjection(options: { readonly rootDir: string; readonly eventStore: EventStreamPort;
  ...
  return {
    ...
    searchDecisions: (filters) => withDatabase(projectionPath, (db) => { const round = catchUpRound(db, options.eventStore, limit), current = watermark(db); return { status: current === round.sourceRevision ? "ready" : "pending", decisions: searchDecisionRows(db, filters), watermark: current, sourceRevision: round.sourceRevision }; }),
...
function applyEvent(db: DatabaseSync, event: CanonicalEventV1, eventJson: string, readBlob: EventStreamPort["readContentBlob"]): void {
  ...
  if (event.type === "task_created") runSql(db, "INSERT OR IGNORE INTO task_package(task_id, package_path) VALUES (?, ?)", event.taskId, `tasks/${event.taskId}-${slugifyTaskTitle(event.payload.task.title)}`);
