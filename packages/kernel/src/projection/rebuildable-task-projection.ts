...
import { assertFactWritePlan, renderFactsDocument, type DecisionEventV1, type FactEventV1 } from "../domain/fact-event.ts";
...
function applyEvent(db: DatabaseSync, event: CanonicalEventV1, eventJson: string, readBlob: EventStreamPort["readContentBlob"]): void {
  ...
  if (isDecisionEvent(event)) { reduceDecisionEvent(db, event); runSql(db, "INSERT INTO event_index(op_id, workspace_revision, task_id, event_json) VALUES (?, ?, NULL, ?)", event.opId, event.workspaceRevision, eventJson); return; }
