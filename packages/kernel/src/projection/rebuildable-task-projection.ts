...
import { checkedPageLimit, createTaskRelationProjectionTable, decodePageCursor, encodePageCursor, listTaskRowsNarrow, readTaskRelationPage, readTaskRelationRows, readTaskStatusRows, refreshTaskRelationProjection, type TaskProjectionListQuery, type TaskRelationQuery } from "./task-query-projection.ts";
...
function readTaskRuntimeBatchPage(db: DatabaseSync, query: TaskRuntimeBatchQuery): Pick<TaskRuntimeBatchRead, "taskIds" | "rows" | "page"> {
  ...
  const limit = query.limit === undefined ? 500 : checkedPageLimit(query.limit), ordered = [...query.taskIds].sort(), after = query.cursor === undefined ? null : decodePageCursor(query.cursor, 1)[0]!, remaining = after === null ? ordered : ordered.filter((taskId) => taskId > after), taskIds = remaining.slice(0, limit), last = taskIds.at(-1), selected = new Set(taskIds);
  // Bounded pages deliberately filter the maintained full-table reads in memory. No
  // variable-length id list reaches SQLite, so the 999-bind ceiling cannot reappear.
  const tasks = new Map(listTaskRowsNarrow(db, {}).rows.filter((row) => selected.has(row.task_id)).map((row) => [row.task_id, row])), sessions = new Map<string, RuntimeSession[]>();
  for (const session of readRuntimeSessions(db)) for (const binding of session.taskBindings) if (selected.has(binding.taskId)) { const rows = sessions.get(binding.taskId) ?? []; rows.push(session); sessions.set(binding.taskId, rows); }
