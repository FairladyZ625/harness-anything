...
import { assertTaskProgressWritePlan, isTaskProgressEvent, type TaskProgressEventV1 } from "../domain/task-progress-event.ts";
import { isTaskBoundRuntimeWriter, resolveLiveTaskBoundRuntimeBinding } from "../domain/task-bound-runtime-authority.ts";
...
import { assertFactAdmission, createFactProjectionTables, FactProjectionError, readFactAnchorRows, readFactGraphRows, readFactRow, reduceFactEvent, searchFactRowsPage } from "./fact-event-projection.ts";
...
export function makeTaskProjection(options: { readonly rootDir: string; readonly eventStore: EventStreamPort;
  ...
  return {
    ...
    readTaskRelations: () => withDatabase(projectionPath, readHead, (db) => { const round = catchUpRound(db, options.eventStore, limit), current = watermark(db); return { status: current === round.sourceRevision ? "ready" : "pending", rows: readTaskRelationRows(db), watermark: current, sourceRevision: round.sourceRevision }; }), readTaskDependencyClosure: (sourceRefs, maxDepth) => withDatabase(projectionPath, readHead, (db) => { const round = catchUpRound(db, options.eventStore, limit), current = watermark(db); return { status: current === round.sourceRevision ? "ready" : "pending", rows: readTaskDependencyClosureRows(db, sourceRefs, maxDepth), watermark: current, sourceRevision: round.sourceRevision }; }), readTaskRelationsByTargets: (targetRefs, relationType) => withDatabase(projectionPath, readHead, (db) => { const round = catchUpRound(db, options.eventStore, limit), current = watermark(db); return { status: current === round.sourceRevision ? "ready" : "pending", rows: readTaskRelationsByTargets(db, targetRefs, relationType), watermark: current, sourceRevision: round.sourceRevision }; }),
    ...
    readTaskRuntimeBatch: (query) => withDatabase(projectionPath, readHead, (db) => { const round = catchUpRound(db, options.eventStore, limit), current = watermark(db), page = readTaskRuntimeBatchPage(db, query); return { status: current === round.sourceRevision ? "ready" : "pending", ...page, watermark: current, sourceRevision: round.sourceRevision }; }),
    ...
    readRuntimeDispatch: (runtimeSessionIdValue, definitionSnapshotRef) => withDatabase(projectionPath, readHead, (db) => { const row = queryRow(db, "SELECT event_json FROM event_index WHERE json_extract(event_json, '$.schema') = 'agent-runtime-event/v1' AND json_extract(event_json, '$.type') = 'runtime_dispatch_requested' AND json_extract(event_json, '$.payload.runtimeSessionId') = ? AND json_extract(event_json, '$.payload.definitionSnapshotRef') = ? ORDER BY workspace_revision LIMIT 1", runtimeSessionIdValue, definitionSnapshotRef); if (!row) return null; const event = parseEventJson(String(row.event_json)); return isAgentRuntimeEvent(event) && event.type === "runtime_dispatch_requested" ? event : null; }),
    readRuntimeDispatches: () => withDatabase(projectionPath, readHead, (db) => queryRows(db, "SELECT event_json FROM event_index WHERE json_extract(event_json, '$.schema') = 'agent-runtime-event/v1' AND json_extract(event_json, '$.type') = 'runtime_dispatch_requested' ORDER BY workspace_revision").map((row) => parseEventJson(String(row.event_json))).filter((event): event is Extract<AgentRuntimeEventV1, { readonly type: "runtime_dispatch_requested" }> => isAgentRuntimeEvent(event) && event.type === "runtime_dispatch_requested")),
    ...
    readDecision: (decisionId) => withDatabase(projectionPath, readHead, (db) => { const round = catchUpRound(db, options.eventStore, limit), current = watermark(db); return { status: current === round.sourceRevision ? "ready" : "pending", decision: readDecisionRow(db, decisionId), watermark: current, sourceRevision: round.sourceRevision }; }), readDecisions: (decisionIds) => withDatabase(projectionPath, readHead, (db) => { const round = catchUpRound(db, options.eventStore, limit), current = watermark(db); return { status: current === round.sourceRevision ? "ready" : "pending", decisions: readDecisionRows(db, decisionIds), watermark: current, sourceRevision: round.sourceRevision }; }),
...
function applyEvent(db: DatabaseSync, event: CanonicalEventV1, eventJson: string, readBlob: EventStreamPort["readContentBlob"]): void {
  ...
  if (isDocEvent(event)) { runSql(db, "INSERT INTO event_index(op_id, workspace_revision, task_id, event_json) VALUES (?, ?, NULL, ?)", event.opId, event.workspaceRevision, eventJson); for (const change of event.payload.changes) { const previous = db.prepare("SELECT value_json FROM document WHERE path = ?").get(change.path) as { readonly value_json: string } | undefined, base = previous ? JSON.parse(previous.value_json) as DocumentState : null; if (change.candidate === null) { if (event.payload.retirementReason === undefined || base !== null && change.baseBlobSha256 !== base.blobSha256) throw new Error(`document retirement mismatch for ${change.path}`); runSql(db, "DELETE FROM document WHERE path = ?", change.path); continue; } if (change.baseBlobSha256 !== (base?.blobSha256 ?? null)) throw new Error(`document base mismatch for ${change.path}`); const bytes = readBlob(change.candidate.sha256); if (!bytes || bytes.byteLength !== change.candidate.size) throw new Error(`document blob ${change.candidate.sha256} is unavailable`); let body: string; try { body = new TextDecoder("utf-8", { fatal: true }).decode(bytes); } catch { throw new Error(`document blob ${change.candidate.sha256} is not UTF-8`); } if (!verifyDocEventChange(change, base?.body ?? "", body)) throw new Error(`document proof mismatch for ${change.path}`);
...
function projectProgress(db: DatabaseSync, event: TaskProgressEventV1, eventJson: string, readBlob: EventStreamPort["readContentBlob"]): void {
  const taskId = event.payload.taskId, snapshot = readSnapshot(db, taskId), lease = storedLease(db, taskId), packagePath = queryRows(db, "SELECT package_path FROM task_package WHERE task_id = ?", taskId)[0]?.package_path, claim = event.payload.resultDocumentClaim, previous = queryRows(db, "SELECT value_json FROM document WHERE path = ?", claim.path)[0], base = previous ? JSON.parse(String(previous.value_json)) as DocumentState : null, bytes = readBlob(claim.sha256), runtimeSessionIdValue = event.payload.runtimeSessionId, runtime = runtimeSessionIdValue ? readRuntimeSession(db, runtimeSessionIdValue) : null;
  const runtimeBinding = runtime === null ? null : resolveLiveTaskBoundRuntimeBinding(runtime, taskId, event.payload.executionId), directHolder = lease !== null && runtimeSessionIdValue === undefined && canonicalJson(lease.actor) === canonicalJson(event.actor) && canonicalJson(lease.source) === canonicalJson(event.source), runtimeWorker = lease !== null && runtimeSessionIdValue !== undefined && runtimeBinding !== null && isTaskBoundRuntimeWriter(lease, event.actor, event.source, runtimeBinding);
  if (snapshot.task?.status !== "active" || !packagePath || claim.path !== `${packagePath}/progress.md` || lease?.phase !== "held" || lease.executionId !== event.payload.executionId || !directHolder && !runtimeWorker) throw new Error(`progress event lease mismatch for task ${taskId}`);
  if (event.payload.baseDocumentSha256 !== (base?.blobSha256 ?? null) || !bytes || bytes.byteLength !== claim.size) throw new Error(`progress document base or blob mismatch for task ${taskId}`); const body = new TextDecoder("utf-8", { fatal: true }).decode(bytes); if (sha256Text(body) !== claim.sha256) throw new Error(`progress append proof mismatch for task ${taskId}`);
...
type SqlValue = string | number | bigint | Uint8Array | null; function runSql(db: DatabaseSync, sql: string, ...values: readonly SqlValue[]): number | bigint { return db.prepare(sql).run(...values).changes; } function prepareQuery(db: DatabaseSync, sql: string) { return db.prepare(sql); } function queryRow(db: DatabaseSync, sql: string, ...values: readonly SqlValue[]): Record<string, unknown> | undefined { return prepareQuery(db, sql).get(...values) as Record<string, unknown> | undefined; } function queryRows(db: DatabaseSync, sql: string, ...values: readonly SqlValue[]): readonly Record<string, unknown>[] { return prepareQuery(db, sql).all(...values) as unknown as readonly Record<string, unknown>[]; }
