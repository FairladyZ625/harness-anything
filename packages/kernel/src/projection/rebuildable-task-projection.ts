...
export function readLifecycleRelationFacts(rootDir: string): readonly FactProjectionRow[] { const projectionPath = defaultLifecycleTaskProjectionPath(rootDir); if (!localRuntimeStateFileSystem.exists(projectionPath)) return []; const db = new DatabaseSync(projectionPath, { readOnly: true }); try { return readFactGraphRows(db).facts; } finally { db.close(); } }
