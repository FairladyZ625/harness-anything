...
import { assertTaskProgressWritePlan, isRuntimeProgressWriter, isTaskProgressEvent, renderTaskProgressDocument, type TaskProgressEventV1 } from "../domain/task-progress-event.ts";
...
function projectProgress(db: DatabaseSync, event: TaskProgressEventV1, eventJson: string, readBlob: EventStreamPort["readContentBlob"]): void {
  ...
  const directHolder = lease !== null && runtimeSessionIdValue === undefined && canonicalJson(lease.actor) === canonicalJson(event.actor) && canonicalJson(lease.source) === canonicalJson(event.source), runtimeWorker = lease !== null && runtimeSessionIdValue !== undefined && runtime?.liveness === "live" && isRuntimeProgressWriter(lease, event.actor, event.source, runtimeSessionIdValue) && runtime.taskBindings.some((binding) => binding.taskId === taskId && binding.executionId === event.payload.executionId);
