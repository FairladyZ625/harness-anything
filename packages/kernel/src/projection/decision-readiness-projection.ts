...
export function projectDecisionReadiness(input: { readonly rootDir: string; readonly commitSha: string; readonly decisions: readonly ScopedDecision[] }, source: DecisionReadinessSource): readonly DecisionReadinessProjection[] {
  ...
  const files = tree.stdout.split("\n").filter(Boolean).sort(), projected: DecisionReadinessProjection[] = input.decisions.map(() => unknown(basis, "The canonical applies_to scope is unavailable.")), scoped: Array<{ readonly index: number; readonly decision: ScopedDecision; readonly scope: ReturnType<typeof resolveScope>; readonly since: number }> = [];
  input.decisions.forEach((decision, index) => { const scope = resolveScope(decision.appliesTo.modules, files); if (!scope.paths.length) { projected[index] = unknown(basis, scope.reason); return; } scoped.push({ index, decision, scope, since: Date.parse(decision.proposedAt) }); });
  if (!scoped.length) return projected;
  const roots = unique(scoped.flatMap(({ scope }) => scope.roots)), valid = scoped.filter(({ since }) => Number.isFinite(since)), earliest = valid.reduce((found, row) => row.since < found.since ? row : found, valid[0]);
  const driftRead = earliest ? source.run(input.rootDir, ["log", "--format=%x1e%cI", "--name-only", `--since=${earliest.decision.proposedAt}`, basis, "--", ...roots]) : { ok: false as const, stdout: "" }, history = driftRead.ok ? changeHistory(driftRead.stdout) : [];
  const conflictRead = source.run(input.rootDir, ["grep", "-n", "-I", "-E", "^(<<<<<<<|=======|>>>>>>>)", basis, "--", ...roots], true), allConflicting = conflictRead.ok ? conflictPaths(conflictRead.stdout) : [];
  for (const { index, scope, since } of scoped) {
    const changed = driftRead.ok && Number.isFinite(since) ? unique(history.filter(({ committedAtMs }) => committedAtMs >= since).flatMap(({ paths }) => paths).filter((entry) => scope.paths.includes(entry))) : [], lastCommitAt = changed.length ? history.find(({ paths }) => paths.some((entry) => changed.includes(entry)))?.committedAt ?? null : null, conflicting = allConflicting.filter((entry) => scope.paths.includes(entry));
    const scopeUnknown = scope.unresolved.length > 0, driftUnknown = !Number.isFinite(since) || !driftRead.ok || changed.length > 0 && lastCommitAt === null, conflictUnknown = !conflictRead.ok;
    const appliesToDrift: DecisionReadinessProjection["appliesToDrift"] = changed.length ? { state: "drift", paths: changed, lastCommitAt, summary: `${changed.length} canonical applies_to path(s) changed after proposal.` } : driftUnknown || scopeUnknown ? { state: "unknown", paths: [], lastCommitAt: null, summary: driftUnknown ? "The proposal timestamp or canonical Git history is unavailable." : scope.reason } : { state: "clear", paths: scope.roots, lastCommitAt: null, summary: "No canonical applies_to path changed after proposal." };
    ...
    projected[index] = { schema: "decision-readiness/v1", basisCommitSha: basis, appliesToDrift, conflictMarker };
  }
  return projected;
}
...
function changeHistory(stdout: string): readonly { readonly committedAt: string; readonly committedAtMs: number; readonly paths: readonly string[] }[] { return stdout.split("\x1e").slice(1).flatMap((block) => { const [rawCommittedAt, ...paths] = block.split("\n"), committedAt = rawCommittedAt?.trim() ?? "", committedAtMs = Date.parse(committedAt); return Number.isFinite(committedAtMs) ? [{ committedAt, committedAtMs, paths: paths.filter(Boolean) }] : []; }); }
