...
const packagePath = "tasks/task_RETITLE000000000000A-plan",
  taskId = "task_RETITLE000000000000A",
  actor = { principal: { personId: "person-retitle" }, executor: null };
...
const amendEvent = (fields: readonly string[], title: string): TaskEventV1 =>
  ({
    schema: "task-event/v1",
    eventId: "event-retitle",
    workspaceRevision: 2,
    opId: "op-retitle",
    taskId,
    type: "task_amended",
    ...
    payload: {
      task: task(title),
      mutation: { command: "amend", reason: "declared retitle", fields },
      documentClaims: [],
    },
  }) as unknown as TaskEventV1;
const snapshot = (title: string): TaskLifecycleSnapshot =>
  ({
    revision: 2,
    task: task(title),
    executions: [],
...
const plan = `${packagePath}/task_plan.md`,
...
test("lifecycle paths keep the plan out of non-retitle amends and add it only to claimed retitles", () => {
  ...
  const claimed = {
    ...amendEvent(["title"], "new title"),
    payload: {
      ...amendEvent(["title"], "new title").payload,
      documentClaims: [
        {
          path: plan,
          sha256: "0".repeat(64),
...
test("compiling a title amend retitles a published plan under the prose policy and keeps the rest byte-for-byte", () => {
  const compiled = compileTaskLifecycleWrite({
    event: amendEvent(["title"], "new title"),
    snapshot: snapshot("new title"),
    packagePath,
    currentDocuments: [
      { path: plan, body: planBody, blobSha256: "0".repeat(64) },
      {
        path: `${packagePath}/INDEX.md`,
        body: "---\ntaskId: x\nstatus: active\nowner: machine\n---\n# old title\n\n## Next\n\nn\n",
        blobSha256: "1".repeat(64),
      },
      { path: `${packagePath}/task-contract.json`, body: "{}\n", blobSha256: "2".repeat(64) },
    ],
  });
  ...
  assert.equal(
    compiled.blobs.find((blob) => blob.sha256 === planClaim.sha256)?.body,
    "# new title\n\n## Brief\n\nworker prose stays byte-for-byte\n",
  );
  assert.deepEqual(validateTaskEvent(compiled.event), []);
  assert.deepEqual(validateCurrentTaskEvent(compiled.event), []);
  assert.deepEqual(
    lifecycleDocumentPaths(compiled.event, packagePath),
    compiled.event.payload.documentClaims.map((claim) => claim.path),
  );
});
...
test("task event validation only accepts the prose policy for task_plan.md lifecycle claims", () => {
  ...
  assert.equal(
    validateTaskEvent(machine).some((issue) => /document claims/u.test(issue.message)),
    true,
  );
  const proseElsewhere = {
    ...amendEvent(["title"], "new title"),
    payload: {
      ...amendEvent(["title"], "new title").payload,
      documentClaims: [
  ...
  assert.equal(
    validateTaskEvent(proseElsewhere).some((issue) => /document claims/u.test(issue.message)),
    true,
  );
});
