...
test("closeout readiness requires a passing exact-cut gate, not witness existence", () => {
  ...
  assert.equal(
    closeoutReadiness({
      ...stale,
      gateWitnesses: stale.gateWitnesses.map((witness) => ({ ...witness, commitSha: "c".repeat(40) })),
    }).readiness,
    "incomplete",
  );
      ...
      codeDocWitnesses: [
        {
          ...
          actor,
          source: "local",
          reconciledAt: "2026-08-18T00:04:00.000Z",
        },
      ],
    }).readiness,
    "ready",
...
test("closeout consumes the latest content-pinned consent selection and ignores other Reviews", () => {
  const base = closeout("pass"),
    selected = base.reviews[0]!,
    selectedConsent = base.consents[0]!;
  const dismissed = {
    ...selected,
    reviewId: "review-dismissed",
    verdict: "dismissed" as const,
    reason: "superseded review history",
  };
  const unselected = { ...selected, reviewId: "review-unselected", reason: "approved but not selected" };
  const earlierConsent = {
    ...selectedConsent,
    consentId: "consent-unselected",
    reviewId: unselected.reviewId,
    reviewDigest: reviewDigest(unselected),
    contentDigest: unselected.contentDigest,
  };
  ...
  const unpinnedSelection = {
    ...history,
    consents: [{ ...selectedConsent, reviewDigest: `sha256:${"0".repeat(64)}` as `sha256:${string}` }],
  };
  assert.equal(
    closeoutReadiness(unpinnedSelection).blocker,
    "consent",
    "an unselected approved Review must not satisfy the gate",
  );
});
...
test("closeout readiness gates milestone and long_running completion on an active decision derives edge", () => {
  ...
  assert.equal(
    closeoutReadiness({
      ...closeout("pass"),
      task: { ...closeout("pass").task!, taskId: "task-1", taskClass: "standard" },
    }).readiness,
    "ready",
  );
...
test("completed tasks retain passed gate badges from their accepted execution cut", () => {
  ...
  const assessment = closeoutReadiness({
    ...snapshot,
    task: { ...snapshot.task!, status: "done" },
    executions: snapshot.executions.map((execution) => ({ ...execution, state: "accepted" })),
  });
...
test("fact liveness retires the target of the canonical supersedes-fact edge", () => {
  const edges = [
    { sourceRef: "fact/task/F-new", targetRef: "fact/task/F-old", relationType: "supersedes-fact", state: "active" },
  ];
...
test("coverage handles transitive evidence, delivered tasks, standing policy, and only live refuters", () => {
  const decisions = [
    ...
    },
    {
      ...
      claims: [{ ref: "decision/policy/C1", loadBearing: true, fulfillment: "standing-policy" as const }],
    },
  ];
  const relations = [
    {
      relationId: "via",
      sourceRef: "decision/d1/C1",
  ...
  const rows = coverageOf(
    decisions,
    [{ ref: "fact/task/F-live" }, { ref: "fact/task/F-retired" }, { ref: "fact/task/F-new" }],
...
test("freshness reason classifies uncovered causes once, in the domain", () => {
  ...
  assert.equal(
    freshnessReasonOf({ status: "covered", fulfillment: "evidenced", refutingFactRefs: ["fact/task/F-live"] }),
    null,
  );
  // refuted outranks the other causes: an undeclared claim under active refutation is refuted.
  assert.equal(
    freshnessReasonOf({ status: "uncovered", fulfillment: "evidenced", refutingFactRefs: ["fact/task/F-live"] }),
    "refuted",
  );
  assert.equal(
    freshnessReasonOf({ status: "uncovered", fulfillment: null, refutingFactRefs: ["fact/task/F-live"] }),
    "refuted",
  );
  assert.equal(
    freshnessReasonOf({ status: "uncovered", fulfillment: "evidenced", refutingFactRefs: [] }),
    "no-live-evidence",
  );
  assert.equal(
    freshnessReasonOf({ status: "uncovered", fulfillment: "delivered", refutingFactRefs: [] }),
    "no-live-evidence",
  );
  assert.equal(
    freshnessReasonOf({ status: "uncovered", fulfillment: "standing-policy", refutingFactRefs: [] }),
    "no-live-evidence",
  );
  assert.equal(
    freshnessReasonOf({ status: "uncovered", fulfillment: null, refutingFactRefs: [] }),
    "fulfillment-undeclared",
  );
