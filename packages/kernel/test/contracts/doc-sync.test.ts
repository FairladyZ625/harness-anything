...
test("new prose may establish frontmatter while existing machine frontmatter stays immutable", () => {
  const path = documentPath("context/frontmatter.md"), candidate = "---\ntitle: New document\n---\n# Notes\nBody\n";
  const created = decide({ path, baseBlobSha256: null, policyId: DOC_POLICY_ID, candidate: claim(candidate) }, null, Buffer.from(candidate));
  assert.equal(created.accepted, true, JSON.stringify(created)); if (!created.accepted) return;
  const change = created.event.payload.changes[0]!;
  assert.equal(change.regionProofs.some((proof) => proof.regionId === "machine/frontmatter"), true);
  assert.equal(verifyDocEventChange(change, "", candidate), true);

  const existingEmpty = { ...state(""), path };
  const introduced = decide({ path, baseBlobSha256: existingEmpty.blobSha256, policyId: DOC_POLICY_ID, candidate: claim(candidate) }, existingEmpty, Buffer.from(candidate));
  assert.equal(introduced.accepted, false); if (!introduced.accepted) assert.deepEqual([introduced.code, introduced.detail.unresolvedTouches[0]?.reason], ["unresolved_touch", "new machine region is forbidden"]);

  const edited = candidate.replace("New document", "Changed document"), current = { ...state(candidate), path };
  const changed = decide({ path, baseBlobSha256: current.blobSha256, policyId: DOC_POLICY_ID, candidate: claim(edited) }, current, Buffer.from(edited));
  assert.equal(changed.accepted, false); if (!changed.accepted) assert.deepEqual([changed.code, changed.detail.unresolvedTouches[0]?.reason], ["unresolved_touch", "machine region changed"]);
});
