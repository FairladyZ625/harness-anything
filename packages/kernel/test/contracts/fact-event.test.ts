...
test("Decision compiler renders the exact single-file package and frozen write plan", () => {
  assert.equal(decision.path, "decisions/decision-dec_CONTRACT/decision.md"); assertDecisionWritePlan(decision.event, decision.plan); assert.deepEqual(decision.plan.targets.map(({ kind }) => kind), ["event_file", "event_head", "authored_file", "content_blob", "projection_invalidation", "projection_invalidation", "projection_invalidation"]); assert.deepEqual(decision.blobs[0], { sha256: decision.event.payload.decisionDocumentClaim.sha256, size: decision.event.payload.decisionDocumentClaim.size, mediaType: "text/markdown", body: decision.body });
