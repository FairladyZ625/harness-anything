...
import {
  validateVerticalScriptAction,
  validateVerticalScriptPlan,
  validateVerticalScriptResult,
} from "../../src/domain/vertical-script-action.ts";
...
const plan = {
  ...
  changes: [
    {
      path: "context/architecture/README.md",
      body: "# Architecture\n",
...
test("vertical script action and plan validators are closed and parse canonical values", () => {
  ...
  for (const invalid of [
    ...
    { ...result, documents: [{ ...result.documents[0], size: "6" }] },
  ])
    assert.notEqual(validateVerticalScriptResult(invalid).length, 0);
});
