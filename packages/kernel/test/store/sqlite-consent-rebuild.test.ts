...
import { queryConsentsBySourceStrength, rebuildTaskProjection } from "../../src/index.ts";
...
test("SQLite consent projection rebuild is deterministic after cache deletion", () => {
  withTempStore((rootDir) => {
    ...
    assert.equal(first[0]?.source_strength, "asserted");
    assert.equal(queryConsentsBySourceStrength({ rootDir, sourceStrength: "asserted" }).length, 1);
  });
});
