...
test("startup recovery is independent of 100 versus 10,000-event history", async (context) => {
  const fixture = async (count: number) => withTempStoreAsync(async (rootDir) => { initRepo(rootDir); const eventsRoot = path.join(rootDir, "harness/events"); mkdirSync(eventsRoot, { recursive: true }); let last = event;
    ...
    const measured = makeTaskEventStore({ repoId: "test-repo", rootDir }), appendParent = measured.currentCommit().sha, appendReceipt = measured.append(bundle(eventAt(count + 1))), objectBytes = incrementalObjectBytes(rootDir, appendParent, appendReceipt.commitSha.sha), next = eventAt(count + 2), interrupted = makeTaskEventStore({ repoId: "test-repo", rootDir, killpoint: (point) => { if (point === "after_head_write") throw new Error("crash"); } }); assert.throws(() => interrupted.append(bundle(next)), /crash/u);
    const started = performance.now(), recovered = makeTaskEventStore({ repoId: "test-repo", rootDir }).recover(), elapsed = performance.now() - started, constructorMs = elapsed - recovered.elapsedMs; assert.equal(recovered.status, "committed"); context.diagnostic(`recovery-${count} constructor=${constructorMs.toFixed(3)}ms recover=${recovered.elapsedMs.toFixed(3)}ms appendProcesses=${appendReceipt.metrics.gitProcesses} appendObjectBytes=${objectBytes}`); return { elapsed, constructorMs, recoverMs: recovered.elapsedMs, appendProcesses: appendReceipt.metrics.gitProcesses, objectBytes }; });
  const hundred = await fixture(100), thousand = await fixture(1_000), tenThousand = await fixture(10_000), ratio = tenThousand.elapsed / hundred.elapsed, constructorRatio = tenThousand.constructorMs / hundred.constructorMs, recoverRatio = tenThousand.recoverMs / hundred.recoverMs, objectRatio = tenThousand.objectBytes / thousand.objectBytes; context.diagnostic(`recovery 100=${hundred.elapsed.toFixed(3)}ms 10000=${tenThousand.elapsed.toFixed(3)}ms ratio=${ratio.toFixed(3)} constructorRatio=${constructorRatio.toFixed(3)} recoverRatio=${recoverRatio.toFixed(3)} appendObjectBytes1000=${thousand.objectBytes} appendObjectBytes10000=${tenThousand.objectBytes} objectRatio=${objectRatio.toFixed(3)}`); assert.equal(ratio < 2, true, `10k/100 ratio ${ratio}`); assert.equal(tenThousand.appendProcesses, hundred.appendProcesses, "append Git subprocess count must be history-independent"); assert.equal(objectRatio < 2, true, `10k/1k append object-byte ratio ${objectRatio}`);
});
