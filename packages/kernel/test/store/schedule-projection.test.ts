...
test("Schedule definition and run view share one canonical stream and rebuild exactly without a schema bump", async () => {
  await withTempStoreAsync(async (rootDir) => {
    initRepo(rootDir);
    assert.equal(taskProjectionSchemaVersion, 12);
