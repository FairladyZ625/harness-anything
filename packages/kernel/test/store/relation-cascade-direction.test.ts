...
test("entity cascade treats undirected relations as incoming and outgoing on both endpoints", () => {
  withTempStore((rootDir) => {
    ...
    assert.deepEqual(
      fromA.incoming.map((edge) => edge.relationId),
      [relation.relation_id],
    );
    assert.deepEqual(
      fromA.outgoing.map((edge) => edge.relationId),
      [relation.relation_id],
    );
    assert.deepEqual(fromA.impactedRefs, ["task/task-undirected-b"]);
    assert.deepEqual(
      fromB.incoming.map((edge) => edge.relationId),
      [relation.relation_id],
    );
    assert.deepEqual(
      fromB.outgoing.map((edge) => edge.relationId),
      [relation.relation_id],
    );
