...
test("runtime events use the canonical envelope, head, store, and the shared projection transaction", async () => {
  await withTempStoreAsync(async (rootDir) => {
    ...
    assert.deepEqual(projection.getEntity("runtime-session", "runtime-session-claude")?.value, { schema: "runtime-session/v1", runtimeSessionId: "runtime-session-claude", taskBindings: [{ taskId: "task-runtime", executionId: "execution-claude" }], liveness: "exited", outcome: "succeeded", semanticState: "succeeded" });
    const executionRelation = runtimeTaskExecutionRelation("runtime-session-claude", "task-runtime");
    assert.equal(projection.readRelationTruth().edges.filter(({ relationId }) => relationId === executionRelation.relation_id).length, 1);
