...
test("task bootstrap publishes and rebuilds one exact Task, snapshot, and document cut", () => {
  ...
  try {
    ...
    const event: TaskBootstrapEventV1 = { schema: "task-bootstrap-event/v1", eventId: "event-bootstrap", workspaceRevision: 1, opId: "op-bootstrap", taskId: "task-bootstrap", type: "task_bootstrapped", actor: { principal: { personId: "person-1" }, executor: null }, source: "local", occurredAt: "2026-08-13T00:00:00.000Z", payload: {
      ...
      presetSnapshotClaim: { digest, sha256: snapshotSha, size: Buffer.byteLength(snapshotBody), mediaType: "application/json" }, initialDocumentClaims: [{ path: "tasks/task-bootstrap/task_plan.md", sha256: documentSha, size: Buffer.byteLength(documentBody), mediaType: "text/markdown", policyId: "markdown-body-replaceable/v1" }] } };
    ...
    assert.throws(() => store.append(tampered, taskBootstrapWritePlan(tampered), tamperedBlobs), /snapshot.*digest/iu); assert.deepEqual(store.currentCommit(), before);
    ...
    const plan = taskBootstrapWritePlan(event); assert.equal(plan.targets.some((target) => target.kind === "authored_file" && target.path === "tasks/task-bootstrap/task_plan.md"), true); const receipt = store.append(event, plan, blobs); assert.equal(receipt.revision, 1); assert.equal(git(rootDir, "rev-parse", "HEAD"), receipt.commitSha.sha); assert.equal(git(rootDir, "rev-parse", "refs/ha/canonical"), receipt.commitSha.sha); assert.equal(readFileSync(path.join(rootDir, "harness/tasks/task-bootstrap/task_plan.md"), "utf8"), documentBody); assert.equal(projection.apply(event, plan).metrics.reducedItems, 1);
