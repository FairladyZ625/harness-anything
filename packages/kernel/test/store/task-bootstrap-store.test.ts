...
test("task bootstrap publishes and rebuilds one exact Task, snapshot, and document cut", async () => {
  ...
  try {
    ...
    projection.rebuild(); assert.deepEqual(projection.read("task-bootstrap").snapshot.task, event.payload.task); assert.equal(projection.list().rows[0]?.packagePath, "tasks/task-bootstrap-bootstrap"); assert.equal(projection.list().rows[0]?.createdAt, event.occurredAt); assert.deepEqual(projection.readPresetSnapshot(digest).snapshot, JSON.parse(snapshotBody)); assert.equal(projection.readDocument("tasks/task-bootstrap-bootstrap/task_plan.md").document?.body, documentBody);
