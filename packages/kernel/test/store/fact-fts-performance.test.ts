...
test("100k Fact FTS exact searches stay indexed with p95 below 10ms", (context) => {
  ...
  try {
    ...
    for (let index = 0; index < 100_000; index += 1) {
      const taskId = `task-${index % 100}`,
        factId = `F-${String(index).padStart(8, "0")}`,
        statement = `Indexed observation token${index}`;
      ...
      insertFts.run(factId, statement, row.evidenceSource);
    }
    db.exec("COMMIT");

    const plan = db
      .prepare("EXPLAIN QUERY PLAN SELECT rowid FROM fact_fts WHERE fact_fts MATCH ?")
      .all('"token99999"') as unknown as readonly { readonly detail: string }[];
    assert.equal(
      plan.some(({ detail }) => /VIRTUAL TABLE INDEX/u.test(detail)),
      true,
      JSON.stringify(plan),
    );
    ...
    assert.equal(
      readFactGraphRows(db).facts.length,
      100_000,
...
test("10k Decision FTS searches stay indexed after refresh with p95 below 10ms", (context) => {
  ...
  try {
    ...
    const insert = db.prepare(
      "INSERT INTO decision(decision_id,state,title,question,risk_tier,urgency,vertical,preset,decision_class,applies_json,proposer_json,arbiter_json,proposed_at,decided_at,workspace_revision) VALUES (?, 'proposed', ?, ?, 'high', 'medium', 'performance', 'default', 'ordinary', ?, ?, NULL, ?, NULL, ?)",
    );
    const insertFts = db.prepare(
        "INSERT INTO decision_fts(decision_id,title,question,option_text,claim_text,body) VALUES (?, ?, ?, '', '', '')",
      ),
      applies = JSON.stringify({ modules: ["kernel"], productLines: [] }),
      proposer = JSON.stringify({ principal: { personId: "performance" }, executor: null });
    db.exec("BEGIN");
    for (let index = 0; index < 10_000; index += 1) {
      const id = `dec_PERF_${String(index).padStart(5, "0")}`,
        title = `Indexed Decision token${index}`,
        question = `Should token${index} ship?`;
      insert.run(id, title, question, applies, proposer, "2026-08-13T00:00:00.000Z", index + 1);
      insertFts.run(id, title, question);
    }
    db.exec("COMMIT");
    const plan = db
      .prepare("EXPLAIN QUERY PLAN SELECT decision_id FROM decision_fts WHERE decision_fts MATCH ?")
      .all('"token9999"') as unknown as readonly { readonly detail: string }[];
    assert.equal(
      plan.some(({ detail }) => /VIRTUAL TABLE INDEX/u.test(detail)),
      true,
      JSON.stringify(plan),
    );
    db.prepare("DELETE FROM decision_fts WHERE decision_id='dec_PERF_00000'").run();
    db.prepare(
