...
test("WriteCoordinator recovers queued journal entries after crash before watermark", () => {
  withTempStore((rootDir) => {
    ...
    assert.equal(existsSync(path.join(rootDir, ".journal/watermark.json")), false);
    ...
    assert.equal(readFileSync(path.join(rootDir, "tasks/task-1/progress.md"), "utf8"), "replayed");
  });
});
