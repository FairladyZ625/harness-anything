...
import { bundle, decisionProposal, event, git, initRepo } from "./task-event-store.fixtures.ts";
test("Decision bundle publishes one canonical document, enforces base CAS, and preserves a hand-edit conflict", async () => {
  await withTempStoreAsync(async (rootDir) => {
      ...
      invalidPlan = freezeDeclaredWritePlan(
        {
          ...
          targets: compiled.plan.targets.filter(
            (target) => target.kind !== "content_blob" && target.kind !== "local_wal_file",
          ),
        },
        ["DecisionWrite"],
      ),
      before = store.currentCommit();
    assert.throws(() => store.append({ ...compiled, plan: invalidPlan }), /decision write plan/u);
    ...
    assert.equal(git(rootDir, "show", `${receipt.commitSha.sha}:${documentTarget}`), compiled.body.trimEnd());
    assert.equal(git(rootDir, "show", `${receipt.commitSha.sha}:${objectTarget}`), compiled.body.trimEnd());
    ...
    assert.equal(readFileSync(path.join(rootDir, materialized.conflicts[0]!), "utf8"), local);
    assert.equal(readFileSync(path.join(rootDir, documentTarget), "utf8"), compiled.body);
    assert.equal(
      readdirSync(path.dirname(path.join(rootDir, documentTarget))).some((name) => name.includes(".conflict-")),
      true,
    );
    ...
    assert.equal(readFileSync(path.join(rootDir, documentTarget), "utf8"), accepted.body);
...
test("authored branch advancement outside the daemon fails closed", async () => {
  await withTempStoreAsync(async (rootDir) => {
    ...
    assert.throws(
      ...
      (error: unknown) => {
        assert.equal((error as { code?: string }).code, "publication_indeterminate");
        const text = String(error);
        assert.ok(text.includes(canonical), `recovery guidance must name the target commit: ${text}`);
        const branch = git(rootDir, "symbolic-ref", "HEAD");
        assert.ok(
          text.includes(`update-ref ${branch} ${canonical}`),
...
for (const killpoint of [
  ...
  test(`Decision authored recovery handles ${killpoint} without duplicate publication`, async () => {
    await withTempStoreAsync(async (rootDir) => {
      ...
      assert.throws(() => interrupted.append(decision), new RegExp(`crash:${killpoint}`, "u"));
      const recovery = makeTaskEventStore({
        repoId: "test-repo",
        rootDir,
      }).recover();
      if (killpoint === "after_head_write") assert.equal(recovery.status, "committed");
      else if (["after_git_commit", "before_worktree_rename", "after_worktree_rename"].includes(killpoint))
      ...
      assert.equal(git(rootDir, "rev-list", "--count", CANONICAL_EVENT_REF), "2");
      assert.equal(git(rootDir, "for-each-ref", "--format=%(refname)", "refs/ha-event-prepared/"), "");
    });
  });
}
...
for (const killpoint of [
  ...
  test(
    ...
    async () => {
      await withTempStoreAsync(async (rootDir) => {
        initRepo(rootDir);
        const moduleUrl = new URL("../../src/store/task-event-store.ts", import.meta.url).href,
          child = spawnSync(
            process.execPath,
            [
        ...
        assert.equal(resumed.read().revision, 1);
        assert.equal(git(rootDir, "for-each-ref", "--format=%(refname)", "refs/ha-event-prepared/"), "");
        assert.equal(git(rootDir, "rev-parse", CANONICAL_EVENT_REF), git(rootDir, "rev-parse", "HEAD"));
      });
    },
  );
}
