...
test("projection cache identity mismatch is explicit and rebuild remains available", async () => {
  await withTempStoreAsync(async (rootA) => {
    await withTempStoreAsync(async (rootB) => {
      ...
      const projectionBOnStaleCache = makeTaskProjection({
        rootDir: rootB,
        eventStore: storeB,
        projectionPath: projectionA.path,
      });
      assert.throws(
        () => projectionBOnStaleCache.readDocument("context/notes.md"),
      ...
      const projectionAOnStaleCache = makeTaskProjection({
        rootDir: rootA,
        eventStore: storeA,
        projectionPath: projectionA.path,
      });
      assert.throws(
        () => projectionAOnStaleCache.readDocument("context/notes.md"),
        /projection cache ledger identity mismatch/iu,
      );
...
function seedFactLedger(rootDir: string): CanonicalEventStore {
  ...
  git(rootDir, "config", "user.email", "identity-test@example.invalid");
  git(rootDir, "commit", "--allow-empty", "--quiet", "-m", "fixture base");
  const eventStore = makeTaskEventStore({ repoId: "relation-source", rootDir }),
    event: FactEventDraftV1 = {
      schema: "fact-event/v1",
      eventId: "event-relation-source",
      workspaceRevision: 1,
      opId: "op-relation-source",
    ...
    compiled = compileFactWrite({ event, packagePath: "tasks/task-identity-identity", currentFacts: [] });
  eventStore.append(compiled);
  return eventStore;
}
...
function seedTaskLedger(rootDir: string, name: string, count: number): CanonicalEventStore {
  ...
  for (const event of lifecycleFixture().events.slice(0, count))
