...
export async function runCommandThroughDaemon(command: ThinCommand, onPhase: (receipt: JsonObject) => void = () => undefined, options: { readonly autostart?: boolean } = {}): Promise<JsonObject> {
  ...
  let result = await withAutostart(() => requestLocalDaemonJsonRpcForTarget(target, command.method, { repo: { repoId: target.repoId }, payload: command.method === "repo.task.run" ? { action: command.action as JsonObject } : payload as JsonObject }, 75), () => cliDaemonServeLaunch(target.userRoot, target.daemonId), target.socketPath, autostart);
