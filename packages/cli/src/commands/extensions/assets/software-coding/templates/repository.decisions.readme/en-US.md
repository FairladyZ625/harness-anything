...
- **Do not hand-write or hand-edit the markdown here.** Decisions go through commands: `ha decision propose ...` to open, `ha decision decide ...` to adjudicate, `ha decision ...` to maintain state. The CLI owns the frontmatter, lifecycle state, fact evidence, and relation edges.
...
- ❌ Do not put: hand-written `.md`. Contents are derived by `ha decision`; manual edits drift from the projection.

## 相关命令 (Related commands)

`ha decision propose` · `ha decision decide` · `ha decision list` · `ha relation ...`
