...
- **不要手写/手改这里的 markdown**。决策走命令：`ha decision propose ...` 立项、`ha decision decide ...` 裁决、`ha decision ...` 维护状态。CLI 负责写 frontmatter、lifecycle state、fact 证据与 relation 边。
...
- ❌ 不放：手写的 `.md`。目录内容由 `ha decision` 命令派生，手改会与投影漂移。
