...
完成前必须写入实际复核结论；所需 review 未通过时，`ha task complete` 会拒绝该 Execution。Execution Review 产出 `approved`、`changes_requested` 或 `dismissed`，不是 decision 原语。若 review 暴露承重选路问题，再运行 `ha decision propose ...`（ADR-0027 D5）。
