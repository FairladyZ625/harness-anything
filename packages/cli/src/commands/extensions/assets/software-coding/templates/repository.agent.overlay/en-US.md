...
- Submission, review, and completion: submit the active Execution with `ha task submit <id> --from-file <submission.json>`. The packet records `completionClaim`, `deliverables`, `outputs`, `verificationNotes`, `knownGaps`, and `residualRisks`; text-only submission and zero Evidence are valid. A different reviewer then uses `ha task review-execution <id> --execution-id <exe-id> --verdict <approved|changes_requested|dismissed> --findings "..." --rationale "..."` and repeats `--evidence-checked` for inspected Evidence IDs. Run `ha task complete <id> --ci passed` only when the resolved profile declares the CI gate. Facts are optional `0..N` promotions, never a quantity gate (dec_mrg3z1we/CH1, CH4; ADR-0027 D3, D5-D7).
