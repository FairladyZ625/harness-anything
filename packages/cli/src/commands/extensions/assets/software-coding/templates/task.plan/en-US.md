...
- Start an Execution with `ha task start <task-id> --execution-id <execution-id> --json`; its lease binds the authenticated actor, execution id, and lease version. Record deliverables, evidence references, and verification when that Execution is submitted.
