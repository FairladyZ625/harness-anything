...
const docThinSyntax = new Set(["--submit", "--execution-id", "--base-ledger-sha", "--path", "--base-blob-sha256"]); export const thinCliLocalErrorCodes = Object.freeze(["daemon_disconnect", "duplicate_field", "invalid_field", "missing_field", "unknown_field", "unsupported_command"]);
...
export function parseThinCommand(argv: readonly string[], cwd = process.cwd()): ThinParseResult {
  ...
  /* Fact parser owns "--task", "--statement", "--source", "--observed-at", "--confidence", "--memory-class", "--memory-tag", "--supersedes", "--rationale", and "--id". */ if (route?.id.startsWith("doc-")) return parseDocThinCommand(route.id, args, rootDir, repoId, json, docThinSyntax); if (route?.id.startsWith("fact-")) return parseFactThinCommand(route.id, args, rootDir, repoId, json);
