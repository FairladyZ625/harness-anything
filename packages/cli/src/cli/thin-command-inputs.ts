...
import type { ProtocolCommand, ThinCliInput, ThinCliInputDirectory } from "./thin-command-types.ts";
...
export function deriveThinCliInputs(command: {
  ...
  if (!Array.isArray(command.inputs)) throw new Error(`${command.id} inputs must be an array`);
  const names = new Set<string>();
  for (const value of command.inputs) {
    if (!isInput(value) || names.has(value.name)) throw new Error(`${command.id} has an invalid input facet`);
    names.add(value.name);
    if (
      value.enum !== undefined &&
      (!Array.isArray(value.enum) || value.enum.length === 0 || value.enum.some((entry) => !nonEmpty(entry)))
    )
  ...
  if (!Array.isArray(command.flags) || JSON.stringify(command.flags) !== JSON.stringify(command.inputs))
...
export function deriveInputDirectory(command: ProtocolCommand | undefined): ThinCliInputDirectory {
  return command
    ? new Map([
        [
          command.id,
          {
            ...
            ...("actionDefaults" in command && command.actionDefaults
              ? { actionDefaults: command.actionDefaults }
              : {}),
            ...("actionConstraints" in command && command.actionConstraints
              ? { actionConstraints: command.actionConstraints }
              : {}),
          },
        ],
      ])
