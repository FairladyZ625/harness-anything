...
export function parseDocThinCommand(routeId: string, args: readonly string[], rootDir: SafePath, repoId: string | undefined, json: boolean): ThinParseResult {
  const offset = routeId === "doc-sync-submit" ? 3 : 2, parsed = flags(args.slice(offset));
...
function flags(tokens: readonly string[]): { readonly ok: true; readonly values: Map<string, string> } | { readonly ok: false; readonly code: string; readonly nextAction: string } {
  const values = new Map<string, string>(); for (let index = 0; index < tokens.length; index += 2) { const name = tokens[index], value = tokens[index + 1];
    if (!name?.startsWith("--")) return { ok: false, code: "unknown_field", nextAction: `Unknown option ${name ?? "<missing>"}.` };
