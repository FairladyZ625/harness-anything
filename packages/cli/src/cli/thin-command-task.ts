...
import {
  ...
  parseSupersede,
  parseTransition,
} from "./thin-command-task-lifecycle.ts";
import type {
...
export function parseTask(
  ...
  if (id === "task-contract-migrate")
    return parseContractMigrate(args, rootDir, repoId, json, inputs);
  if (id === "task-delete")
    return parseTaskDelete(args, rootDir, repoId, json, inputs);
  if (id === "task-archive")
    return parseTaskArchive(args, rootDir, repoId, json, inputs);
  ...
  if (id === "task-progress-append")
    return parseProgress(rootDir, repoId, json, args, inputs);
  ...
  if (id === "task-transition")
    return parseTransition(args, taskId, rootDir, repoId, json, inputs);
  if (id === "task-amend")
    return parseAmend(args, taskId, rootDir, repoId, json, inputs);
  if (id === "task-pin" || id === "task-unpin")
    return accepted(rootDir, repoId, json, {
      kind: "task-amend",
      taskId,
      patches: [
        { field: "pinned", value: id === "task-pin" ? "true" : "false" },
      ],
    });
  if (id === "task-supersede")
    return parseSupersede(args, taskId, rootDir, repoId, json, inputs);
  if (id === "task-relate")
    return parseRelate(args, taskId, rootDir, repoId, json, inputs);
  if (id === "task-complete")
    return parseComplete(rootDir, repoId, json, args, taskId, inputs);
  ...
  if (id === "task-review-execution" || id === "task-review-consent")
    return parseProjected(id, args.slice(3), rootDir, repoId, json, inputs, {
      taskId,
      commandType:
        id === "task-review-execution" ? "RecordReview" : "RecordReviewConsent",
    });
