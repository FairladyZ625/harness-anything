...
export const decisionsCommandSpecs = defineCommandSpecs([
  ...
  },
  {
    ...
    "usage": "decision propose --title <title> --question <text> --chosen <text|json>... --rejected <text|json>... [--why-not <fallback-for-text-rejections>] [--from-file <path>|--json-input <json>] [--risk-tier low|medium|high] [--urgency low|medium|high] [--module <key[,key]>] [--product-line <key[,key]>] [--claim <text>]... [--fulfillment <claim-id>:<mode>]... [--non-load-bearing] [--evidence-relation <anchor>:<type>:<task|fact-ref>:<rationale>] [--surface <token>]... [--body <text>|--body-file <path>] [--dry-run] [--json]",
    "options": [{"flag":"--title","description":"Set the required task title used for generated package metadata and slug."},{"flag":"--question","description":"Set the decision question being answered."},{"flag":"--chosen","description":"Add one independently answerable judgment (maximum 120 characters); repeat to split parallel judgments. Put reasons in the body and implementation in tasks. See harness/standards/decision-writing.md."},{"flag":"--rejected","description":"Add a rejected option; repeat independently and use JSON to carry its own why_not."},{"flag":"--why-not","description":"Set the fallback rationale applied to text-only rejected options."},{"flag":"--from-file","description":"Read command input JSON from a file; flags remain shortcut overrides."},{"flag":"--json-input","description":"Read command input JSON from an inline string; flags remain shortcut overrides."},{"flag":"--risk-tier","description":"Set decision risk tier: low, medium, or high."},{"flag":"--urgency","description":"Set decision urgency: low, medium, or high."},{"flag":"--module","description":"Select a registered module key; use module list to discover keys."},{"flag":"--product-line","description":"Attach a comma-separated product line list to a decision."},{"flag":"--claim","description":"Set the primary supporting claim text for a decision."},{"flag":"--fulfillment","description":"Declare one claim fulfillment mode as claim-id:evidenced, claim-id:delivered, or claim-id:standing-policy."},{"flag":"--non-load-bearing","description":"Mark the proposed primary claim, or an existing amended claim, as exempt from reckon coverage."},{"flag":"--evidence-relation","description":"Attach a decision anchor to a task, decision, or fact ref as anchor:type:target:rationale; repeat for multiple relations."},{"flag":"--surface","description":"Declare a command, flag, file path, or identifier to search in existing decisions; repeat as needed and use --surface=--flag for flag-shaped values."},{"flag":"--body","description":"Set authored body content for the generated decision document; mutually exclusive with --body-file."},{"flag":"--body-file","description":"Read authored body markdown from a file; mutually exclusive with --body."},{"flag":"--dry-run","description":"Preview the operation without writing changes."},{"flag":"--json","description":"Emit command-receipt/v2 JSON."}],
    "hiddenOptions": [{"flag":"--id","description":"Accept a legacy caller-selected Decision ID at the parser boundary so canonical ingress can reject it explicitly; new Decisions always receive generated IDs."}],
    "inlineValueOptions": ["--surface"],
    "summary": "Create a proposed decision with optional typed evidence relations through the decision write service.",
    "examples": ["harness-anything decision propose --title \"Adopt CLI decision loop\" --question \"Should M3 expose decision CLI?\" --chosen \"Expose it\" --rejected \"Keep write API only\" --why-not \"No human fallback path\" --surface long-running-task"],
    "parse": parseDecisionArgs,
    "run": runDecisionCommand,
    "receiptContract": {
      "data": ["decisionId", "decisionState", "report"],
  ...
  {
    "kind": "record-fact",
    "usage": "fact record --task <task-id> --statement <text> [--source <text>] [--from-file <path>|--json-input <json>] [--confidence low|medium|high] [--memory-class semantic|episodic|procedural] [--memory-tag <tag>] [--observed-at <iso>] [--dry-run] [--json]",
    "options": [{"flag":"--task","description":"Set the task id."},{"flag":"--statement","description":"Set the fact statement text."},{"flag":"--source","description":"Override the evidence source; defaults to the active execution or current session."},{"flag":"--from-file","description":"Read command input JSON from a file; flags remain shortcut overrides."},{"flag":"--json-input","description":"Read command input JSON from an inline string; flags remain shortcut overrides."},{"flag":"--confidence","description":"Set fact confidence as low, medium, or high."},{"flag":"--memory-class","description":"Classify fact memory as semantic, episodic, or procedural."},{"flag":"--memory-tag","description":"Attach a fact memory tag; repeat or comma-separate values."},{"flag":"--observed-at","description":"Set the observation timestamp for a fact record."},{"flag":"--dry-run","description":"Preview the operation without writing changes."},{"flag":"--json","description":"Emit command-receipt/v2 JSON."}],
    "hiddenOptions": [{"flag":"--id","description":"Accept a legacy caller-selected Fact ID at the parser boundary so canonical ingress can reject it explicitly; new Facts always receive generated IDs."}],
    ...
    "examples": ["harness-anything fact record --task task_01ABC --statement \"CLI fallback passed\" --confidence high"],
    "parse": parseRecordArgs,
    "run": runFactCommand,
    "receiptContract": {
      "data": ["taskId", "factId", "factRef", "report"],
