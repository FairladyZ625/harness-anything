...
export const coreCommandSpecs = defineCommandSpecs([
  ...
  {
    "kind": "new-task",
    "usage": "task create --title <title> [--id <task-id> --migration|--import|--admin] [--idempotency-key <key>] [--parent <task-id>] [--kind feat|fix|refactor|docs|test|chore] [--risk-tier low|medium|high] [--urgency low|medium|high] [--from-file <path>|--json-input <json>] [--vertical software/coding --preset <id> --profile <id> --module <key>] [--register-module <key> --module-title <title> --module-prefix <prefix> --module-scope <path>] [--slug <slug>] [--surface <token>]... [--long-running] [--dry-run] [--locale zh-CN|en-US] [--from-legacy <legacy-id>] [--json]",
    "options": [{"flag":"--title","description":"Set the required task title used for generated package metadata and slug."},{"flag":"--id","description":"Supply a compatibility task id only with --migration, --import, or --admin."},{"flag":"--idempotency-key","description":"Repeat a stable caller key to reuse the existing task package instead of creating another one."},{"flag":"--parent","description":"Bind a new task to an existing parent task id."},{"flag":"--kind","description":"Set task work kind: feat, fix, refactor, docs, test, or chore."},{"flag":"--risk-tier","description":"Set task risk tier: low, medium, or high."},{"flag":"--urgency","description":"Set task urgency: low, medium, or high."},{"flag":"--from-file","description":"Read command input JSON from a file; flags remain shortcut overrides."},{"flag":"--json-input","description":"Read command input JSON from an inline string; flags remain shortcut overrides."},{"flag":"--vertical","description":"Select a vertical definition; task create defaults to software/coding."},{"flag":"--preset","description":"Select a preset id; task create defaults to standard-task and preset list shows installed presets."},{"flag":"--profile","description":"Select the task profile within the chosen preset."},{"flag":"--module","description":"Select a registered module key; use module list to discover keys."},{"flag":"--register-module","description":"Register a module while creating the task."},{"flag":"--module-title","description":"Set the human-readable title for a registered module."},{"flag":"--module-prefix","description":"Set the generated task id prefix for a registered module."},{"flag":"--module-scope","description":"Set the registered module source scope, such as packages/name/**."},{"flag":"--slug","description":"Set the explicit task package slug."},{"flag":"--surface","description":"Declare a command, flag, file path, or identifier to search in existing decisions; repeat as needed and use --surface=--flag for flag-shaped values."},{"flag":"--long-running","description":"Mark the task as long-running."},{"flag":"--dry-run","description":"Preview the operation without writing changes."},{"flag":"--locale","description":"Set generated content locale."},{"flag":"--from-legacy","description":"Create from a legacy task id."},{"flag":"--migration","description":"Enable compatibility migration mode for an explicitly supplied task id."},{"flag":"--import","description":"Enable compatibility import mode for an explicitly supplied task id."},{"flag":"--admin","description":"Enable compatibility administration mode for an explicitly supplied task id."},{"flag":"--json","description":"Emit command-receipt/v2 JSON."}],
