...
test("lifecycle CLI maps submit, Review, consent, reconcile, and completion facade through closed typed inputs", () => {
  ...
  assert.equal(parseThinCommand(["task", "submit", "task-1", "--execution-id", "execution-1"]).ok, false); assert.equal(parseThinCommand(["task", "declare-executor", "task-1", "--execution-id", "execution-1"]).ok, false); assert.equal(parseThinCommand(["task", "review-execution", "task-1", "--execution-id", "execution-1", "--review-id", "review-1"]).ok, false); assert.equal(parseThinCommand(["task", "review-consent", "task-1", "--execution-id", "execution-1", "--review-id", "review-1"]).ok, false); assert.equal(parseThinCommand(["task", "code-doc", "reconcile", "task-1", "--execution-id", "execution-1", "--commit-sha", "short", "--iteration", "2", "--path", "a.ts"]).ok, false); assert.equal(parseThinCommand(["task", "complete", "task-1", "--execution-id", "execution-1", "--ci", "failed"]).ok, false); assert.equal(parseThinCommand(["task", "complete", "task-1", "--execution-id", "execution-1", "--commit-sha", "a".repeat(40)]).ok, false);
});
