...
test("capabilities is an exact-set projection of the command contract", () => {
  assert.deepEqual(deriveCliCapabilities(), {
    ...
    task: ["task-amend", "task-archive", "task-artifact-add", "task-closeout", "task-code-doc-reconcile", "task-complete", "task-contract-migrate", "task-create", "task-declare-executor", "task-delete", "task-dispatches", "task-list", "task-pin", "task-progress-append", "task-relate", "task-release", "task-reopen", "task-review", "task-review-consent", "task-review-execution", "task-show", "task-start", "task-submit", "task-supersede", "task-transition", "task-unpin"],
...
test("runtime work commands parse into closed daemon facade actions", () => {
  const run = parseThinCommand(["runtime", "run", "worker", "--agent", "fable", "--to", "terra", "--prompt", "Inspect", "--cwd", "packages/cli", "--task", "task-1", "--resume", "provider-1", "--idempotency-key", "once", "--no-stream"]), taskOnly = parseThinCommand(["runtime", "run", "worker", "--agent", "terra", "--task", "task-1", "--cwd", "."]), file = parseThinCommand(["runtime", "run", "worker", "--prompt-file", "prompt.txt"]), batch = parseThinCommand(["runtime", "batch", "dispatches.json"]), detached = parseThinCommand(["runtime", "run", "worker", "--prompt", "Inspect", "--detach", "--on-exit", "./notify.sh"]), resumed = parseThinCommand(["runtime", "run", "--resume-dispatch", "dispatch_0123456789abcdef01234567", "--prompt", "Continue"]), dispatches = parseThinCommand(["task", "dispatches", "task-1"]), list = parseThinCommand(["runtime", "status", "--task", "task-1"]), show = parseThinCommand(["runtime", "status", "runtime-1"]), wait = parseThinCommand(["runtime", "status", "runtime-1", "--wait", "--no-stream"]), cancel = parseThinCommand(["runtime", "cancel", "runtime-1"]);
  for (const parsed of [run, taskOnly, file, batch, detached, resumed, dispatches, list, show, wait, cancel]) assert.equal(parsed.ok, true, JSON.stringify(parsed));
  if (run.ok) assert.deepEqual(run.command.action, { kind: "runtime-run", runtimeInstanceId: "worker", agentId: "fable", targetAgentId: "terra", prompt: "Inspect", cwd: { scope: "repo-relative", path: "packages/cli" }, taskId: "task-1", providerSessionId: "provider-1", idempotencyKey: "once", noStream: true });
  if (taskOnly.ok) assert.deepEqual(taskOnly.command.action, { kind: "runtime-run", runtimeInstanceId: "worker", agentId: "terra", cwd: { scope: "repo-root" }, taskId: "task-1" });
  ...
  if (batch.ok) assert.deepEqual(batch.command.action, { kind: "runtime-batch", batchFile: "dispatches.json" });
  if (detached.ok) assert.deepEqual(detached.command.action, { kind: "runtime-run", runtimeInstanceId: "worker", prompt: "Inspect", cwd: { scope: "repo-root" }, taskId: null, detach: true, onExitCommand: "./notify.sh" });
  ...
  assert.equal(parseThinCommand(["runtime", "run", "worker"]).ok, false); assert.equal(parseThinCommand(["runtime", "batch"]).ok, false); assert.equal(parseThinCommand(["runtime", "batch", "dispatches.json", "--detach"]).ok, false); assert.equal(parseThinCommand(["runtime", "run", "worker", "--prompt", "Inspect", "--on-exit", "./notify.sh"]).ok, false); assert.equal(parseThinCommand(["runtime", "run", "worker", "--prompt", "one", "--prompt-file", "two"]).ok, false); assert.equal(parseThinCommand(["runtime", "run", "worker", "--task", "task-1", "--prompt", "one", "--prompt-file", "two"]).ok, false); assert.equal(parseThinCommand(["runtime", "run", "worker", "--to", "terra", "--prompt", "Inspect"]).ok, false); assert.equal(parseThinCommand(["runtime", "status", "runtime-1", "--task", "task-1"]).ok, false); assert.equal(parseThinCommand(["runtime", "status", "--wait"]).ok, false); assert.equal(parseThinCommand(["runtime", "status", "runtime-1", "--no-stream"]).ok, false); assert.equal(parseThinCommand(["runtime", "wait", "runtime-1"]).ok, false);
});

test("squad run derives the runtime prompt input union", () => {
  const promptFile = parseThinCommand(["squad", "run", "core-squad", "--instance", "worker", "--prompt-file", "mission.md", "--cwd", "work", "--task", "task-1"]), prompt = parseThinCommand(["squad", "run", "core-squad", "--instance", "worker", "--prompt", "mission", "--cwd", "work", "--task", "task-1"]), both = parseThinCommand(["squad", "run", "core-squad", "--instance", "worker", "--prompt", "mission", "--prompt-file", "mission.md", "--cwd", "work", "--task", "task-1"]), neither = parseThinCommand(["squad", "run", "core-squad", "--instance", "worker", "--cwd", "work", "--task", "task-1"]);
  ...
  assert.equal(neither.ok, false); if (!neither.ok) { assert.match(neither.nextAction, /exactly one/u); assert.doesNotMatch(neither.nextAction, /omit both/u); }
});
...
test("lifecycle CLI maps explicit selectors and accepts every derivable execution or Review selector", () => {
  const submit = parseThinCommand(["task", "submit", "task-1", "--execution-id", "execution-1", "--from-file", "submission.json"]), declare = parseThinCommand(["task", "declare-executor", "task-1", "--execution-id", "execution-1", "--reason", "Recover omitted executor attribution"]), review = parseThinCommand(["task", "review-execution", "task-1", "--execution-id", "execution-1", "--review-id", "review-1", "--from-file", "review.json"]), consent = parseThinCommand(["task", "review-consent", "task-1", "--execution-id", "execution-1", "--review-id", "review-1", "--consent-id", "consent-1", "--from-file", "consent.json"]), reconcile = parseThinCommand(["task", "code-doc", "reconcile", "task-1", "--execution-id", "execution-1", "--commit-sha", "a".repeat(40), "--iteration", "0", "--path", "packages/kernel/src/domain/task.ts"]), complete = parseThinCommand(["task", "complete", "task-1", "--execution-id", "execution-1", "--ci", "passed", "--path", "packages/kernel/src/domain/task.ts"]);
  for (const parsed of [submit, declare, review, consent, reconcile, complete]) assert.equal(parsed.ok, true, JSON.stringify(parsed));
  ...
  if (declare.ok) assert.deepEqual(declare.command.action, { kind: "task-declare-executor", taskId: "task-1", executionId: "execution-1", reason: "Recover omitted executor attribution" });
  ...
  if (complete.ok) assert.deepEqual(complete.command.action, { kind: "task-complete", verb: "complete", commandType: "CompleteTask", taskId: "task-1", executionId: "execution-1", ci: "passed", paths: ["packages/kernel/src/domain/task.ts"] });
  const derivedDeclare = parseThinCommand(["task", "declare-executor", "task-1", "--reason", "Recover omitted executor attribution"]), derivedSubmit = parseThinCommand(["task", "submit", "task-1", "--from-file", "submission.json"]), derivedReview = parseThinCommand(["task", "review-execution", "task-1", "--review-id", "review-1", "--from-file", "review.json"]), derivedPairConsent = parseThinCommand(["task", "review-consent", "task-1", "--consent-id", "consent-1"]), derivedComplete = parseThinCommand(["task", "complete", "task-1", "--ci", "passed", "--path", "packages/kernel/src/domain/task.ts"]);
  for (const parsed of [derivedDeclare, derivedSubmit, derivedReview, derivedPairConsent, derivedComplete]) assert.equal(parsed.ok, true, JSON.stringify(parsed));
  if (derivedDeclare.ok) assert.deepEqual(derivedDeclare.command.action, { kind: "task-declare-executor", taskId: "task-1", reason: "Recover omitted executor attribution" });
  if (derivedSubmit.ok) assert.deepEqual(derivedSubmit.command.action, { kind: "task-submit", verb: "submit", commandType: "SubmitExecution", taskId: "task-1", fromFile: "submission.json" });
  if (derivedReview.ok) assert.deepEqual(derivedReview.command.action, { kind: "task-review-execution", taskId: "task-1", reviewId: "review-1", commandType: "RecordReview", fromFile: "review.json" });
  if (derivedPairConsent.ok) assert.deepEqual(derivedPairConsent.command.action, { kind: "task-review-consent", taskId: "task-1", commandType: "RecordReviewConsent", consentId: "consent-1" });
  if (derivedComplete.ok) assert.deepEqual(derivedComplete.command.action, { kind: "task-complete", verb: "complete", commandType: "CompleteTask", taskId: "task-1", ci: "passed", paths: ["packages/kernel/src/domain/task.ts"] });
  assert.equal(parseThinCommand(["task", "submit", "task-1", "--execution-id", "execution-1"]).ok, false); assert.equal(parseThinCommand(["task", "declare-executor", "task-1", "--execution-id", "execution-1"]).ok, false); assert.equal(parseThinCommand(["task", "review-execution", "task-1", "--execution-id", "execution-1", "--review-id", "review-1"]).ok, false); assert.equal(parseThinCommand(["task", "review-consent", "task-1", "--execution-id", "execution-1", "--review-id", "review-1"]).ok, false); assert.equal(parseThinCommand(["task", "code-doc", "reconcile", "task-1", "--execution-id", "execution-1", "--commit-sha", "short", "--iteration", "2", "--path", "a.ts"]).ok, false); assert.equal(parseThinCommand(["task", "complete", "task-1", "--execution-id", "execution-1", "--ci", "failed"]).ok, false);
  const pathOnlyComplete = parseThinCommand(["task", "complete", "task-1", "--execution-id", "execution-1", "--path", "packages/kernel/src/domain/task.ts"]);
  assert.equal(pathOnlyComplete.ok, true);
  if (pathOnlyComplete.ok) assert.deepEqual(pathOnlyComplete.command.action, { kind: "task-complete", verb: "complete", commandType: "CompleteTask", taskId: "task-1", executionId: "execution-1", paths: ["packages/kernel/src/domain/task.ts"] });
  assert.equal(parseThinCommand(["task", "complete", "task-1", "--execution-id", "execution-1", "--commit-sha", "a".repeat(40)]).ok, false);
});
