...
test("CLI merges two independently initialized Git Harness repositories into a third center", (context) => {
  ...
  try {
    ...
    const store = makeTaskEventStore({ repoId: "center", rootDir: center }),
      ...
      firstDigest = digest(path.join(parent, "cold-one.sqlite")),
      secondDigest = digest(path.join(parent, "cold-two.sqlite"));
    assert.equal(firstDigest, secondDigest);
    context.diagnostic(
      `cli.outcome=${String(receipt.outcome)}\ncli.sources=2\ncenter.taskMigrationEvents=${events.length}\ncenter.idMaps=${maps.length}\ncenter.remapRecorded=true\ncold.digest.one=${firstDigest}\ncold.digest.two=${secondDigest}`,
    );
  } finally {
    stop(userRoot, center);
...
function addSourceData(root: string, label: string, personId: string): void {
  const authoredRoot = path.join(root, "harness"),
    taskRoot = path.join(authoredRoot, `tasks/task_shared-${label}`);
  mkdirSync(taskRoot, { recursive: true });
  writeFileSync(
    path.join(authoredRoot, "people.yaml"),
    `${JSON.stringify(
      {
  ...
  const gitRoot = git(authoredRoot, "rev-parse", "--show-toplevel");
  git(gitRoot, "config", "user.name", "CLI Migration Test");
  git(gitRoot, "config", "user.email", "cli-migration@example.invalid");
  git(gitRoot, "add", ".");
