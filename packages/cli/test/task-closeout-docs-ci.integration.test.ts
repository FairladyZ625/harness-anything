...
test("a real docs task with no declared ci gate closes out on not_applicable and refuses an invented passed", (context) => {
  ...
  try {
    ...
    run(root, userRoot, ["daemon", "repo", "register", "--repo-id", "closeout-docs-ci", "--root", root, "--no-link"]);
    writeFileSync(
    ...
    assert.equal(standardPreview.status, 0, standardPreview.stderr || standardPreview.stdout);
    assert.match(
    ...
    assert.equal(
      run(root, userRoot, [
        ...
        taskId,
        "--statement",
        "The docs closeout was reviewed without a declared CI gate.",
        "--source",
        "test:task-closeout-docs-ci",
      ]).outcome,
      "applied",
    );
    ...
    const invented = runMaybe(root, userRoot, ["task", "closeout", taskId, "--from-file", "invented.json"]);
    ...
    assert.equal(
      (
        JSON.parse(
          String(
            (JSON.parse(runMaybe(root, userRoot, ["task", "show", taskId]).stdout) as Record<string, unknown>).evidence,
          ),
