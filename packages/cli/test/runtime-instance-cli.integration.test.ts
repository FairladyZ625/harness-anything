...
test("real CLI performs machine runtime instance CRUD through an isolated resident daemon", () => {
  ...
  try {
    ...
    const created = run(root, env, ["runtime", "instance", "create", "--id", "cli-isolated", "--name", "CLI Isolated", "--kind", installation.kindId, "--provider", "codex_local_access", "--model", "runtime-test-model", "--effort", "high", "--fast", "--base-url", "http://127.0.0.1:1/v1", "--wire-api", "responses", "--requires-openai-auth", "--http-header", "X-Harness-Probe=present", "--auth", "api-key", "--credential-ref", "keychain:harness/cli-isolated"]);
    ...
    assert.deepEqual(shown.instance, created.instance); assert.deepEqual((shown.instance as Record<string, unknown>).codex, { reasoningEffort: "high", fast: true, baseUrl: "http://127.0.0.1:1/v1", baseUrlConfigured: true, wire_api: "responses", requires_openai_auth: true, http_headers: { "X-Harness-Probe": "present" } }); assert.equal((listed.instances as Array<Record<string, unknown>>).length, 1);
