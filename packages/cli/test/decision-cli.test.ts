...
test("CLI decision accept blocks zero-evidence decisions without judgment-only", () => {
  withTempRoot((rootDir) => {
    runJson(rootDir, [
      ...
      "Decision CLI",
      "--question",
      "Should CLI accept decisions without evidence?",
      "--chosen",
      "Accept via CLI",
      "--rejected",
      "Leave proposed",
      "--why-not",
      "The acceptance path needs a non-empty floor"
    ]);
...
test("CLI decision accept transitions an existing decision with evidence through the coordinator", () => {
  withTempRoot((rootDir) => {
    ...
    runJson(rootDir, [
      ...
      "Leave proposed",
      "--why-not",
      "The acceptance path needs evidence",
      "--evidence-relation",
      `C1:supports:task/${task.taskId}:Task evidence establishes a non-empty acceptance floor`
    ]);
...
test("CLI decision accept records judgment-only rationale", () => {
  withTempRoot((rootDir) => {
    runJson(rootDir, [
      ...
      "dec_JUDGMENTONLY",
      "--title",
      "Judgment-only Decision",
      "--question",
      "Can the arbiter accept without evidence?",
      "--chosen",
      "Accept as judgment-only",
      "--rejected",
      "Require evidence",
      "--why-not",
      "The arbiter explicitly records the escape hatch"
    ]);
    ...
    const result = runJson(rootDir, [
      ...
      "human:ZeyuLi",
      "--judgment-only",
      "Human arbiter accepts this policy bet before facts exist."
    ]);
...
test("CLI decision reckon fails closed on uncovered load-bearing claims", () => {
  withTempRoot((rootDir) => {
    ...
    runJson(rootDir, [
      ...
      "Reckon Failure",
      "--question",
      "Should uncovered claims pass?",
      "--chosen",
      "Require coverage at reckon",
      "--rejected",
      "Let uncovered claims pass",
      "--why-not",
      "Reckon is the coverage gate",
      "--evidence-relation",
      `C1:supports:task/${task.taskId}:Task evidence only satisfies accept floor`
    ]);
...
test("CLI decision reckon passes covered load-bearing claims and records a fact", () => {
  withTempRoot((rootDir) => {
    ...
    runJson(rootDir, [
      ...
      "F-C0VERED1",
      "--statement",
      "The implementation covers the load-bearing claim.",
      "--source",
      "test",
      "--confidence",
      "high"
    ...
    runJson(rootDir, [
      ...
      "Reckon Pass",
      "--question",
      "Should covered claims pass?",
      "--chosen",
      "Pass only after coverage",
      "--rejected",
      "Ignore coverage",
      "--why-not",
      "Reckon is mechanical",
      "--evidence-relation",
      `C1:supports:fact/${task.taskId}/F-C0VERED1:Fact covers claim C1`
    ]);
...
test("CLI decision reckon ignores non-load-bearing uncovered claims", () => {
  withTempRoot((rootDir) => {
    ...
    runJson(rootDir, [
      ...
      "Reckon Ignore",
      "--question",
      "Should non-load-bearing claims block?",
      "--chosen",
      "Ignore non-load-bearing claims",
      "--rejected",
      "Block every claim",
      "--why-not",
      "Only load-bearing claims are the reckon gate",
      "--non-load-bearing",
      "--evidence-relation",
      `C1:supports:task/${task.taskId}:Task evidence only satisfies accept floor`
    ]);
