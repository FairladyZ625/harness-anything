...
test("explicit daemon bootstraps an empty workspace before its first lifecycle write", () => {
  ...
  try {
    ...
    assert.deepEqual(initialized.created, ["harness/harness.yaml", "harness/people.yaml", "harness/context/README.md", "harness/context/architecture/README.md", "harness/context/development/README.md", "harness/context/integrations/README.md", "harness/context/research/README.md"]); assert.deepEqual(initialized.preserved, []); assert.deepEqual(initialized.drifted, []);
    const plan = initialized.plan as { digest: string; baseScaffoldDigest: string; projectOverlayPath: string | null; projectOverlayDigest: string | null; documents: Array<{ path: string; contentSha256: string; disposition: string }> }; assert.match(plan.digest, /^sha256:[0-9a-f]{64}$/u); assert.match(plan.baseScaffoldDigest, /^sha256:[0-9a-f]{64}$/u); assert.equal(plan.projectOverlayPath, null); assert.equal(plan.projectOverlayDigest, null); assert.deepEqual(plan.documents.map(({ disposition }) => disposition), Array(5).fill("created")); assert.equal((initialized.publication as { ok: boolean }).ok, true);
    for (const target of initialized.created as string[]) assert.equal(existsSync(path.join(fixture.repo, target)), true, target); assert.match(readFileSync(path.join(fixture.repo, "harness/harness.yaml"), "utf8"), /scaffolds:\n    task: governance\/task-scaffold\.json\n    repository: governance\/repository-scaffold\.json/u); for (const target of ["harness/adr", "harness/milestones", "harness/governance/standards", "harness/governance/walls", "AGENTS.md", "CLAUDE.md"]) assert.equal(existsSync(path.join(fixture.repo, target)), false, target);
    assert.equal(git(fixture.repo, "rev-list", "--count", "HEAD"), "1");
    const repeated = run(fixture.repo, fixture.userRoot, ["init", "--repo-id", "fresh", "--person-id", "owner", "--display-name", "Owner"]); assert.equal(repeated.outcome, "noop"); assert.equal(repeated.commit, null); assert.deepEqual(repeated.created, []); assert.deepEqual(repeated.preserved, initialized.created); assert.equal(git(fixture.repo, "rev-list", "--count", "HEAD"), "1"); const textReceipt = spawnSync(process.execPath, [cli, "--root", fixture.repo, "init", "--repo-id", "fresh", "--person-id", "owner", "--display-name", "Owner"], { encoding: "utf8", env: { ...process.env, HOME: path.join(fixture.repo, ".home"), GIT_CONFIG_GLOBAL: "/dev/null", HARNESS_DAEMON_USER_ROOT: fixture.userRoot } }); assert.equal(textReceipt.status, 0, textReceipt.stderr); assert.match(textReceipt.stdout, /init: noop\ncreated: \[\]\npreserved: \["harness\/harness.yaml"/u); assert.match(textReceipt.stdout, /drifted: \[\]\ncommit: none\nnext: Create a task/u);
