...
test("real CLI reaches one resident multi-workspace daemon and publishes Git event -> SQLite -> receipt", async () => {
  ...
  try {
    ...
    const decision = JSON.parse(String(decisionPropose.evidence)) as { decisionId: string; state: string }, decisionPath = `decisions/decision-${decision.decisionId}/decision.md`, beforeAccepted = makeTaskEventStore({ rootDir: fixture.alpha, repoId: "alpha" }).readHead()!.revision; assert.equal(decision.state, "proposed"); assert.equal(decisionPropose.path, decisionPath); assert.equal(decisionPropose.worktreeVisible, true); assert.equal(decisionPropose.commitSha, git(fixture.alpha, "rev-parse", "HEAD")); assert.match(String(decisionPropose.documentSha256), /^[0-9a-f]{64}$/u); assert.match(readFileSync(path.join(fixture.alpha, "harness", decisionPath), "utf8"), /^---\nschema: decision-package\/v1[\s\S]*\nstate: proposed[\s\S]*\n---\n\n# Canonical Decision from CLI\n$/u);
...
test("local init isolates the ledger from later project commits and removes tracked runtime paths", (context) => {
  ...
  try {
    ...
    const written = run(fixture.repo, fixture.userRoot, ["task", "create", "--id", "task-after-project-commit", "--admin", "--title", "Still writable"]), ledgerAfter = git(ledgerRoot, "rev-parse", "HEAD"); assert.equal(written.outcome, "applied", JSON.stringify(written)); assert.equal(git(fixture.repo, "rev-parse", "HEAD"), projectHead); assert.notEqual(ledgerAfter, ledgerHead); assert.equal(git(ledgerRoot, "rev-parse", "refs/ha/canonical"), ledgerAfter); assert.equal(spawnSync("git", ["-C", fixture.repo, "rev-parse", "--verify", "refs/ha/canonical"], { encoding: "utf8" }).status, 128); context.diagnostic(`ledger.git=true\nouter.check-ignore harness=${harnessIgnored}\nouter.check-ignore .harness=${runtimeIgnored}\nouter.ls-files harness/=${harnessTracked}\nouter.ls-files .harness/=${runtimeTracked}\nproject.head.after=${projectHead}\nledger.head.before=${ledgerHead}\nledger.head.after=${ledgerAfter}\nwrite.outcome=${String(written.outcome)}`);
  } finally { stop(fixture.repo, fixture.userRoot); rmSync(fixture.root, { recursive: true, force: true }); }
});
...
test("center registration keeps an external ledger repository readable and writable", (context) => {
  const fixture = setup();
  try {
    assert.equal(existsSync(path.join(fixture.alpha, "harness/.git")), false); assert.equal(run(fixture.alpha, fixture.userRoot, ["daemon", "start", "--service"]).ok, true); register(fixture.alpha, fixture.userRoot, "center"); const before = git(fixture.alpha, "rev-parse", "HEAD"), written = run(fixture.alpha, fixture.userRoot, ["task", "create", "--id", "task-center", "--admin", "--title", "Center ledger"]), after = git(fixture.alpha, "rev-parse", "HEAD"); assert.equal(written.outcome, "applied", JSON.stringify(written)); assert.notEqual(after, before); assert.equal(git(fixture.alpha, "rev-parse", "refs/ha/canonical"), after); assert.equal(git(fixture.alpha, "ls-tree", "-r", "--name-only", "HEAD").split("\n").some((target) => target.startsWith("harness/events/")), true); assert.match(String(run(fixture.alpha, fixture.userRoot, ["task", "show", "task-center"]).evidence), /Center ledger/u); context.diagnostic(`ledger.git=${fixture.alpha}\nledger.head.before=${before}\nledger.head.after=${after}\nledger.canonical=${git(fixture.alpha, "rev-parse", "refs/ha/canonical")}\nwrite.outcome=${String(written.outcome)}\nread.task=Center ledger`);
  } finally { stop(fixture.alpha, fixture.userRoot); rmSync(fixture.root, { recursive: true, force: true }); }
});
