...
test("real CLI reaches one resident multi-workspace daemon and publishes Git event -> SQLite -> receipt", async () => {
  ...
  try {
    ...
    const decision = JSON.parse(String(decisionPropose.evidence)) as { decisionId: string; state: string }, decisionPath = `decisions/decision-${decision.decisionId}/decision.md`, beforeRejected = makeTaskEventStore({ rootDir: fixture.alpha, repoId: "alpha" }).readHead()!.revision; assert.equal(decision.state, "proposed"); assert.equal(decisionPropose.path, decisionPath); assert.equal(decisionPropose.worktreeVisible, true); assert.equal(decisionPropose.commitSha, git(fixture.alpha, "rev-parse", "HEAD")); assert.match(String(decisionPropose.documentSha256), /^[0-9a-f]{64}$/u); assert.match(readFileSync(path.join(fixture.alpha, "harness", decisionPath), "utf8"), /^---\nschema: decision-package\/v1[\s\S]*\nstate: proposed[\s\S]*\n---\n\n# Canonical Decision from CLI\n$/u);
