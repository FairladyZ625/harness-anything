...
test("an owner break-glasses a gate with no automated receipt; a later receipt voids the waiver", async (context) => {
  ...
  try {
    ...
    const afterWaiver = taskSnapshot(run(root, userRoot, daemonId, ["task", "show", taskId])),
      waiver = afterWaiver.gateWitnesses.find((witness) => witness.gateId === "ci" && witness.override);
    assert.equal(waiver?.override?.waivedReceiptId, null);
    assert.equal(waiver?.provenance?.source, "human");
    ...
    assert.ok(
      afterRed.gateWitnesses.some((witness) => witness.gateId === "ci" && witness.override?.waivedReceiptId === null),
    );
    ...
    assert.ok(
      afterReWaiver.gateWitnesses.some(
        (witness) =>
          witness.gateId === "ci" &&
          typeof witness.override?.waivedReceiptId === "string" &&
          witness.override.waivedReceiptId.length > 0,
      ),
    );
