...
test("real CLI signs in interactively, runs real provider work, reauths, signs out, and rejects mode mismatches", async () => {
  ...
  try {
    ...
    const reauth = await runInteractive(root, env, ["runtime", "instance", "reauth", "cli-worker", "--idempotency-key", "reauth-once"], "second-operator-token\n"); assert.equal(reauth.status, 0, reauth.stderr); assert.equal(JSON.parse(readFileSync(authFile, "utf8")).token, "second-operator-token");
    const second = run(root, env, ["runtime", "run", "cli-worker", "--prompt", "after reauth", "--no-stream"]); assert.equal((second.result as Record<string, unknown>).text, "final:after reauth");
