...
test("real CLI signs in interactively, refreshes through login, runs real provider work, signs out, and rejects mode mismatches", async () => {
  ...
  const home = path.join(parent, "home"), env = { ...process.env, HOME: home, USERPROFILE: home, CODEX_HOME: undefined, PATH: `${binRoot}${path.delimiter}${process.env.PATH ?? ""}`, HARNESS_DAEMON_USER_ROOT: userRoot, HARNESS_DAEMON_ID: "runtime-auth-cli-test" }, authFile = path.join(userRoot, "runtime-instances", "cli-worker", "home", ".codex", "auth.json");
  try {
    ...
    const blocked = runMaybe(root, env, ["runtime", "run", "cli-worker", "--prompt", "too early", "--no-stream"]); assert.notEqual(blocked.status, 0, JSON.stringify(blocked)); assert.equal((blocked.receipt.error as Record<string, unknown>).code, "runtime_subscription_required");
