...
test("task create preserves the complete contract and initial relations in one closed action", () => {
  ...
  assert.equal(
    parseThinCommand(["task", "create", "--from-legacy", "legacy-1"]).ok,
    true,
  );
...
test("long-running work arrives only as --task-class long_running; the retired boolean flag is unknown", () => {
  const resident = parseThinCommand([
    "task",
    "create",
    "--title",
    "Resident ledger",
...
test("task lifecycle and read surfaces parse every F03 F04 F05 leaf into closed actions", () => {
  const cases = [
    ...
    [
      [
        ...
        "planned",
        "--reason",
        "Owner rolled back the batch cancellation",
      ],
      {
        kind: "task-transition",
        taskId: "task-1",
    ...
    [
      ["agenda", "--limit", "25", "--cursor", "cursor-a"],
      { kind: "agenda", limit: 25, cursor: "cursor-a" },
    ],
    [
      [
        "task",
  ...
  for (const [argv, expected] of cases) {
    ...
    assert.equal(
      parsed.ok,
      true,
      `${argv.join(" ")}: ${JSON.stringify(parsed)}`,
    );
  ...
  assert.equal(
    parseThinCommand(["task", "transition", "task-1", "done"]).ok,
    false,
  );
  const bareReinstate = parseThinCommand([
    "task",
    "transition",
    "task-1",
    "planned",
  ...
  const cancelReasonOnly = parseThinCommand([
    "task",
    "transition",
    "task-1",
    "cancelled",
  ...
  const cancelForceOnly = parseThinCommand([
    "task",
    "transition",
    "task-1",
    "cancelled",
  ...
  const cancelBare = parseThinCommand([
    "task",
    "transition",
    "task-1",
    "cancelled",
  ...
  const forceOutsideCancel = parseThinCommand([
    "task",
    "transition",
    "task-1",
    "active",
  ...
  if (!forceOutsideCancel.ok)
    assert.doesNotMatch(forceOutsideCancel.nextAction, /add --reason/u);
  assert.equal(
    parseThinCommand([
      "task",
      "delete",
  ...
  assert.equal(
    parseThinCommand(["task", "contract", "migrate", "--apply", "--dry-run"])
      .ok,
    false,
  );
  assert.equal(
    parseThinCommand(["task", "supersede", "task-1", "--by", "task-2"]).ok,
    false,
  );
  assert.equal(
    parseThinCommand([
