...
const parseCases: ReadonlyArray<ParseCase> = [
  ...
  { name: "task complete", argv: ["task", "complete", "task_1", "--ci", "passed", "--reviewer", "alice"], kind: "task-complete", fields: { taskId: "task_1", ciGate: "passed", reviewerId: "alice", evidenceMode: "execution-review" } },
  ...
  { name: "relation list", argv: ["relation", "list", "--entity", "task/task_1", "--source", "task/task_1", "--target", "task/task_2", "--type", "depends-on", "--state", "active"], kind: "relation-list", fields: { filters: { entity: "task/task_1", source: "task/task_1", target: "task/task_2", type: "depends-on", state: "active" } } },
  {
    ...
    argv: ["decision", "propose", "--title", "Decision", "--question", "Question?", "--chosen", "Chosen", "--rejected", "Rejected", "--why-not", "Because", "--risk-tier", "high", "--urgency", "medium", "--module", "kernel,cli", "--non-load-bearing", "--dry-run"],
    kind: "decision-propose",
    fields: { title: "Decision", question: "Question?", chosen: [{ id: "CH1", text: "Chosen" }], rejected: [{ id: "RJ1", text: "Rejected", why_not: "Because" }], claims: [{ id: "C1", text: "Chosen", load_bearing: false }], riskTier: "high", urgency: "medium", modules: ["kernel", "cli"], claimLoadBearing: false, dryRun: true }
  },
  ...
  { name: "decision amend", argv: ["decision", "amend", "dec_TEST", "--title", "Updated", "--standing-policy", "--non-load-bearing", "C2", "--append", "rejected:{\"text\":\"Manual mapping\",\"why_not\":\"Coverage gate required\"}", "--body", "## Background\n\nUpdated prose."], kind: "decision-amend", fields: { decisionId: "dec_TEST", title: "Updated", standingPolicy: true, body: "## Background\n\nUpdated prose.", patches: [{ field: "claims", operation: "metadata", value: "{\"id\":\"C2\",\"load_bearing\":false}" }, { field: "rejected", operation: "append", value: "{\"text\":\"Manual mapping\",\"why_not\":\"Coverage gate required\"}" }] } },
  ...
  { name: "fact record", argv: ["fact", "record", "--task", "task_1", "--statement", "Fact", "--source", "Fixture", "--confidence", "high", "--memory-class", "procedural", "--memory-tag", "tool_memory,task_skill", "--observed-at", "2026-07-03T00:00:00.000Z"], kind: "record-fact", fields: { taskId: "task_1", statement: "Fact", source: "Fixture", confidence: "high", memoryClass: "procedural", memoryTags: ["tool_memory", "task_skill"], observedAt: "2026-07-03T00:00:00.000Z" } },
...
test("parseArgs pins stable parse error envelopes", () => {
  const cases = [
    ...
    { argv: ["decision", "amend", "dec_TEST", "--set", "body:replacement"], code: "invalid_decision_amend_patch" },
    { argv: ["decision", "amend", "dec_TEST", "--append", "body:\"addendum\""], code: "invalid_decision_amend_patch" },
    { argv: ["decision", "amend", "dec_TEST", "--set", "chosen:replacement"], code: "invalid_decision_amend_patch" },
