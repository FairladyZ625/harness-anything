...
export function prepareBuiltinVerticalScriptExecution(input: { readonly rootDir: string; readonly action: unknown; readonly commitSha: string }): PreparedBuiltinVerticalScript {
  ...
  const command = path.resolve(assetsRoot, declaration.command); if (!inside(assetsRoot, command) || !regularContainedFile(command)) throw new BuiltinVerticalScriptError("script_command_invalid", `Declared script command is unavailable: ${declaration.command}.`);
  let outputRoot = layout.contextRoot; if (action.taskId !== null) { outputRoot = layout.taskPackagePath(action.taskId as Parameters<typeof layout.taskPackagePath>[0]); if (!existsSync(path.join(outputRoot, "INDEX.md")) || !lstatSync(outputRoot).isDirectory() || lstatSync(outputRoot).isSymbolicLink() || !inside(realpathSync(layout.authoredRoot), realpathSync(outputRoot))) throw new BuiltinVerticalScriptError("task_not_found", `Task ${action.taskId} is unavailable.`); }
...
function regularContainedFile(target: string): boolean { try { return lstatSync(target).isFile() && inside(realpathSync(assetsRoot), realpathSync(target)); } catch { return false; } }
function inside(root: string, target: string): boolean { const relative = path.relative(path.resolve(root), path.resolve(target)); return relative === "" || relative !== ".." && !relative.startsWith(`..${path.sep}`) && !path.isAbsolute(relative); }
...
function authoredPattern(authoredRoot: string, target: string): string { const base = globBase(target); if (!inside(authoredRoot, base)) throw new BuiltinVerticalScriptError("script_scope_violation", `Declared write is outside the authored root: ${target}.`); return path.relative(authoredRoot, target).split(path.sep).join("/"); }
