...
test("start durably returns admitted before spawn and status exposes every daemon-owned phase", async () => {
  ...
  try {
    ...
    assert.deepEqual(terminal.phases, [
      "admitted",
      "spawned",
      "running",
      "publishing",
...
test("an inherited entrypoint stages its declaring package and detects post-admission changes", async () => {
  ...
  try {
    ...
    assert.deepEqual(applied.phases, [
      "admitted",
      "spawned",
      "running",
      "publishing",
    ...
    write(
      path.join(fixture.parentObject, "scripts/check.mjs"),
      "process.exit(0);",
    );
  ...
  try {
    ...
    assert.equal(
      [entrypoint, badInput, capability, shadow].every(
        ({ outcome, phases }) =>
          outcome === "op_rejected" && phases.join() === "op_rejected",
      ),
      true,
    );
  } finally {
...
test("idempotency never respawns and restart recovery stays outcome_unknown without retry", async () => {
  ...
  try {
    ...
    assert.equal(
      unknown.phases.filter((phase) => phase === "spawned").length,
      1,
    );
...
test("permission staging denies authored, Git, and raw paths while declared produces use only the admitted command", async () => {
  ...
  try {
    ...
    assert.deepEqual(published, [
      { kind: "task-create", title: "Produced", taskId: "task-produced" },
    ]);
  } finally {
    await service.close();
    producer.cleanup();
...
function inheritedScriptPackage() {
  ...
  write(
    path.join(leaf, "preset.json"),
    JSON.stringify(manifest("leaf-canary", { extends: "base-canary" })),
  );
  write(
...
async function waitFor<T>(
  ...
  throw new Error(
    `Timed out waiting for ${description}; last observed: ${JSON.stringify(last!)}`,
  );
}
