...
test("timeout forcibly reaps a child that ignores SIGTERM before publishing terminal receipt", async () => {
  ...
  try {
    const started = await service.start({ presetId: "user-canary", entrypoint: "check", inputs: { title: "Canary" }, idempotencyKey: "ignore-term-timeout" }), staging = path.join(fixture.rootDir, ".harness/preset-runs/staging", started.runId), pidPath = path.join(staging, "child.pid"), termPath = path.join(staging, "term.seen"); await waitFor(() => existsSync(pidPath), Boolean); pid = Number(readFileSync(pidPath, "utf8")); await waitFor(() => existsSync(termPath), Boolean); assert.doesNotThrow(() => process.kill(pid!, 0)); assert.equal(terminalOutcome(service.status(started.runId).outcome), false); const terminal = await waitFor(() => service.status(started.runId), ({ outcome }) => outcome === "failed"); assert.equal(terminal.code, "timeout"); assert.throws(() => process.kill(pid!, 0), { code: "ESRCH" });
...
test("close forcibly reaps a child that ignores SIGTERM within a fixed bound", async () => {
  ...
  try {
    const started = await service.start({ presetId: "user-canary", entrypoint: "check", inputs: { title: "Canary" }, idempotencyKey: "ignore-term-close" }), staging = path.join(fixture.rootDir, ".harness/preset-runs/staging", started.runId), pidPath = path.join(staging, "child.pid"), termPath = path.join(staging, "term.seen"); await waitFor(() => existsSync(pidPath), Boolean); pid = Number(readFileSync(pidPath, "utf8")); const before = performance.now(), closing = service.close(); await waitFor(() => existsSync(termPath), Boolean); assert.doesNotThrow(() => process.kill(pid!, 0)); assert.equal(terminalOutcome(service.status(started.runId).outcome), false); await closing; assert.equal(performance.now() - before < 1_000, true); assert.equal(service.status(started.runId).outcome, "outcome_unknown"); assert.throws(() => process.kill(pid!, 0), { code: "ESRCH" });
  } finally { if (pid) try { process.kill(pid, "SIGKILL"); } catch { /* already reaped */ } await service.close(); fixture.cleanup(); }
});
