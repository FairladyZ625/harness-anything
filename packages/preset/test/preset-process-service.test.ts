...
test("start durably returns admitted before spawn and status exposes every daemon-owned phase", async () => {
  const fixture = scriptedPackage(`console.log(JSON.stringify({ schema: "preset-script-result/v1", produces: [] }));`),
  ...
  try {
    ...
    assert.deepEqual(service.status(started.runId).phases, ["admitted"]);
    const terminal = await waitFor(
      "preset run to reach applied",
      () => service.status(started.runId),
      ({ outcome }) => outcome === "applied",
    );
    assert.equal(terminal.outcome, "applied");
    assert.deepEqual(terminal.phases, ["admitted", "spawned", "running", "publishing", "applied"]);
    assert.match(terminal.resultDigest ?? "", /^sha256:[0-9a-f]{64}$/u);
  } finally {
    await service.close();
    fixture.cleanup();
  }
...
test("admission rejects missing entrypoint, bad input, missing capability, and invalid user shadow within one second", async () => {
  const valid = scriptedPackage(`console.log(JSON.stringify({ schema: "preset-script-result/v1", produces: [] }));`),
    missing = scriptedPackage("", {
      requires: [{ id: "cap:missing/v1", kind: "command", version: "1" }],
    });
  const service = createPresetProcessService({
      rootDir: valid.rootDir,
      userRoot: valid.userRoot,
      publish: async () => {
        throw new Error("not spawned");
      },
    }),
    missingService = createPresetProcessService({
      rootDir: missing.rootDir,
      userRoot: missing.userRoot,
      publish: async () => {
        throw new Error("not spawned");
      },
    });
  try {
    const started = performance.now(),
      entrypoint = await service.start({
        presetId: "user-canary",
        entrypoint: "absent",
        inputs: { title: "Canary" },
        idempotencyKey: "missing-entry",
      }),
      badInput = await service.start({
        presetId: "user-canary",
        entrypoint: "check",
        inputs: {},
        idempotencyKey: "bad-input",
      }),
      capability = await missingService.start({
        presetId: "user-canary",
        entrypoint: "check",
        inputs: { title: "Canary" },
    ...
    write(
      path.join(valid.userRoot, "active/user-canary.json"),
      JSON.stringify({
    ...
    const shadow = await service.start({
      presetId: "user-canary",
      entrypoint: "check",
      inputs: { title: "Canary" },
      idempotencyKey: "invalid-shadow",
    });
    assert.equal(performance.now() - started < 1_000, true);
    assert.deepEqual(
      [entrypoint.code, badInput.code, capability.code, shadow.code],
      ["entrypoint_not_found", "invalid_input", "missing_provider", "shadow_invalid"],
    );
    assert.equal(
      [entrypoint, badInput, capability, shadow].every(
        ({ outcome, phases }) => outcome === "op_rejected" && phases.join() === "op_rejected",
      ),
      true,
    );
  } finally {
...
test("bounded child protocol classifies every failure without a silent phase", async () => {
  ...
  const cases = [
    ["never-output", "setInterval(() => {}, 1_000);", "timeout"],
    ["nonzero", "process.exit(7);", "child_exit"],
    ["signal", 'process.kill(process.pid, "SIGTERM");', process.platform === "win32" ? "child_exit" : "child_signal"],
    ["malformed", 'console.log("{");', "malformed_result"],
    ["oversize", 'process.stdout.write("x".repeat(200));', "result_oversize"],
    ["disconnect", "process.stdout.end(); setInterval(() => {}, 1_000);", "child_disconnect"],
  ] as const;
  for (const [name, script, code] of cases) {
    const fixture = scriptedPackage(script),
      service = createPresetProcessService({
        rootDir: fixture.rootDir,
        userRoot: fixture.userRoot,
        timeoutMs: 300,
    ...
    try {
      ...
      const result = await waitFor(
        `${name} child protocol failure`,
        () => service.status(started.runId),
        ({ outcome }) => outcome === "failed",
      );
      assert.equal(result.code, code, JSON.stringify(result));
      assert.equal(result.phases[0], "admitted");
      assert.equal(result.phases.at(-1), "failed");
    } finally {
      await service.close();
      fixture.cleanup();
    }
...
test("timeout forcibly reaps a child that ignores SIGTERM before publishing terminal receipt", async () => {
  ...
  try {
    const started = await service.start({
        ...
        entrypoint: "check",
        inputs: { title: "Canary" },
        idempotencyKey: "ignore-term-timeout",
      }),
      staging = path.join(fixture.rootDir, ".harness/preset-runs/staging", started.runId),
...
test("close forcibly reaps a child that ignores SIGTERM within a fixed bound", async () => {
  ...
  try {
    const started = await service.start({
        ...
        entrypoint: "check",
        inputs: { title: "Canary" },
        idempotencyKey: "ignore-term-close",
      }),
      staging = path.join(fixture.rootDir, ".harness/preset-runs/staging", started.runId),
      pidPath = path.join(staging, "child.pid"),
      termPath = path.join(staging, "term.seen"),
    ...
    await closing;
    assert.equal(performance.now() - before < 1_000, true);
    assert.equal(service.status(started.runId).outcome, "outcome_unknown");
    await assertReaped(pid!);
  } finally {
    if (pid)
      try {
...
test("idempotency never respawns and restart recovery stays outcome_unknown without retry", async () => {
  const fixture = scriptedPackage("setInterval(() => {}, 1_000);"),
    first = createPresetProcessService({
      rootDir: fixture.rootDir,
      userRoot: fixture.userRoot,
      timeoutMs: 5_000,
      publish: async () => ({ outcome: "applied" }),
    });
  let recovered: ReturnType<typeof createPresetProcessService> | undefined;
  try {
    ...
    await waitFor(
      "first preset run to reach running",
      () => first.status(started.runId),
      ({ phase }) => phase === "running",
    );
    recovered = createPresetProcessService({
      rootDir: fixture.rootDir,
      userRoot: fixture.userRoot,
      timeoutMs: 50,
      publish: async () => {
    ...
    const unknown = recovered.status(started.runId),
      replay = await recovered.start({
        presetId: "user-canary",
        entrypoint: "check",
        inputs: { title: "Canary" },
        idempotencyKey: "crash-once",
    ...
    assert.equal(unknown.phases.filter((phase) => phase === "spawned").length, 1);
    assert.equal(unknown.phases.at(-1), "outcome_unknown");
  } finally {
...
test("permission staging denies authored, Git, and raw paths while declared produces use only the admitted command", async () => {
  const writer = scriptedPackage(
    'const { target } = JSON.parse(process.env.HA_PRESET_INPUT); (await import("node:fs")).writeFileSync(target, "forbidden"); console.log(JSON.stringify({ schema: "preset-script-result/v1", produces: [] }));',
    { inputs: [{ name: "target", type: "string", required: true }] },
  );
  const targets = [
      path.join(writer.rootDir, "authored.txt"),
      path.join(writer.rootDir, ".git/config-owned"),
      path.join(tmpdir(), `ha-raw-${path.basename(writer.rootDir)}`),
    ],
    denied = createPresetProcessService({
      rootDir: writer.rootDir,
      userRoot: writer.userRoot,
      timeoutMs: 1_000,
      publish: async () => {
        throw new Error("not reached");
      },
  ...
  try {
    for (const [index, target] of targets.entries()) {
      const started = await denied.start({
          presetId: "user-canary",
          entrypoint: "check",
          inputs: { target },
        ...
        result = await waitFor(
          `permission denial for ${target}`,
          () => denied.status(started.runId),
          ({ outcome }) => outcome === "failed",
        );
      assert.equal(result.code, "child_exit");
  ...
  const producer = scriptedPackage(
      'console.log(JSON.stringify({ schema: "preset-script-result/v1", produces: [{ capabilityId: "policy:task-create/v1", payload: { title: "Produced", taskId: "task-produced" } }] }));',
      {
        produces: [{ id: "policy:task-create/v1", kind: "command", version: "1" }],
      },
    ),
    published: Array<Record<string, unknown>> = [],
    service = createPresetProcessService({
      rootDir: producer.rootDir,
      userRoot: producer.userRoot,
      timeoutMs: 1_000,
      admitProduce: (kind) => kind === "task-create",
  ...
  try {
    const started = await service.start({
        ...
        entrypoint: "check",
        inputs: { title: "Canary" },
        idempotencyKey: "produce",
      }),
      result = await waitFor(
        "produced preset run to reach applied",
        () => service.status(started.runId),
        ({ outcome }) => outcome === "applied",
      );
    assert.equal(result.outcome, "applied");
    assert.deepEqual(published, [{ kind: "task-create", title: "Produced", taskId: "task-produced" }]);
  } finally {
    await service.close();
    producer.cleanup();
  }
...
function scriptedPackage(script: string, entrypoint: Record<string, unknown> = {}) {
  const rootDir = mkdtempSync(path.join(tmpdir(), "ha-preset-process-")),
    userRoot = path.join(rootDir, ".harness/presets"),
    source = path.join(rootDir, "source/user-canary");
  write(
  ...
  installPresetPackage({ source, userRoot });
  return {
    rootDir,
    userRoot,
...
// the parent's event loop reaps it. Wait for that event instead of assuming the
// service's grace window always outlasts it.
async function assertReaped(pid: number): Promise<void> {
  await waitFor(
    `child ${pid} to be reaped (kill(pid, 0) reports ESRCH)`,
    () => {
      try {
        process.kill(pid, 0);
      } catch (error) {
...
function observeFileAppearance(target: string): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const watcher = watch(root, { recursive: true }, (_event, filename) => {
      if (
        !settled &&
        ((filename !== null && path.normalize(String(filename)) === relativeTarget) || existsSync(target))
      ) {
        settled = true;
        watcher.close();
        resolve();
