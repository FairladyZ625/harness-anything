...
test("bounded child protocol classifies every failure without a silent phase", async () => {
  ...
  const cases = [
    ["never-output", "setInterval(() => {}, 1_000);", "timeout"], ["nonzero", "process.exit(7);", "child_exit"], ["signal", 'process.kill(process.pid, "SIGTERM");', process.platform === "win32" ? "child_exit" : "child_signal"], ["malformed", 'console.log("{");', "malformed_result"], ["oversize", 'process.stdout.write("x".repeat(200));', "result_oversize"], ["disconnect", "(await import('node:fs')).closeSync(1); setInterval(() => {}, 1_000);", "child_disconnect"]
  ] as const;
...
test("timeout forcibly reaps a child that ignores SIGTERM before publishing terminal receipt", async () => {
  const fixture = scriptedPackage('const { writeFileSync } = await import("node:fs"); process.on("SIGTERM", () => writeFileSync("term.seen", "yes")); writeFileSync("child.pid", String(process.pid)); setInterval(() => {}, 1_000);'), service = createPresetProcessService({ rootDir: fixture.rootDir, userRoot: fixture.userRoot, timeoutMs: 500, publish: async () => { throw new Error("not reached"); } }); let pid: number | undefined;
  try {
    const started = await service.start({ presetId: "user-canary", entrypoint: "check", inputs: { title: "Canary" }, idempotencyKey: "ignore-term-timeout" }), staging = path.join(fixture.rootDir, ".harness/preset-runs/staging", started.runId), pidPath = path.join(staging, "child.pid"), termPath = path.join(staging, "term.seen"); await waitFor(() => existsSync(pidPath), Boolean); pid = Number(readFileSync(pidPath, "utf8")); if (process.platform !== "win32") await waitFor(() => existsSync(termPath), Boolean); // On Windows kill(SIGTERM) terminates unconditionally, so the handler phase cannot be witnessed; the reaped end state is asserted instead.
    assert.doesNotThrow(() => process.kill(pid!, 0)); assert.equal(terminalOutcome(service.status(started.runId).outcome), false); const terminal = await waitFor(() => service.status(started.runId), ({ outcome }) => outcome === "failed"); assert.equal(terminal.code, "timeout"); assert.throws(() => process.kill(pid!, 0), { code: "ESRCH" });
...
test("close forcibly reaps a child that ignores SIGTERM within a fixed bound", async () => {
  ...
  try {
    ...
    else { await waitFor(() => existsSync(termPath), Boolean); assert.doesNotThrow(() => process.kill(pid!, 0)); assert.equal(terminalOutcome(service.status(started.runId).outcome), false); }
