...
test("task create materializes preset scripts into artifacts/scripts as byte-identical regular files", () => {
  ...
  try {
    ...
    const bootstrap = compileTaskBootstrap({ bundledRoot: fixture.bundledRoot, userRoot: fixture.userRoot, assetsRoot: fixture.assetsRoot, verticalId: "software/coding", profileId: "baseline", locale: "en-US", actor, source: "local", occurredAt: "2026-08-19T00:00:00.000Z", taskId: "task-scripts", title: "Scripts", presetId: "standard-task", workspaceRevision: 1, eventId: "event-scripts", opId: "op-scripts" });
    const store = makeTaskEventStore({ repoId: "preset-scripts-e2e", rootDir }); store.append({ event: bootstrap.event, plan: bootstrap.plan, blobs: bootstrap.blobs });
    const packageDir = path.join(rootDir, "harness", bootstrap.packagePath), scriptsDir = path.join(packageDir, "artifacts", "scripts");
    ...
    for (const name of ["check-env.mjs", "sum.mjs"]) {
      const target = path.join(scriptsDir, name), status = lstatSync(target);
  ...
  } finally { rmSync(rootDir, { recursive: true, force: true }); fixture.cleanup(); }
});

test("a preset without scripts leaves the task package unchanged and creates no artifacts/scripts directory", () => {
  const fixture = makeFixture(), rootDir = mkdtempSync(path.join(tmpdir(), "ha-preset-scripts-none-"));
  try {
    git(rootDir, "init", "-q"); git(rootDir, "config", "user.name", "Preset Test"); git(rootDir, "config", "user.email", "preset@example.invalid"); git(rootDir, "commit", "--allow-empty", "-qm", "base");
    const bootstrap = compileTaskBootstrap({ bundledRoot: fixture.bundledRoot, userRoot: fixture.userRoot, assetsRoot: fixture.assetsRoot, verticalId: "software/coding", profileId: "baseline", locale: "en-US", actor, source: "local", occurredAt: "2026-08-19T00:00:00.000Z", taskId: "task-plain", title: "Plain", presetId: "standard-task", workspaceRevision: 1, eventId: "event-plain", opId: "op-plain" });
    assert.equal(bootstrap.documents.some(({ relativePath }) => relativePath.startsWith("artifacts/scripts/")), false);
    assert.equal(bootstrap.documents.length, 6);
    const store = makeTaskEventStore({ repoId: "preset-scripts-none", rootDir }); store.append({ event: bootstrap.event, plan: bootstrap.plan, blobs: bootstrap.blobs });
...
test("irregular scripts nodes fail closed instead of widening copy semantics", () => {
  const nested = makeFixture({ "top.mjs": "console.log(\"top\");\n" }), flat = makeFixture(), linked = makeFixture({ "real.mjs": "console.log(\"real\");\n" });
  try {
    write(path.join(nested.presetRoot, "scripts/nested/deep.mjs"), "console.log(\"deep\");\n");
    assert.throws(() => compileTaskBootstrap({ bundledRoot: nested.bundledRoot, userRoot: nested.userRoot, assetsRoot: nested.assetsRoot, verticalId: "software/coding", profileId: "baseline", locale: "en-US", actor, source: "local", occurredAt: "2026-08-19T00:00:00.000Z", taskId: "task-nested", title: "Nested", presetId: "standard-task", workspaceRevision: 1, eventId: "event-nested", opId: "op-nested" }), (error: unknown) => (error as { code?: string }).code === "invalid_preset_scripts");
    write(path.join(flat.presetRoot, "scripts"), "not a directory");
    assert.throws(() => compileTaskBootstrap({ bundledRoot: flat.bundledRoot, userRoot: flat.userRoot, assetsRoot: flat.assetsRoot, verticalId: "software/coding", profileId: "baseline", locale: "en-US", actor, source: "local", occurredAt: "2026-08-19T00:00:00.000Z", taskId: "task-flat", title: "Flat", presetId: "standard-task", workspaceRevision: 1, eventId: "event-flat", opId: "op-flat" }), (error: unknown) => (error as { code?: string }).code === "invalid_preset_scripts");
    write(path.join(path.dirname(linked.presetRoot), "outside.mjs"), "console.log(\"outside\");\n"); symlinkSync(path.join(path.dirname(linked.presetRoot), "outside.mjs"), path.join(linked.presetRoot, "scripts/link.mjs"));
    assert.throws(() => compileTaskBootstrap({ bundledRoot: linked.bundledRoot, userRoot: linked.userRoot, assetsRoot: linked.assetsRoot, verticalId: "software/coding", profileId: "baseline", locale: "en-US", actor, source: "local", occurredAt: "2026-08-19T00:00:00.000Z", taskId: "task-linked", title: "Linked", presetId: "standard-task", workspaceRevision: 1, eventId: "event-linked", opId: "op-linked" }), (error: unknown) => (error as { code?: string }).code === "symlink_forbidden");
  } finally { nested.cleanup(); flat.cleanup(); linked.cleanup(); }
});

function makeFixture(scripts: Readonly<Record<string, string>> = {}) {
  const root = mkdtempSync(path.join(tmpdir(), "ha-preset-scripts-")), bundledRoot = path.join(root, "bundled"), userRoot = path.join(root, "user"), assetsRoot = path.join(root, "assets"), presetRoot = path.join(bundledRoot, "standard-task");
  ...
  return { bundledRoot, userRoot, assetsRoot, presetRoot, cleanup: () => rmSync(root, { recursive: true, force: true }) };
}
