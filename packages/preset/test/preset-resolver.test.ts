...
import { eventObjectTarget, makeTaskEventStore, makeTaskProjection, serializeCanonicalEvent, serializeEventHead, sha256Text, type TaskEventV1 } from "../../kernel/src/index.ts";
...
test("snapshot upgrade atomically replaces the complete snapshot and typed task contract without touching task prose", () => {
  ...
  try {
    ...
    const bootstrap = compileTaskBootstrap({ userRoot, verticalId: "software/coding", profileId: "baseline", locale: "en-US", actor: { principal: { personId: "person-1" }, executor: null }, source: "local", occurredAt: "2026-08-14T00:00:00.000Z", taskId, title: "Stable package", slug: "stable-package", presetId: "upgrade-task", workspaceRevision: 1, eventId: "event-upgrade-create", opId: "op-upgrade-create" }), historical: TaskEventV1 = { schema: "task-event/v1", eventId: "event-upgrade-create", workspaceRevision: 1, opId: "op-upgrade-create", taskId, type: "task_created", actor: bootstrap.event.actor, source: bootstrap.event.source, occurredAt: bootstrap.event.occurredAt, payload: { task: { ...bootstrap.event.payload.task, metadata: { ...bootstrap.metadata, longRunning: false } } as typeof bootstrap.event.payload.task, documentClaims: [] } }, eventBody = serializeCanonicalEvent(historical), eventPath = path.join(rootDir, eventObjectTarget(historical.opId)); mkdirSync(path.dirname(eventPath), { recursive: true }); writeFileSync(eventPath, eventBody); writeFileSync(path.join(rootDir, "harness/events/head.json"), serializeEventHead({ revision: 1, opId: historical.opId, eventDigest: `sha256:${sha256Text(eventBody)}` })); git(rootDir, "add", "harness/events"); git(rootDir, "commit", "-qm", "historical task"); const store = makeTaskEventStore({ repoId: "preset-upgrade", rootDir }); projection = makeTaskProjection({ rootDir, eventStore: store }); projection.rebuild();
    const planPath = path.join(rootDir, "harness", bootstrap.packagePath, "task_plan.md"), editedPlan = "# User plan\n\nKeep this prose.\n"; mkdirSync(path.dirname(planPath), { recursive: true }); writeFileSync(planPath, editedPlan); writePackage(sourceRoot, "upgrade-task", { version: "3.2.0" }); installPresetPackage({ source: path.join(sourceRoot, "upgrade-task"), userRoot }); const task = projection.read(taskId).snapshot.task!, contractPath = `${bootstrap.packagePath}/task-contract.json`, contract = { body: bootstrap.documents.find(({ relativePath }) => relativePath === "task-contract.json")!.body };
    let upgraded: ReturnType<typeof compilePresetSnapshotUpgrade> | undefined; assert.doesNotThrow(() => { upgraded = compilePresetSnapshotUpgrade({ userRoot, task, taskContractBody: contract.body, actor: { principal: { personId: "person-1" }, executor: null }, source: "local", workspaceRevision: 2, eventId: "event-upgrade", opId: "op-upgrade", occurredAt: "2026-08-14T00:01:00.000Z" }); }, "a persisted package slug must not be reported as a changed document set"); assert.ok(upgraded); assert.notEqual(upgraded.snapshot.digest, task.presetSnapshotDigest); assert.equal(upgraded.snapshot.identity.version, "3.2.0"); assert.equal(upgraded.event.payload.taskContractClaim.path, contractPath); assert.equal(Object.hasOwn(upgraded.event.payload.task.metadata ?? {}, "longRunning"), false); assert.equal(JSON.parse(upgraded.blobs.find(({ sha256 }) => sha256 === upgraded!.event.payload.taskContractClaim.sha256)!.body).packagePath, bootstrap.packagePath); store.append(upgraded); projection.apply(upgraded.event, upgraded.plan);
