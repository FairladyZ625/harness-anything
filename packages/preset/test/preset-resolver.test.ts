...
test("generic list, inspect, check, install, and uninstall actions share the canonical inventory", async () => {
  ...
  try {
    const listed = await runPresetAction({ rootDir, action: { kind: "preset-list" } }) as Array<{ id: string; title: string; description: string; verticalId: string; layer: string; source: string; validity: string; version?: string; kind?: string; defaultProfile?: string; entrypoints?: string[]; issues: unknown[]; issueCount?: number }>; assert.equal(listed.length, 11); const standardRow = listed.find(({ id }) => id === "standard-task")!; assert.deepEqual({ ...standardRow, source: path.basename(standardRow.source) }, { id: "standard-task", title: "Standard Task", description: "Create the standard planning, facts, and closeout scaffold for general software work.", verticalId: "software/coding", layer: "bundled", source: "standard-task", validity: "valid", version: "3.0.0", kind: "template-content", defaultProfile: "baseline", entrypoints: [], issues: [], issueCount: 0 });
...
test("user shadow lifecycle dry-runs mutation, blocks invalid content, and reveals bundled on uninstall", async () => {
  ...
  try {
    ...
    const audited = await runPresetAction({ rootDir, action: { kind: "preset-audit" } }) as { blocked: number; issues: Array<{ presetId: string; code: string; source: string }> }; assert.equal(audited.blocked, 1); assert.deepEqual(audited.issues.find(({ presetId }) => presetId === "standard-task"), { presetId: "standard-task", source: path.join(userRoot, "preset-objects", validated.preset!.digest), code: "shadow_invalid", message: "preset manifest fields are incomplete or unknown; bundled standard-task remains blocked." });
...
test("seed and audit dry-runs report the two-layer inventory without mutation", async () => {
  ...
  try {
    const audit = await runPresetAction({ rootDir, action: { kind: "preset-audit" } }) as { schema: string; total: number; valid: number; unavailable: number; blocked: number; issues: Array<{ presetId: string; code: string }> }; assert.deepEqual({ schema: audit.schema, total: audit.total, valid: audit.valid, unavailable: audit.unavailable, blocked: audit.blocked, issues: audit.issues.map(({ presetId, code }) => ({ presetId, code })) }, { schema: "preset-audit-report/v1", total: 11, valid: 11, unavailable: 0, blocked: 0, issues: [] });
    const drySeed = await runPresetAction({ rootDir, action: { kind: "preset-seed", dryRun: true } }) as { schema: string; mode: string; packageCount: number; packages: Array<{ presetId: string }> }; assert.equal(drySeed.schema, "preset-seed-report/v1"); assert.equal(drySeed.mode, "dry-run"); assert.equal(drySeed.packageCount, 11); assert.deepEqual(drySeed.packages.map(({ presetId }) => presetId), ["architecture-rot-audit", "code-impact-analysis", "create-milestone", "decision-conformance", "docs-task", "github-issue-repair", "milestone-closeout", "module", "standard-task", "subtask-expansion", "worker-dispatch"]); assert.equal(existsSync(userRoot), false);
    const seeded = await runPresetAction({ rootDir, action: { kind: "preset-seed" } }) as { mode: string; packageCount: number }; assert.deepEqual({ mode: seeded.mode, packageCount: seeded.packageCount }, { mode: "apply", packageCount: 11 }); assert.equal(readdirSync(path.join(userRoot, "active")).length, 11);
  } finally { rmSync(rootDir, { recursive: true, force: true }); }
});
