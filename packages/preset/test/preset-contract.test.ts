...
test("CLI help derives length only from a full single-atom quantifier", () => {
  assert.deepEqual(regexLength("^[\\s\\S]{1,199}$"), [1, 199]);
  assert.deepEqual(regexLength("^\\d{4}$"), [4, 4]);
  assert.deepEqual(regexLength("^.{2,5}$"), [2, 5]);
  assert.equal(parameterRelationHint("Use exactly one of --dry-run or --apply."), true);
  for (const regex of [
    "^[0-9]{4}-[0-9]{2}$",
    "^F-[0-9A-HJKMNP-TV-Z]{8}$",
    "^[a-z0-9](?:[a-z0-9-]{0,70}[a-z0-9])?$",
    "^sha256:[0-9a-f]{64}$",
  ])
    assert.equal(regexLength(regex), undefined, regex);
});
...
test("preset v3 rejects identity kinds and identity fields now that Agent and Squad are standalone entities", () => {
  ...
  assert.match(
    validatePresetManifestV3({ ...task, kind: "agent" }).join("\n"),
    /kind.*must be one of template-content, process-action/u,
  );
  assert.match(
    validatePresetManifestV3({ ...task, kind: "squad" }).join("\n"),
    /kind.*must be one of template-content, process-action/u,
  );
  assert.match(
    validatePresetManifestV3({
  ...
  assert.match(
    validatePresetManifestV3({
      ...task,
      squad: { id: "core-squad", name: "Core Squad", leader: "terra", workers: ["terra"], roster: "Terra leads." },
    }).join("\n"),
...
test("preset snapshot codec rejects nested field deletion and unknown aliases", () => {
  const hash = "a".repeat(64),
    snapshot = {
      ...
      identity: { id: "standard-task", version: "3.0.0", verticalId: "software/coding", layer: "bundled" },
      profile: { id: "baseline", outputShape: "repository-diff", completionGateIds: ["ci"] },
      guidance: { description: "Standard", whenToUse: "Ordinary work", bodySha256: hash },
      scaffold: { baseVersion: "software-coding/v1", overlayDigest: null, resolvedSelectionDigest: `sha256:${hash}` },
      templates: [
        {
          slot: "task.plan",
          path: "task_plan.md",
          ...
          content: { sha256: hash, size: 1, mediaType: "text/markdown" },
        },
      ],
      entrypoints: {},
      provenance: {
        ...
        packageSha256: hash,
        verticalSha256: hash,
        templateCatalogSha256: hash,
        resolverVersion: "1",
        ancestry: ["standard-task"],
      },
      digest: `sha256:${hash}`,
    };
  assert.deepEqual(validatePresetSnapshotV1(snapshot), []);
  assert.match(
    validatePresetSnapshotV1({ ...snapshot, profile: { id: "baseline", completionGateIds: ["ci"] } }).join("\n"),
    /invalid/u,
  );
  assert.match(
    validatePresetSnapshotV1({ ...snapshot, provenance: { ...snapshot.provenance, manifestDigest: hash } }).join("\n"),
    /invalid/u,
  );
});
...
test("script run declares one closed start route and one read-only status route", () => {
  ...
  assert.deepEqual(
    presetContract.methods
      .filter(({ method }) => method.startsWith("repo.preset.run."))
      .map(({ method, commandClass, params }) => ({
        method,
        commandClass,
        fields: Object.keys(params.fields.payload.fields),
      })),
    [
      {
        method: "repo.preset.run.start",
        commandClass: "repo-write",
        fields: ["presetId", "entrypoint", "taskId", "inputs", "idempotencyKey", "executor"],
      },
      { method: "repo.preset.run.status", commandClass: "repo-read", fields: ["runId", "executor"] },
    ],
  );
  for (const id of ["script-run", "preset-run-start"]) {
    const inputs = presetContract.commands.find((candidate) => candidate.id === id)?.inputs ?? [];
    assert.equal(
      inputs.some(({ name }) => name === "--task"),
      true,
      id,
    );
    assert.equal(
      inputs.some(({ name }) => name === "--task-id"),
      false,
      id,
    );
  }
...
test("every preset method accepts the caller's executor declaration", () => {
  ...
  assert.deepEqual(
    presetContract.methods
      .filter(({ params }) => params.fields.payload.fields.executor !== "json?")
      .map(({ method }) => method),
    [],
  );
});
...
test("builtin vertical and template discovery stay read-only while vertical run is one typed write route", () => {
  assert.deepEqual(
    presetContract.commands
      .filter(({ id }) =>
        ["vertical-validate", "template-list", "template-render", "script-list", "script-inspect"].includes(id),
      )
      .map(({ id, path, method, commandClass }) => ({ id, path, method, commandClass })),
    [
      {
  ...
  const run = presetContract.methods.find(({ actionKind }) => actionKind === "script-run");
  assert.deepEqual(
    run && {
      method: run.method,
      commandClass: run.commandClass,
      fields: Object.keys(run.params.fields.payload.fields),
    },
    {
      method: "repo.script.run",
      commandClass: "repo-write",
      fields: ["scriptId", "taskId", "inputs", "dryRun", "executor"],
    },
  );
});
...
test("preset failures reach the daemon boundary as readable text and survive report serialization", () => {
  const thrown = (() => {
    try {
      decodePresetPackageV3("/nonexistent-preset-package");
      return null;
    } catch (error) {
      return error;
    }
