...
test("standard and milestone bootstrap compile one exact canonical birth and rebuild from L1", () => {
  ...
  try {
    ...
    assert.equal(standard.event.payload.task.taskClass, "standard"); assert.equal(standard.event.payload.initialDocumentClaims.length, 4); assert.deepEqual(standard.documents.map(({ path: target }) => target), ["tasks/task-standard/task_plan.md", "tasks/task-standard/facts.md", "tasks/task-standard/closeout.md", "tasks/task-standard/artifacts/.gitkeep"]); assert.match(standard.documents[0]!.body, /^# Standard$/mu);
