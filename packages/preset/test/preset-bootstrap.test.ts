...
test("standard and milestone bootstrap compile one exact canonical birth and rebuild from L1", () => {
  ...
  try {
    ...
    const receipt = store.append({ event: standard.event, plan: standard.plan, blobs: standard.blobs }); assert.equal(receipt.revision, 1); assert.equal(store.read().events.length, 1); assert.equal(git(rootDir, "rev-list", "--count", "refs/ha/canonical"), "2"); projection.apply(standard.event, standard.plan); assert.deepEqual(projection.read("task-standard").snapshot.task, standard.event.payload.task); assert.equal(projection.read("task-standard").packagePath, standard.packagePath); assert.deepEqual(projection.readPresetSnapshot(standard.snapshot.digest).snapshot, standard.snapshot); for (const document of standard.documents) assert.equal(projection.readDocument(document.path).document?.body, document.body);
    rmSync(path.join(rootDir, "harness", standard.packagePath), { recursive: true, force: true }); const restored = store.materialize(); assert.deepEqual(restored.changed, [...standard.documents.map(({ path: target }) => target)].sort((left, right) => left.localeCompare(right))); for (const document of standard.documents) assert.equal(readFileSync(path.join(rootDir, "harness", document.path), "utf8"), document.body);
    const repeated = compileTaskBootstrap({ ...common, taskId: "task-repeated", title: "Repeated", presetId: "standard-task", workspaceRevision: 2, eventId: "event-repeated", opId: "op-repeated" }); assert.equal(repeated.snapshot.digest, standard.snapshot.digest); store.append({ event: repeated.event, plan: repeated.plan, blobs: repeated.blobs }); projection.apply(repeated.event, repeated.plan); assert.deepEqual(projection.read("task-repeated").snapshot.task, repeated.event.payload.task);
