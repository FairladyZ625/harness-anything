...
- **The minimum targeted set is fixed, not self-selected**: (1) `npm test -- --tier fast --tier contract` in full (two minutes in CI; cross-package contract consumers live in these two tiers); (2) the integration test files of the surface you touched; (3) `git grep` every error code, message text and protocol field name you changed, find the tests in other packages that assert it, and run them green. On 2026-09-04 four PRs went red on their first CI run because (3) was skipped — the worker ran the touched files while the consumers lived in another package.
