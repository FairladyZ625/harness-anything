...
test("a flat ledger keeps working end to end, advertises migration, and migrates with a cold restart", async () => {
  const rootDir = mkdtempSync(path.join(tmpdir(), "ha-ledger-flat-consistency-"));
  let cell: Awaited<ReturnType<typeof openRepoCell>> | undefined;
  try {
    const flatEvent = flatTaskEvent(1, "task_flat_one");
    initFlatLedger(rootDir, [flatEvent]);
    cell = await openRepoCell({
      repoId: workspaceId("ledger-flat-consistency"),
      rootDir: canonicalRoot(rootDir),
      ownerId: "ledger-flat-consistency",
      now: () => "2026-08-18T00:00:01.000Z",
    });
    const listed = await cell.read("repo.tasks.list");
    assert.equal(listed.status, "ready");
    assert.equal(
      listed.rows.some(({ taskId }) => taskId === flatEvent.taskId),
      true,
    );
    const archived = (await cell.run(
      { kind: "task-archive", taskId: flatEvent.taskId, reason: "flat layout drill" },
      { actor, source: "local" },
    ...
    assert.deepEqual(
      listTree(rootDir, "harness/events").filter((name) => /^[0-9a-f]{2}$/u.test(name)),
      [],
    );
    assert.equal(listTree(rootDir, "harness/events").filter((name) => name.endsWith(".json")).length, 2);
    const migrated = (await cell.run({ kind: "ledger-migrate" }, { actor, source: "local" })) as Record<
      string,
      unknown
    >;
    assert.equal(migrated.outcome, "applied", JSON.stringify(migrated));
    assert.deepEqual(
      listTree(rootDir, "harness/events").filter((name) => name.endsWith(".json")),
      ["head.json"],
    );
    await cell.close();
    cell = undefined;
    const restarted = await openRepoCell({
      repoId: workspaceId("ledger-flat-consistency"),
      rootDir: canonicalRoot(rootDir),
      ownerId: "ledger-flat-consistency",
    ...
    assert.equal(
      projected.rows.some(({ taskId }) => taskId === flatEvent.taskId),
      true,
    );
  } finally {
    await cell?.close();
...
test("a mixed ledger is repaired by ha migrate ledger, replays idempotently, and reattaches cold", async () => {
  const rootDir = mkdtempSync(path.join(tmpdir(), "ha-ledger-mixed-consistency-"));
  let cell: Awaited<ReturnType<typeof openRepoCell>> | undefined;
  try {
    const flatEvents = [flatTaskEvent(1, "task_mixed_one"), flatTaskEvent(2, "task_mixed_two")],
      shardedEvent = flatTaskEvent(3, "task_mixed_three");
    initMixedLedger(rootDir, flatEvents, shardedEvent);
    cell = await openRepoCell({
      repoId: workspaceId("ledger-mixed-consistency"),
      rootDir: canonicalRoot(rootDir),
      ownerId: "ledger-mixed-consistency",
      now: () => "2026-08-18T00:00:01.000Z",
    });
    const migrated = (await cell.run({ kind: "ledger-migrate" }, { actor, source: "local" })) as Record<
      string,
    ...
    assert.equal(projected.status, "ready");
    assert.deepEqual(
      new Set(projected.rows.map(({ taskId }) => taskId)),
      new Set([...flatEvents, shardedEvent].map((value) => value.taskId)),
    );
  } finally {
    await cell?.close();
    rmSync(rootDir, { recursive: true, force: true });
...
test("ha migrate ledger runs through a data-shape latch instead of being rejected by it", async () => {
  const rootDir = mkdtempSync(path.join(tmpdir(), "ha-ledger-latched-migrate-"));
  let cell: Awaited<ReturnType<typeof openRepoCell>> | undefined;
  try {
    const flatEvents = [flatTaskEvent(1, "task_latched_one")],
      shardedEvent = flatTaskEvent(2, "task_latched_two");
    initMixedLedger(rootDir, flatEvents, shardedEvent);
    cell = await openRepoCell({
      repoId: workspaceId("ledger-latched-migrate"),
      rootDir: canonicalRoot(rootDir),
      ownerId: "ledger-latched-migrate",
      now: () => "2026-08-18T00:00:01.000Z",
    });
...
function initRepo(rootDir: string): void {
  run(rootDir, "init", "-q");
  run(rootDir, "config", "user.name", "Daemon Layout Test");
  run(rootDir, "config", "user.email", "daemon-layout@example.invalid");
  run(rootDir, "config", "gc.auto", "0");
  run(rootDir, "config", "maintenance.auto", "false");
  run(rootDir, "commit", "--allow-empty", "-qm", "base");
}
...
function initMixedLedger(rootDir: string, flatEvents: readonly TaskEventV1[], shardedEvent: TaskEventV1): void {
  ...
  mkdirSync(path.dirname(twinSharded), { recursive: true });
  writeFileSync(twinSharded, twinBody);
...
function writeHead(rootDir: string, events: readonly TaskEventV1[]): void {
  ...
  run(rootDir, "commit", "-qm", "layout fixture");
}
function shardedEventRelativePath(opId: string): string {
  return `events/${sha256Text(opId).slice(0, 2)}/${opId}.json`;
}
function listTree(rootDir: string, target: string): readonly string[] {
  return run(rootDir, "ls-tree", "--name-only", `HEAD:${target}`).split("\n").filter(Boolean);
}
function show(rootDir: string, target: string): string {
  return run(rootDir, "show", target);
}
function run(rootDir: string, ...args: readonly string[]): string {
