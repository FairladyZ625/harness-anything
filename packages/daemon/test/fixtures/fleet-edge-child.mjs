...
for (;;) {
  try {
    ...
    }; const write = await runFleetWriteClient({ ...peer, channel: "replica", changes: [{ path: config.path, body, baseBlobSha256: config.baseBlobSha256 }], onFrame }); if (write.center.outcome !== "applied") throw new Error(`center write ${write.center.outcome}:${write.center.code ?? "retry"}`); const pulled = await runFleetReplicaPullClient({ ...peer, diskQuotaBytes, onFrame, edgeKillpoint: (point) => { if (config.edgeKillpoint === point) { if (config.markerFile) writeFileSync(config.markerFile, point); process.exit(75); } } });
