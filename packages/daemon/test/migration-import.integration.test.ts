...
test("authored coverage migrates ordinary documents but blocks an unwired specialized channel before any write", async () => {
  ...
  try {
    ...
    assert.match(
      String(result.summary),
      /\| objects\/\*\* \| excluded \| 1 \| PASS \|/u,
    );
    assert.match(
      String(result.summary),
      /\| repo-document \| migrated \| 1 \| PASS \|/u,
    );
...
test("non-UTF-8 attachments stay explicit in authored coverage as owner-excluded content", async () => {
  const scratch = mkdtempSync(
      path.join(tmpdir(), "ha-migrate-binary-attachments-"),
    ),
    source = path.join(scratch, "legacy"),
    destination = path.join(scratch, "new");
  let cell: Awaited<ReturnType<typeof openRepoCell>> | undefined;
  try {
    ...
    assert.equal(
      statOrNull(path.join(destination, "harness/field-notes/screenshot.png")),
      null,
    );
    assert.equal(
      statOrNull(
        path.join(
          destination,
          "harness/tasks/task_coverage-coverage-fixture/artifacts/screenshot.png",
        ),
      ),
      null,
    );
...
test(
  ...
  async () => {
    const scratch = mkdtempSync(
        path.join(tmpdir(), "ha-migrate-symbolic-link-"),
      ),
      source = path.join(scratch, "legacy"),
      destination = path.join(scratch, "new"),
    ...
    try {
      ...
      assert.equal(
        (event.payload.entity as { readonly nodeKind?: string }).nodeKind,
        "symbolic-link",
      );
      assert.equal(
        (
...
test("an authored document in an unfamiliar directory migrates as a repo document", async () => {
  ...
  try {
    ...
    assert.equal(
      readFileSync(
        path.join(destination, "harness/field-notes/2024/xyz.md"),
        "utf8",
      ),
    ...
    const event = makeTaskEventStore({
      ...
      .events.find(
        (candidate) =>
          candidate.schema === "migration-import-event/v1" &&
          candidate.payload.entity.kind === "repo-document",
      );
