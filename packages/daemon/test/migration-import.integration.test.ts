...
test("legacy copy -> initialized repository -> migration import -> reconciliation", async () => {
  ...
  try {
    ...
    assert.equal(dryRun.exitCode, 3, JSON.stringify(dryRun)); assert.equal(makeTaskEventStore({ repoId: "migration-target", rootDir: destination }).readHead()?.revision ?? 0, before); assert.deepEqual(snapshot(source), sourceBefore); assert.match(String(dryRun.summary), /\| task \| 2 \| 1 \| 1 \| 1 \| PASS \|/u); assert.match(String(dryRun.summary), /Format validation: 1 skipped/u);
    const applied = await cell.run({ kind: "migrate-import", sourceRoot: source }, { actor, source: "local" }) as Record<string, unknown>; assert.equal(applied.exitCode, 3, JSON.stringify(applied)); assert.match(String(applied.summary), /Reconciliation: PASS/u); assert.deepEqual(snapshot(source), sourceBefore);
    const store = makeTaskEventStore({ repoId: "migration-target", rootDir: destination }), events = store.read().events.filter((event) => event.schema === "migration-import-event/v1"); assert.equal(events.length, 5); assert.equal(events.every((event) => event.source === "migration-import/v1" && event.payload.generation === "v0" && typeof event.payload.migratedFrom === "string"), true); assert.deepEqual(events.slice(0, 3).map(({ occurredAt }) => occurredAt), ["2026-01-01T00:00:00.000Z", "2026-01-02T00:00:00.000Z", "2026-01-03T00:00:00.000Z"]);
    const tasks = await cell.read("repo.tasks.list"), task = tasks.rows.find(({ taskId }) => taskId === "task_legacy"); assert.equal(task?.generation, "v0"); assert.equal(task?.snapshot.task?.status, "done");
    const graph = await cell.read("repo.triadic.relationGraph"); assert.equal(graph.edges.length, 1); assert.equal(graph.coverageRows.length, 1); assert.equal(graph.coverageRows[0]?.status, "covered");
    const mapPath = path.join(destination, "harness", String(applied.idMapPath)); const idMap = JSON.parse(readFileSync(mapPath, "utf8")) as { maps: { task: Record<string, string>; relation: Record<string, string> }; skipped: unknown[] }; assert.equal(idMap.maps.task.task_legacy, "task_legacy"); assert.equal(Object.keys(idMap.maps.relation).length, 1); assert.equal(idMap.skipped.length, 1);
