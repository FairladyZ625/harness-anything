...
test("a different destination document at the same path requires a decision and reports both sides", async () => {
  ...
  try {
    ...
    const resolution = ["harness/field-notes/2024/xyz.md=source"], preview = await cell.run({ kind: "migrate-import", sourceRoots: sources(source), resolutions: resolution, dryRun: true }, { actor, source: "local" }) as Record<string, unknown>; assert.equal(preview.exitCode, 0, JSON.stringify(preview)); assert.equal(preview.outcome, "pending"); assert.equal((preview.proof as { readonly canonicalVisible: boolean }).canonicalVisible, false); assert.match(String(preview.summary), /resolved: source/u); assert.equal(makeTaskEventStore({ repoId: "migration-repo-conflict-target", rootDir: destination }).readHead()?.revision ?? 0, 0); assert.equal(readFileSync(target, "utf8"), destinationBody);
