...
test("task prose attaches the active execution while progress remains machine-owned", async () => {
  ...
  try { assert.equal((await host.run(fixture.repoId, { kind: "task-create", taskId: "task-doc", title: "Docs" }, auth)).outcome, "applied"); assert.equal((await host.run(fixture.repoId, { kind: "task-start", taskId: "task-doc", executionId: "execution-doc" }, auth)).outcome, "applied"); atomicSave(fixture.rootDir, "tasks/task-doc/notes.md", "# Notes\n\nwatch me\n"); await waitFor(() => revision(fixture) === 3); const event = makeTaskEventStore({ repoId: fixture.repoId, rootDir: fixture.rootDir }).read().events[2]; assert.equal(event?.schema, "doc-event/v1"); if (event?.schema === "doc-event/v1") assert.equal(event.payload.executionId, "execution-doc");
    const before = watcherScans(host); atomicSave(fixture.rootDir, "tasks/task-doc/progress.md", "# Progress\n\nmanual edit\n"); await waitFor(() => watcherScans(host) > before); assert.equal(revision(fixture), 3); const status = await host.run(fixture.repoId, { kind: "doc-status", paths: ["tasks/task-doc/progress.md"] }, auth); assert.equal(JSON.parse(status.evidence!.slice("doc-scan:".length)).rows[0].state, "blocked"); assert.equal(status.detail?.unresolvedTouches[0]?.requiredRoute, "task-progress-append");
  } finally { await host.close(); fixture.close(); }
});
