...
test("vim save is harvested without a doc command and restart collects offline edits", async () => {
  ...
  try {
    ...
    await waitFor(() => revision(fixture) === 2); assert.equal(readFileSync(path.join(fixture.rootDir, "harness/context/offline.md"), "utf8"), "# Offline\n\nSaved while stopped.\n"); await host.close(); assert.equal(git(fixture.rootDir, "diff", "--name-only", "--", "harness"), ""); assert.equal(git(fixture.rootDir, "status", "--porcelain", "-uall").includes(".conflict-"), false);
...
test("task progress appends canonically, reports its file, and watcher loopback stays zero-write", async () => {
  ...
  await host.attachmentsSettled();
  try { const beforePeopleWake = watcherScans(host), people = readFileSync(path.join(fixture.rootDir, "harness/people.yaml"), "utf8"); atomicSave(fixture.rootDir, "people.yaml", `${people.trimEnd()}\n\n`); await waitFor(() => watcherScans(host) > beforePeopleWake); assert.equal(watcherState(host), "active", "people-registry-owned bytes must not turn the doc-sync observer red"); assert.equal((await host.run(fixture.repoId, { kind: "task-create", taskId: "task-doc", title: "Docs" }, auth)).outcome, "applied"); const withoutLease = await host.run(fixture.repoId, { kind: "task-progress-append", taskId: "task-doc", text: "too early", evidence: [] }, auth); assert.equal(withoutLease.code, "progress_lease_required"); assert.equal(revision(fixture), 1); assert.equal((await host.run(fixture.repoId, { kind: "task-start", taskId: "task-doc", executionId: "execution-doc" }, auth)).outcome, "applied"); const beforeBinaryWake = watcherScans(host), binary = path.join(fixture.rootDir, "harness/tasks/task-doc-docs/artifacts/screenshots/evidence.png"); mkdirSync(path.dirname(binary), { recursive: true }); writeFileSync(binary, Buffer.from([0x89, 0x50, 0x4e, 0x47, 0xff, 0x00])); await waitFor(() => watcherScans(host) > beforeBinaryWake); assert.equal(watcherState(host), "active", "an inapplicable binary artifact must not turn the observer red"); assert.equal(revision(fixture), 2); atomicSave(fixture.rootDir, "tasks/task-doc-docs/notes.md", "# Notes\n\nwatch me\n"); await waitFor(() => revision(fixture) === 3); const event = makeTaskEventStore({ repoId: fixture.repoId, rootDir: fixture.rootDir }).read().events[2]; assert.equal(event?.schema, "doc-event/v1"); if (event?.schema === "doc-event/v1") assert.equal(event.payload.executionId, "execution-doc");
...
test("watch admission fails closed when the queued path fingerprint changes", async () => {
  ...
  try { atomicSave(rootDir, "context/current.md", body); const rejected = await cell.run(action, { actor: { principal: { personId: "person-owner" }, executor: null }, source }); assert.equal(rejected.outcome, "op_rejected"); assert.equal(rejected.code, "watch_fingerprint_changed"); assert.equal(makeTaskEventStore({ repoId: "watch-fingerprint", rootDir }).read().revision, 0); const accepted = await cell.run(action, { actor: { principal: { personId: "person-owner" }, executor: null }, source: { ...source, fingerprint: sha256Text(body) } }); assert.equal(accepted.outcome, "applied", JSON.stringify(accepted)); }
...
test("SIGKILL after WAL visibility restarts the watcher and converges without a duplicate", async () => {
...
test("watch session concurrent editing soak remains single-writer and convergent", async (context) => {
  ...
  await host.attachmentsSettled();
  try { for (let round = 1; round <= rounds; round += 1) { atomicSave(fixture.rootDir, "context/soak.md", `# Soak\n\neditor-a ${round}\n`); atomicSave(fixture.rootDir, "context/soak.md", `# Soak\n\neditor-b ${round}\n`); try { await waitFor(() => watcherCommits(host) === round); } catch (error) { context.diagnostic(`round=${round} revision=${revision(fixture)} status=${JSON.stringify(host.status().repos[0])} body=${JSON.stringify(readFileSync(path.join(fixture.rootDir, "harness/context/soak.md"), "utf8"))}`); throw error; } }
    const elapsedMs = performance.now() - started; context.diagnostic(`watch-soak rounds=${rounds} edits=${rounds * 2} durationMs=${elapsedMs.toFixed(1)}`); assert.equal(revision(fixture), rounds); assert.equal(makeTaskEventStore({ repoId: fixture.repoId, rootDir: fixture.rootDir }).read().events.every((event) => typeof event.source === "object" && event.source.kind === "watch_session"), true); await host.close(); assert.equal(git(fixture.rootDir, "diff", "--name-only", "--", "harness"), "");
  } finally { await host.close(); fixture.close(); }
});

function scan(base: string, rows: readonly { path: string; state: string; candidateBlobSha256: string }[]): WriteReceipt { const baseLedgerSha = { repoId: "watch-test", revision: 0, headDigest: `sha256:${sha256Text(base)}` }; return { outcome: "pending", opId: `scan:${base}`, revision: 0, evidence: `doc-scan:${JSON.stringify({ baseLedgerSha, rows: rows.map((row) => ({ ...row, reason: null, baseBlobSha256: null, size: 1, mediaType: "text/markdown" })) })}`, visibility: "center", proof: { committedRevision: 0, appliedCut: 0, durable: false, canonicalVisible: false, worktreeVisible: false }, nextAction: "Submit an eligible scan candidate to publish its canonical event." }; }
...
function watcherState(host: Awaited<ReturnType<typeof openDaemonHost>>): string { return (host.status().repos[0] as { docSync?: { state: string } } | undefined)?.docSync?.state ?? "missing"; }
