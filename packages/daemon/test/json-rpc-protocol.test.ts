...
test("RepoCell serializes identical lifecycle intents into one WAL event and one drained Git publication", async () => {
  ...
  try {
    ...
    assert.equal(git(rootDir, "rev-list", "--count", "refs/ha/canonical"), "1");
...
for (const killpoint of ["before_event_write", "after_event_write", "after_head_write", "before_worktree_rename", "after_worktree_rename",
  ...
  test(`RepoCell new generation recovers ${killpoint} without a duplicate publication`, async () => {
    ...
    try {
      ...
      const prePublicationCrash = killpoint === "before_event_write"; assert.equal(makeTaskEventStore({ repoId: "crash", rootDir }).read().revision, prePublicationCrash ? 0 : 1);
      ...
      if (prePublicationCrash) { assert.equal(git(rootDir, "rev-list", "--count", "refs/ha/canonical"), "1"); await recovered.close(); recovered = undefined; }
      assert.equal(git(rootDir, "rev-list", "--count", "refs/ha/canonical"), "2");
