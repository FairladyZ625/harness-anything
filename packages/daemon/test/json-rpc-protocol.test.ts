...
test("invalid Decision payload stays invalid_command and reckon records exact projected basis", async () => {
  ...
  try {
    ...
    assert.equal(proposed.outcome, "applied", JSON.stringify(proposed)); const decisionId = (JSON.parse(proposed.evidence) as { decisionId: string }).decisionId, related = await cell.run({ kind: "relation-relate", sourceRef: `decision/${decisionId}/CH1`, targetRef: "task/task-decision", relationType: "derives", rationale: "Decision creates this task.", expectedVersion: 0 }, binding); assert.equal(related.outcome, "applied", JSON.stringify(related)); const decisionPath = `decisions/decision-${decisionId}/decision.md`, decisionFile = path.join(rootDir, "harness", decisionPath), beforeInvalid = makeTaskEventStore({ repoId: "decision-cell", rootDir }).readHead()!.revision;
