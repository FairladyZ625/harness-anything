...
test("relation graph contract accepts the materialized ledger row schema and rejects malformed rows", () => {
  ...
  assert.deepEqual(validateDaemonRelationGraph({ ...payload, coverageRows: [{ ...payload.coverageRows[0], status: "uncovered", fulfillment: null, freshnessReason: "refuted", refutingFactRefs: ["fact/task_REAL/F-REAL"] }] }), []);
...
test("lifecycle commands publish typed events, machine files, rebuildable L2, and complete receipts in one cut", async () => {
  ...
  try {
    ...
    writeFileSync(path.join(rootDir, "review.json"), JSON.stringify({ verdict: "approved", reason: "Independent review passed.", evidenceChecked: ["tests"] })); const reviewBinding = { actor: { principal: { personId: "person-reviewer" }, executor: { kind: "agent" as const, id: "arbiter" } }, source: "local" as const, roles: ["$arbiter"] };
...
test("milestone-closeout uses the normal completion facade, review, and gates exactly once", async () => {
  ...
  try {
    ...
    const reviewBinding = (id: string) => ({ actor: { principal: { personId: `person-${id}` }, executor: { kind: "agent" as const, id } }, source: "local" as const, roles: ["$arbiter"] });
