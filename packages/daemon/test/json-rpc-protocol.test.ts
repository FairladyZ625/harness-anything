...
test("review-consent derives the recorded Review digests without a packet and still rejects mismatched ones", async () => {
  ...
  try {
    ...
    writeFileSync(path.join(rootDir, "review.json"), JSON.stringify({ verdict: "approved", reason: "Independent review passed.", evidenceChecked: ["tests"] }));
