...
test("runtime instance CRUD is a closed defineCliCommand surface", async () => {
  ...
  assert.deepEqual(
    ...
    ]),
    {
      ...
      code: "invalid_field",
      nextAction:
        "Claude runtime instances cannot accept Codex options: --effort, --wire-api, --requires-openai-auth, or --http-header.",
      json: false,
    },
  );
...
test("public instance projections keep provider options in the matching kind section", async () => {
  ...
  try {
    ...
    store.create({
      ...
      defaultModel: "claude",
      enabled: true,
      claude: { baseUrl: "https://gateway.example.test/v1" },
      auth: { mode: "subscription" },
    });
    ...
    assert.equal("codex" in claudeDto, false);
    assert.deepEqual(claudeDto.claude, { baseUrl: "https://gateway.example.test/v1", baseUrlConfigured: true });
    assert.equal(
      listed.every((item) => (item.kindId === "codex" ? "codex" in item : "claude" in item)),
      true,
    );
  } finally {
    rmSync(userRoot, { recursive: true, force: true });
...
test("flat Claude effort from schema v2 migrates away without granting Claude Codex configuration", async () => {
  ...
  try {
    ...
    assert.deepEqual(store.read("claude-flat")?.claude, { baseUrl: "https://gateway.example.test/v1" });
    const persisted = JSON.parse(readFileSync(path.join(userRoot, "runtime-instances.json"), "utf8")).instances[0];
    assert.deepEqual(persisted.claude, { baseUrl: "https://gateway.example.test/v1" });
    assert.equal("reasoningEffort" in persisted, false);
...
test("agy owns its access policy and rejects harness permission declarations", async () => {
  ...
  try {
    ...
    await assert.rejects(
      store.prepareLaunch("agy-open", { cwd: "/workspace/repo", prompt: "Bad", permissionMode: "bypass" }),
      (error: unknown) => codedAs(error, "invalid_runtime_permission"),
    );
  } finally {
    rmSync(userRoot, { recursive: true, force: true });
...
test("persisted schema v2 records normalize permission and isolation fields once on read", async () => {
  ...
  try {
    ...
    const storeAgain = openRuntimeInstanceStore({ userRoot, discover: () => [observed, claude] }),
...
test("win32 installation discovery probes PATHEXT suffixes and witnesses the shim", async () => {
  ...
  try {
    ...
    const installations = discoverRuntimeInstallations({
      env: { PATH: bin },
      platform: "win32",
