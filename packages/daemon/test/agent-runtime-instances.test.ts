...
test("runtime instance CRUD is a closed defineCliCommand surface", async () => {
  ...
  assert.deepEqual(parseThinCommand(["runtime", "run", "codex-review", "--effort", "turbo", "--prompt", "Inspect"]), { ok: false, code: "invalid_runtime_effort", nextAction: "Use minimal, low, medium, high, xhigh, or max with Claude or Codex; agy supports low, medium, or high.", json: false });
...
test("runtime instance command receipts expose readiness metadata without credential refs or host paths", async () => {
  ...
  try {
    ...
    assert.deepEqual(created.instance, { schemaVersion: 2, instanceId: "codex-safe", name: "Codex Safe", kindId: "codex", installationId: observed.installationId, providerId: "openai", models: ["gpt-5.6-sol"], defaultModel: "gpt-5.6-sol", enabled: true, permissionMode: "bypass", codex: { reasoningEffort: null, fast: false, baseUrl: null, baseUrlConfigured: false, wire_api: null, requires_openai_auth: null, http_headers: null }, authMode: "api-key", authState: "configured", authReadiness: { status: "not-ready", code: "runtime_auth_not_checked", hint: "Authentication has not been verified in this daemon generation." }, isolationState: "enforced" });
...
test("Codex fast reaches its CLI, per-run false overrides its default, and unsupported kinds reject", async () => {
  const userRoot = mkdtempSync(path.join(tmpdir(), "ha-runtime-fast-")), agy = { installationId: "agy-fast-installation", kindId: "agy" as const, executablePath: "/opt/runtime-test/agy", version: "1.1.22", observedAt: "2026-08-29T00:00:00.000Z" };
  try {
    const store = openRuntimeInstanceStore({ userRoot, discover: () => [observed, agy], resolveCredential: () => "instance-secret", subscriptionReady: () => ({ status: "ready", code: null, hint: null }) });
    store.create({ schemaVersion: 2, instanceId: "codex-fast", name: "Fast", kindId: "codex", installationId: observed.installationId, providerId: "openai", models: ["gpt-5.6-sol"], defaultModel: "gpt-5.6-sol", enabled: true, codex: { fast: true }, auth: { mode: "api-key", credentialRef: "credential:v1:codex-fast" } });
    const configured = await store.prepareLaunch("codex-fast", { cwd: "/workspace/repo", prompt: "Configured" }), overridden = await store.prepareLaunch("codex-fast", { cwd: "/workspace/repo", prompt: "Override", fast: false });
    assert.deepEqual(configured.args.slice(configured.args.indexOf("--config"), configured.args.indexOf("--config") + 2), ["--config", 'service_tier="fast"']); assert.equal(overridden.args.includes('service_tier="fast"'), false); assert.equal(configured.definition.fast, true); assert.equal(overridden.definition.fast, false); assert.equal(store.read("codex-fast")?.kindId === "codex" && store.read("codex-fast")?.codex.fast, true);
    const created = parseThinCommand(["runtime", "instance", "create", "--id", "codex-fast", "--name", "Fast", "--kind", "codex", "--installation", observed.installationId, "--provider", "openai", "--model", "gpt-5.6-sol", "--fast", "--auth", "subscription"]), updated = parseThinCommand(["runtime", "instance", "update", "codex-fast", "--fast"]), run = parseThinCommand(["runtime", "run", "codex-fast", "--fast", "--prompt", "Inspect"]);
    assert.equal(created.ok && (created.command.action.codex as { fast?: boolean }).fast, true); assert.equal(updated.ok && updated.command.action.fast, true); assert.equal(run.ok && run.command.action.fast, true);
    assert.deepEqual(parseThinCommand(["runtime", "instance", "create", "--id", "agy-fast", "--name", "AGY Fast", "--kind", "agy", "--installation", agy.installationId, "--provider", "google", "--model", "gemini", "--fast", "--auth", "subscription"]), { ok: false, code: "invalid_runtime_fast", nextAction: "Fast mode is supported only by Codex runtime instances.", json: false });
    store.command({ kind: "runtime-instance-create", instanceId: "agy-fast", name: "AGY Fast", kindId: "agy", installationId: agy.installationId, providerId: "google", models: ["gemini-3.1-pro-low"], authMode: "subscription" });
    await assert.rejects(store.prepareLaunch("agy-fast", { cwd: "/workspace/repo", prompt: "Reject", fast: true }), (error: unknown) => codedAs(error, "invalid_runtime_fast")); assert.throws(() => store.command({ kind: "runtime-instance-update", instanceId: "agy-fast", fast: true }), (error: unknown) => codedAs(error, "invalid_runtime_fast"));
  } finally { rmSync(userRoot, { recursive: true, force: true }); }
});
