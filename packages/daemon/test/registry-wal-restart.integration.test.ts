...
import { reproduceRegistrySqliteRestart } from "./registry-wal-restart.repro.mjs";

for (const arm of ["graceful-stop", "sigkill"] as const) {
  test(`daemon preserves acknowledged SQLite writes after ${arm}`, async () => {
    const fixtureRoot = mkdtempSync(path.join(tmpdir(), `ha-registry-wal-test-${arm}-`));
    try {
      const result = await reproduceRegistrySqliteRestart(arm, { fixtureRoot });
      ...
      assert.equal(result.before.receipts.taskReceipt.status, "accepted_durable");
      assert.equal(result.before.receipts.factReceipt.outcome, "applied");
      assert.equal(result.before.receipts.factReceipt.status, "accepted_durable");
      assert.equal(result.after.registrySchema, "harness-daemon-registry/v2");
      assert.equal(result.after.canonicalEventHead, result.before.canonicalEventHead);
      assert.equal(result.after.eventDigest, result.before.eventDigest);
