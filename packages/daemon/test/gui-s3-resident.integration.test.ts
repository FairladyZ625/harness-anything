...
test("GUI S3 resident daemon bridge serves two RepoCells, catalog/runtime/control, secret rejection, and real PTY", async () => {
    ...
    try {
      ...
      const unavailable = rows.find((row) => row.repoId === "beta"); assert.equal(unavailable?.cellState, "unavailable"); assert.equal(unavailable?.queueDepth, 0); assert.equal(unavailable?.lockState, "unknown"); assert.equal(typeof unavailable?.unavailableReason, "string");
      ...
      const restart = await rpc("daemon.gui.control.request", { payload: { kind: "restart", authorityRepoId: "alpha" } }); assert.equal(restart.outcome, "rejected"); assert.equal((restart.error as Record<string, unknown>).code, "supervisor_required"); assert.equal(typeof restart.operationId, "string");
      ...
      const beforeSpawn = await rpc("repo.agentRuntime.overview", { repo: { repoId: "alpha" }, payload: {} }); assert.equal((beforeSpawn.instances as Array<Record<string, unknown>>)[0]?.instanceId, "resident-codex"); assert.doesNotMatch(JSON.stringify(beforeSpawn), /credentialRef|executablePath/iu);
      const spawnedRuntime = await rpc("repo.agentRuntime.spawn", { repo: { repoId: "alpha" }, payload: { runtimeInstanceId: "resident-codex", cwd: { scope: "repo-root" }, prompt: "Inspect", taskId: null, idempotencyKey: "resident-runtime" } }); assert.equal(spawnedRuntime.outcome, "applied"); const overview = await rpc("repo.agentRuntime.overview", { repo: { repoId: "alpha" }, payload: {} }), session = (overview.sessions as Array<Record<string, unknown>>).find((candidate) => candidate.runtimeSessionId === spawnedRuntime.runtimeSessionId); assert.equal(session?.instanceId, "resident-codex"); assert.equal((session?.definitionSnapshot as Record<string, unknown>).installationId, "installation-codex"); assert.equal(launched?.executablePath, executablePath); assert.deepEqual(launched?.args, ["exec", "--model", "runtime-test-model", "-c", "model_provider=\"openai\"", "-"]); assert.match(String((launched?.env as Record<string, unknown>).HOME), /runtime-instances\/resident-codex\/home$/u);
