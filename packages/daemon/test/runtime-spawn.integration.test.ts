...
test("runtime spawn publishes a canonical session and makes it visible in overview", async () => {
    ...
    try {
      ...
      try {
        ...
        assert.deepEqual(launchedCredential, { kindId: "codex", baseUrl: "https://api.example.test/", credentialRef: "keychain:harness/codex", configuredAt: "2026-08-14T00:00:00.000Z" });
...
test("runtime spawn maps the GUI Claude kind to a canonical claude-compatible installation", async () => {
    ...
    try {
      ...
      const cell = await openRepoCell({ repoId: workspaceId("runtime-spawn-claude"), rootDir: canonicalRoot(root), ownerId: "spawn-test", runtimeLaunch: (input) => { launchedKind = input.kindId; return { pid: 124, onExit: () => undefined, terminate: () => undefined }; } });
      try {
        ...
        assert.equal(receipt.outcome, "applied"); assert.equal(launchedKind, "claude");
      } finally { await cell.close(); }
    } finally { rmSync(root, { recursive: true, force: true }); }
});
