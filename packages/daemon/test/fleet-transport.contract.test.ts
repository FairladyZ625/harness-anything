...
const frames = [
  ...
  { schema: "fleet.task.command/v1", messageId: "m22-task", assignmentId: "a1", writerEpoch: 1, opId: "op-task-1", repoId: "repo", taskId: "task_abc", action: { kind: "task-start", taskId: "task_abc", ttlMs: 86_400_000 }, waitMs: 30_000, docChanges: null, mirrorBaseCut: null },
...
test("Fleet codec rejects unknown provenance, nested fields, malformed values, and limits", () => {
  const taskCommand = frames.find((frame) => frame.schema === "fleet.task.command/v1")!, snapshotPage = frames.find((frame) => frame.schema === "fleet.snapshot.page/v1")!, snapshotChunk = frames.find((frame) => frame.schema === "fleet.snapshot.chunk/v1")!, snapshotCurrent = frames.find((frame) => frame.schema === "fleet.replica.current/v1")!;
  ...
  for (const invalid of [
    ...
    { ...taskCommand, action: { kind: "task-start", taskId: "task_abc", verb: "start" } },
    ...
    { ...taskCommand, action: { kind: "task-create", title: "t", registerModule: { key: "m", title: "M", prefix: "m", scope: "repo", actor: "spoof" } } },
