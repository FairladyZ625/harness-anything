...
test("compile rejection before PREPARED preserves its structured rejection code", async () => {
  const messages: RepoWriteChildMessage[] = [];
  ...
  await host.start();
  await host.receive(submit("request-compile-rejected"));
  ...
  assert.deepEqual({
    ...
    code: "authority_ingress_rejected",
    diagnostic:
      "Error: AUTHORITY_MANUAL_ENTITY_ID_FORBIDDEN: omit --id; decision-propose identity is generated by canonical ingress"
  });
});
