...
test("Decision prose is an explicit idempotent doc-sync region in the canonical authored document", async () => {
  ...
  try {
    ...
    const retried = await fixture.cell.run(firstAction, binding); assert.equal(retried.outcome, "applied"); assert.match(retried.opId, /^noop:/u); assert.equal(retried.revision, first.revision);
