...
test("a produced terminal op_rejected receipt passes its validator", () => {
  assert.deepEqual(
    validateTerminalControlReceipt({
      ...
      ok: false,
      outcome: "op_rejected",
      operationId: "terminal-op-x",
      sessionId: "s1",
      daemonGeneration: 1,
      state: "exited",
      error: { code: "terminal_exited", hint: "Start a new terminal session." },
    }),
    [],
  );
});
...
test("every GUI S3 result validator fails closed", () => {
  for (const validate of [
    ...
    assert.notDeepEqual(validate({ ok: true, unexpected: true }), []);
});

test("every pinned GUI S3 negative fixture is rejected by its declared validator", () => {
  const cases = [
    ["gui-system-status-invalid.json", validateSystemStatus],
    ["daemon-control-receipt-invalid.json", validateDaemonControlReceipt],
    ["gui-catalog-snapshot-invalid.json", validateCatalogSnapshot],
    ["gui-catalog-preset-invalid.json", validateCatalogPreset],
    ["catalog-reread-receipt-v1-invalid.json", validateCatalogRereadReceipt],
    ["terminal-session-list-invalid.json", validateTerminalSessionList],
    ["terminal-control-receipt-v1-invalid.json", validateTerminalControlReceipt],
    ["terminal-input-ack-v1-invalid.json", validateTerminalInputAck],
    ["terminal-detach-ack-v1-invalid.json", validateTerminalDetachAck],
    ["terminal-attach-v1-invalid.json", validateTerminalAttach],
    ["terminal-attach-event-v1-invalid.json", validateTerminalAttachEvent],
  ] as const;
  for (const [fixture, validate] of cases)
    assert.notDeepEqual(
      validate(JSON.parse(readFileSync(new URL(`../fixtures/contracts/${fixture}`, import.meta.url), "utf8"))),
      [],
      fixture,
    );
});
