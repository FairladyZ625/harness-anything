...
export function openDocSyncWatcher(input: { readonly rootDir: string; readonly personId: string; readonly run: Runner; readonly debounceMs?: number; readonly watchFilesystem?: boolean; readonly startupScan?: boolean }): DocSyncWatcher {
  ...
  if (input.watchFilesystem !== false) try { filesystem = watch(authoredRoot, { recursive: true }, (_event, filename) => filename === null ? wake() : wake(String(filename))); filesystem.on("error", () => wake()); } catch (error) { consumeKnownError(error); state = "blocked"; }
  ...
  return { wake, overflow: () => wake(), flush: async () => { if (timer) { clearTimeout(timer); timer = null; } enqueue(true); await tail; }, status: () => ({ sessionId, personId: input.personId, state, pendingPaths: [...pending].sort(), lastReceipt, metrics: { ...metrics } }),
    close: async () => { if (state === "closed") return; state = "closed"; if (timer) clearTimeout(timer); timer = null; filesystem?.close(); await tail; } };
...
function normalize(value: string): string | null { const normalized = value.split(path.sep).join("/").replace(/^\.\//u, ""); return normalized && !normalized.startsWith("/") && !normalized.split("/").includes("..") && (normalized.endsWith(".md") || normalized.endsWith(".txt")) && !normalized.includes(".conflict-") ? normalized : null; }
