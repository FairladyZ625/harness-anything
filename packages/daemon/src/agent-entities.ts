...
export interface AgentEntityGuiDetail { readonly id: string; readonly name: string; readonly runtimeType: string; readonly role: "worker" | "commander"; readonly instructions: string; readonly model: string | null; readonly skills: readonly string[]; readonly prompts: readonly string[]; readonly preset: string | null }
...
function entitySkillIds(value: unknown): readonly string[] { return Array.isArray(value) ? value.map(agentEntityRecord).map((skill) => entityText(skill.id)).filter(Boolean) : []; }
