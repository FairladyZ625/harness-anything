...
function daemonSocketConnectError(socketPath: string, cause: unknown): unknown {
  ...
  if (shape === "directory" && ownership === "unowned") {
    try {
      ...
      cleanup = socketErrorCode(error) === "ENOTEMPTY" ? "preserved-non-empty-directory" : `failed:${String(socketErrorCode(error) ?? "unknown")}`;
    }
  }
  return new Error(
