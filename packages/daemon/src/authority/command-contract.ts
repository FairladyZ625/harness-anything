// @slice-activation PLT-Boundary W2 publishes the daemon host command seam to CLI grammar and production composition.
import type {
  DomainStatus,
  FactMemoryClass,
  FactMemoryTag,
...
export interface CommandGitCommitAuthor {
  readonly name: string;
  readonly email: string;
}

export interface CommandActorAttribution {
  readonly writeAttribution: WriteAttribution;
  readonly commitAuthor: CommandGitCommitAuthor;
...
export interface AuthorityCutoverCommandResult {
  ...
  readonly error?: {
    readonly code: AuthorityCutoverCommandErrorCode;
