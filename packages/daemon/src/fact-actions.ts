...
function filters(action: Readonly<Record<string, unknown>>): FactSearchFilters {
  const allowed = ["kind", "query", "taskId", "confidence", "memoryClass", "observedAfter", "observedBefore", "limit", "cursor"];
  if (Object.keys(action).some((field) => !allowed.includes(field))) throw factActionError("invalid_command", "Fact search filters contain an unknown field.");
