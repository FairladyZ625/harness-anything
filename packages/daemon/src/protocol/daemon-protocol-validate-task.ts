...
// The declaration set is closed, so each pattern compiles once and is memoized by its source.
const actionPatternCache = new Map<string, RegExp>();
function actionPattern(regex: string): RegExp {
  let compiled = actionPatternCache.get(regex);
  if (compiled === undefined) {
    compiled = new RegExp(regex, "u");
    actionPatternCache.set(regex, compiled);
  }
  return compiled;
}
...
function validateProjectedTaskActionInput(
  ...
  for (const field of input.fields) {
    ...
    if (field.regex && typeof item === "string" && !actionPattern(field.regex).test(item))
...
export function validateDaemonTaskSnapshotList(value: unknown): readonly string[] {
  const served = validateDaemonTaskSnapshotListServed(value);
  if (served.length || !isJsonObject(value) || !Array.isArray(value.rows)) return served;
...
/** The served-shape half an in-process exit judges: envelope fields plus the isolated-row
 * diagnostics. Row content was already judged by the producer — task-query-read.ts isolates bad
 * rows at build — so a same-process exit must not re-walk every row; the client inbound parse
 * escalates to the full validator because it trusts nothing across the wire. */
export function validateDaemonTaskSnapshotListServed(value: unknown): readonly string[] {
