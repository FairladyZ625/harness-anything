import { shape, type DaemonGuiRpcReadMethod } from "./daemon-protocol-gui-types.ts";
import { observeTailReadMethod } from "./daemon-protocol-observe-tail-read.ts";
import {
  DAEMON_AGENDA_SCHEMA,
  DAEMON_ARTIFACTS_LIST_SCHEMA,
  DAEMON_CI_OBSERVATORY_SCHEMA,
  DAEMON_AGENT_ENTITY_CATALOG_SCHEMA,
  DAEMON_AGENT_ENTITY_DETAIL_SCHEMA,
  DAEMON_AGENT_RUNTIME_EVENTS_SCHEMA,
  DAEMON_AGENT_RUNTIME_OVERVIEW_SCHEMA,
  DAEMON_AGENT_RUNTIME_SESSION_SCHEMA,
  DAEMON_AGENT_RUNTIME_TOKEN_USAGE_SCHEMA,
  DAEMON_AGENT_RUNTIME_TOKEN_USAGE_DETAIL_SCHEMA,
  DAEMON_AGENT_SKILL_CATALOG_SCHEMA,
  DAEMON_CONTROL_RECEIPT_SCHEMA,
  DAEMON_DECISION_LIST_SCHEMA,
  DAEMON_DOCUMENT_READ_SCHEMA,
  DAEMON_ENTITY_ACTION_EXPLANATION_SCHEMA,
  DAEMON_ENTITY_KIND_CATALOG_SCHEMA,
  DAEMON_VERTICAL_DECLARATION_READ_SCHEMA,
  DAEMON_ENTITY_ROW_LIST_SCHEMA,
  DAEMON_ENTITY_CONTENT_READ_SCHEMA,
  DAEMON_ENTITY_LOCATOR_READ_SCHEMA,
  DAEMON_PROTOCOL_ERROR_SCHEMA,
  DAEMON_USE_CASE_PROJECTION_SCHEMA,
  DAEMON_RELATION_GRAPH_SCHEMA,
  DAEMON_SQUAD_ENTITY_CATALOG_SCHEMA,
  DAEMON_SQUAD_ENTITY_DETAIL_SCHEMA,
  DAEMON_SQUAD_RUN_LIST_SCHEMA,
  DAEMON_SQUAD_RUN_READ_SCHEMA,
  DAEMON_TASK_DISPATCHES_SCHEMA,
  DAEMON_TASK_DOCUMENT_LIST_SCHEMA,
  DAEMON_TASK_SNAPSHOT_LIST_SCHEMA,
  DAEMON_TASK_COMPLETION_SCHEMA,
  DAEMON_TASK_WIP_SCHEMA,
  DAEMON_WORKSPACE_SUMMARY_SCHEMA,
  DAEMON_WORKSPACE_SCOPE_SCHEMA,
  DAEMON_SETTINGS_READ_SCHEMA,
  daemonAgendaPayloadShape,
  daemonArtifactsListPayloadShape,
  daemonDecisionListPayloadShape,
  daemonTokenUsagePayloadShape,
  daemonTokenUsageDetailPayloadShape,
  daemonRelationQueryPayloadShape,
  daemonTaskQueryPayloadShape,
  daemonUseCaseProjectionPayloadShape,
  GUI_CATALOG_PRESET_SCHEMA,
  GUI_CATALOG_SNAPSHOT_SCHEMA,
  GUI_SYSTEM_STATUS_SCHEMA,
  TERMINAL_SESSION_LIST_SCHEMA,
  ENTITY_ACTION_EXPLAIN_REQUEST_SCHEMA,
} from "./daemon-protocol-schema-ids.ts";

export const daemonGuiReadMethods = Object.freeze([
  {
    id: "gui.system.read",
    phase: "W5-GUI-S3",
    method: "daemon.gui.system.read",
    requiresRepo: false,
    params: shape({}),
    guiBridgeMethod: "getSystemStatus",
    httpMethod: "GET",
    path: "/api/system",
    inputSchemaId: "gui.empty/v1",
    outputSchemaId: GUI_SYSTEM_STATUS_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "getSystemStatus",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "gui.control.receipt",
    phase: "W5-GUI-S3",
    method: "daemon.gui.control.receipt",
    requiresRepo: false,
    params: shape({ payload: shape({ operationId: "string" }) }),
    guiBridgeMethod: "getDaemonControlReceipt",
    httpMethod: "GET",
    path: "/api/system/control/:operationId",
    inputSchemaId: "gui.daemon-control-receipt/v1",
    outputSchemaId: DAEMON_CONTROL_RECEIPT_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "getDaemonControlReceipt",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  observeTailReadMethod,
  {
    id: "ci.observatory.read",
    phase: "PLT-TestEng-W1",
    method: "repo.ci.observatory.read",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }), payload: shape({ window: "number?" }) }),
    guiBridgeMethod: "getCiObservatory",
    httpMethod: "GET",
    path: "/api/ci/observatory",
    inputSchemaId: "gui.ci-observatory-query/v1",
    outputSchemaId: DAEMON_CI_OBSERVATORY_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readCiObservatory",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "settings.read",
    phase: "Settings-Kind",
    method: "repo.settings.read",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }) }),
    guiBridgeMethod: "getSettings",
    httpMethod: "GET",
    path: "/api/settings",
    inputSchemaId: "gui.empty/v1",
    outputSchemaId: DAEMON_SETTINGS_READ_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "getSettings",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "tasks.list",
    phase: "W2-GUI",
    method: "repo.tasks.list",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: daemonTaskQueryPayloadShape,
    }),
    guiBridgeMethod: "getTasks",
    httpMethod: "GET",
    path: "/api/tasks",
    inputSchemaId: "gui.task-query/v1",
    outputSchemaId: DAEMON_TASK_SNAPSHOT_LIST_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "listTasks",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "tasks.wip",
    phase: "W2-GUI",
    method: "repo.tasks.wip",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }) }),
    guiBridgeMethod: "getTaskWip",
    httpMethod: "GET",
    path: "/api/tasks/wip",
    inputSchemaId: "gui.empty/v1",
    outputSchemaId: DAEMON_TASK_WIP_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "getTaskWip",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    // dec_5B135F46 CH4 layer two. One transport row carries every named use-case projection; the
    // catalog that says which names exist lives in kernel `use-case-projection-catalog.ts` and
    // holds no method / path / commandClass / auth / lease dimension, which is CH2's condition.
    id: "projection.read",
    phase: "PLT-Ontology-4.1",
    method: "repo.projection.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: daemonUseCaseProjectionPayloadShape,
    }),
    guiBridgeMethod: "readUseCaseProjection",
    httpMethod: "POST",
    path: "/api/projections",
    inputSchemaId: "gui.use-case-projection/v1",
    outputSchemaId: DAEMON_USE_CASE_PROJECTION_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readUseCaseProjection",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "entity.actions.explain",
    phase: "Ontology-Explain-A",
    method: "repo.entity.actions.explain",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({
        schema: "string",
        mode: { values: ["catalog", "object"], optional: false },
        entityKind: "string-null?",
        refs: "array",
        executor: "json?",
      }),
    }),
    guiBridgeMethod: "explainEntityActions",
    httpMethod: "POST",
    path: "/api/entities/actions/explain",
    inputSchemaId: ENTITY_ACTION_EXPLAIN_REQUEST_SCHEMA.id,
    outputSchemaId: DAEMON_ENTITY_ACTION_EXPLANATION_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "explainEntityActions",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "vertical.declaration.read",
    phase: "Governed-Entity-W2-0",
    method: "repo.vertical.declaration.read",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }) }),
    guiBridgeMethod: "readVerticalDeclaration",
    httpMethod: "GET",
    path: "/api/vertical/declaration",
    inputSchemaId: "gui.empty/v1",
    outputSchemaId: DAEMON_VERTICAL_DECLARATION_READ_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readVerticalDeclaration",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "entity.kinds.read",
    phase: "Governed-Entity-W2-0",
    method: "repo.entity.kinds.read",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }) }),
    guiBridgeMethod: "readEntityKinds",
    httpMethod: "GET",
    path: "/api/entities/kinds",
    inputSchemaId: "gui.empty/v1",
    outputSchemaId: DAEMON_ENTITY_KIND_CATALOG_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readEntityKinds",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "entity.rows.read",
    phase: "Governed-Entity-W2-0",
    method: "repo.entity.rows.read",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }) }),
    guiBridgeMethod: "readEntityRows",
    httpMethod: "GET",
    path: "/api/entities/rows",
    inputSchemaId: "gui.empty/v1",
    outputSchemaId: DAEMON_ENTITY_ROW_LIST_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readEntityRows",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "entity.locator.read",
    phase: "Governed-Entity-W2-0",
    method: "repo.entity.locator.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ locatorKind: "string", locatorValue: "string" }),
    }),
    guiBridgeMethod: "readEntityLocator",
    httpMethod: "POST",
    path: "/api/entities/locator",
    inputSchemaId: "gui.entity-locator-read/v1",
    outputSchemaId: DAEMON_ENTITY_LOCATOR_READ_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readEntityLocator",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    // Owned content, addressed by entity identity rather than by a repository path: the bindings an entity was
    // accepted with are authored-root relative, and the authored root is configurable, so no caller can be
    // asked to assemble that path itself.
    id: "entity.content.read",
    phase: "Governed-Entity-W2-0",
    method: "repo.entity.content.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ entityKind: "string", entityId: "string", path: "string?" }),
    }),
    guiBridgeMethod: "readEntityContent",
    httpMethod: "POST",
    path: "/api/entities/content",
    inputSchemaId: "gui.entity-content-read/v1",
    outputSchemaId: DAEMON_ENTITY_CONTENT_READ_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readEntityContent",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "workspace.summary.read",
    phase: "W2-GUI",
    method: "repo.workspace.summary.read",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }) }),
    guiBridgeMethod: "getWorkspaceSummary",
    httpMethod: "GET",
    path: "/api/workspace/summary",
    inputSchemaId: "gui.empty/v1",
    outputSchemaId: DAEMON_WORKSPACE_SUMMARY_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readWorkspaceSummary",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "workspace.scope.read",
    phase: "W2-GUI",
    method: "repo.workspace.scope.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ rootTaskId: "string", limit: "number?", cursor: "string?" }),
    }),
    guiBridgeMethod: "getWorkspaceScope",
    httpMethod: "GET",
    path: "/api/workspace/scope",
    inputSchemaId: "gui.workspace-scope-query/v1",
    outputSchemaId: DAEMON_WORKSPACE_SCOPE_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readWorkspaceScope",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "agenda.read",
    phase: "W3",
    method: "repo.agenda.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: daemonAgendaPayloadShape,
    }),
    guiBridgeMethod: "getAgenda",
    httpMethod: "GET",
    path: "/api/agenda",
    inputSchemaId: "gui.agenda-query/v1",
    outputSchemaId: DAEMON_AGENDA_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readAgenda",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "triadic.relationGraph",
    phase: "W2-GUI",
    method: "repo.triadic.relationGraph",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: daemonRelationQueryPayloadShape,
    }),
    guiBridgeMethod: "getRelationGraph",
    httpMethod: "GET",
    path: "/api/triadic/relation-graph",
    inputSchemaId: "gui.relation-query/v1",
    outputSchemaId: DAEMON_RELATION_GRAPH_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readRelationGraph",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "decisions.list",
    phase: "W2-GUI",
    method: "repo.decisions.list",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }), payload: daemonDecisionListPayloadShape }),
    guiBridgeMethod: "getDecisions",
    httpMethod: "GET",
    path: "/api/decisions",
    inputSchemaId: "gui.decision-projection/v1",
    outputSchemaId: DAEMON_DECISION_LIST_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "listDecisions",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "tasks.document.read",
    phase: "DocSync-B",
    method: "repo.tasks.document.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ taskId: "string", path: "string" }),
    }),
    guiBridgeMethod: "getTaskDocument",
    httpMethod: "GET",
    path: "/api/tasks/:taskId/documents/:path",
    inputSchemaId: "gui.task-document/v1",
    outputSchemaId: DAEMON_DOCUMENT_READ_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readDocument",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "tasks.completion.read",
    phase: "W3",
    method: "repo.tasks.completion.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ taskId: "string" }),
    }),
    guiBridgeMethod: "getTaskCompletion",
    httpMethod: "GET",
    path: "/api/tasks/:taskId/completion",
    inputSchemaId: "gui.task-completion/v1",
    outputSchemaId: DAEMON_TASK_COMPLETION_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readTaskCompletion",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "tasks.documents.list",
    phase: "DocSync-B",
    method: "repo.tasks.documents.list",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ taskId: "string" }),
    }),
    guiBridgeMethod: "getTaskDocuments",
    httpMethod: "GET",
    path: "/api/tasks/:taskId/documents",
    inputSchemaId: "gui.task-document-list/v1",
    outputSchemaId: DAEMON_TASK_DOCUMENT_LIST_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "listTaskDocuments",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "artifacts.list",
    phase: "DocSync-B",
    method: "repo.artifacts.list",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: daemonArtifactsListPayloadShape,
    }),
    guiBridgeMethod: "listArtifacts",
    httpMethod: "GET",
    path: "/api/artifacts",
    inputSchemaId: "gui.artifacts-list/v2",
    outputSchemaId: DAEMON_ARTIFACTS_LIST_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "listArtifacts",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "agentRuntime.overview",
    phase: "Runtime-B",
    method: "repo.agentRuntime.overview",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ taskId: "string?", limit: "number?", cursor: "string?" }),
    }),
    guiBridgeMethod: "getAgentRuntimeOverview",
    httpMethod: "GET",
    path: "/api/agent-runtime",
    inputSchemaId: "gui.agent-runtime-overview/v1",
    outputSchemaId: DAEMON_AGENT_RUNTIME_OVERVIEW_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readAgentRuntimeOverview",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "agentRuntime.sessions.read",
    phase: "Runtime-B",
    method: "repo.agentRuntime.sessions.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ runtimeSessionId: "string?", taskId: "string?", dispatchId: "string?" }),
    }),
    guiBridgeMethod: "getAgentRuntimeSession",
    httpMethod: "GET",
    path: "/api/agent-runtime/sessions/:id",
    inputSchemaId: "gui.agent-runtime-session/v1",
    outputSchemaId: DAEMON_AGENT_RUNTIME_SESSION_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readAgentRuntimeSession",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "agentRuntime.events.read",
    phase: "Runtime-B",
    method: "repo.agentRuntime.events.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ runtimeSessionId: "string", afterCursor: "string" }),
    }),
    guiBridgeMethod: "getAgentRuntimeEvents",
    httpMethod: "GET",
    path: "/api/agent-runtime/sessions/:id/events",
    inputSchemaId: "gui.agent-runtime-events/v1",
    outputSchemaId: DAEMON_AGENT_RUNTIME_EVENTS_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readAgentRuntimeEvents",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    // Per-agent (and per-squad) consumption aggregate over dispatch stream metrics: totals,
    // time buckets and one row per member for the requested range (daemon-local calendar).
    id: "agentRuntime.tokenUsage",
    phase: "Runtime-B",
    method: "repo.agentRuntime.tokenUsage",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: daemonTokenUsagePayloadShape,
    }),
    guiBridgeMethod: "getAgentRuntimeTokenUsage",
    httpMethod: "GET",
    path: "/api/agent-runtime/token-usage",
    inputSchemaId: "gui.agent-runtime-token-usage/v1",
    outputSchemaId: DAEMON_AGENT_RUNTIME_TOKEN_USAGE_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readAgentRuntimeTokenUsage",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    // The member-scoped companion: the same window filtered to one agent or squad, with one
    // row per dispatch so the detail page can link consumption to sessions and tasks.
    id: "agentRuntime.tokenUsageDetail",
    phase: "Runtime-B",
    method: "repo.agentRuntime.tokenUsageDetail",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: daemonTokenUsageDetailPayloadShape,
    }),
    guiBridgeMethod: "getAgentRuntimeTokenUsageDetail",
    httpMethod: "GET",
    path: "/api/agent-runtime/token-usage/detail",
    inputSchemaId: "gui.agent-runtime-token-usage-detail/v1",
    outputSchemaId: DAEMON_AGENT_RUNTIME_TOKEN_USAGE_DETAIL_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readAgentRuntimeTokenUsageDetail",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "task.dispatches",
    phase: "Runtime-B",
    method: "repo.task.dispatches",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({
        taskId: "string?",
        taskIds: "array?",
        limit: "number?",
        cursor: "string?",
      }),
    }),
    guiBridgeMethod: "getTaskDispatches",
    httpMethod: "GET",
    path: "/api/tasks/:taskId/dispatches",
    inputSchemaId: "gui.task-dispatches/v1",
    outputSchemaId: DAEMON_TASK_DISPATCHES_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readTaskDispatches",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "agent.entities.list",
    phase: "Runtime-B",
    method: "repo.agent.entities.list",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }) }),
    guiBridgeMethod: "listAgents",
    httpMethod: "GET",
    path: "/api/agents",
    inputSchemaId: "gui.empty/v1",
    outputSchemaId: DAEMON_AGENT_ENTITY_CATALOG_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "listAgents",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "agent.entity.read",
    phase: "Runtime-B",
    method: "repo.agent.entity.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ agentId: "string" }),
    }),
    guiBridgeMethod: "showAgent",
    httpMethod: "GET",
    path: "/api/agents/:agentId",
    inputSchemaId: "gui.agent-entity-read/v1",
    outputSchemaId: DAEMON_AGENT_ENTITY_DETAIL_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "showAgent",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "agent.skills.list",
    phase: "Runtime-B",
    method: "repo.agent.skills.list",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }) }),
    guiBridgeMethod: "listAgentSkills",
    httpMethod: "GET",
    path: "/api/agent-skills",
    inputSchemaId: "gui.empty/v1",
    outputSchemaId: DAEMON_AGENT_SKILL_CATALOG_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "listAgentSkills",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "squad.entities.list",
    phase: "Runtime-B",
    method: "repo.squad.entities.list",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }) }),
    guiBridgeMethod: "listSquads",
    httpMethod: "GET",
    path: "/api/squads",
    inputSchemaId: "gui.empty/v1",
    outputSchemaId: DAEMON_SQUAD_ENTITY_CATALOG_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "listSquads",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "squad.entity.read",
    phase: "Runtime-B",
    method: "repo.squad.entity.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ squadId: "string" }),
    }),
    guiBridgeMethod: "showSquad",
    httpMethod: "GET",
    path: "/api/squads/:squadId",
    inputSchemaId: "gui.squad-entity-read/v1",
    outputSchemaId: DAEMON_SQUAD_ENTITY_DETAIL_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "showSquad",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "squad.runs.list",
    phase: "Runtime-B",
    method: "repo.squad.runs.list",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ since: "string?", query: "string?", limit: "number?" }),
    }),
    guiBridgeMethod: "listSquadRuns",
    httpMethod: "GET",
    path: "/api/squad-runs",
    inputSchemaId: "gui.squad-run-query/v1",
    outputSchemaId: DAEMON_SQUAD_RUN_LIST_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "listSquadRuns",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "squad.run.read",
    phase: "Runtime-B",
    method: "repo.squad.run.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({ squadRunId: "string" }),
    }),
    guiBridgeMethod: "readSquadRun",
    httpMethod: "GET",
    path: "/api/squad-runs/:id",
    inputSchemaId: "gui.squad-run-read/v1",
    outputSchemaId: DAEMON_SQUAD_RUN_READ_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "readSquadRun",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "gui.catalog.snapshot",
    phase: "W5-GUI-S3",
    method: "repo.gui.catalog.snapshot",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }) }),
    guiBridgeMethod: "getCatalogSnapshot",
    httpMethod: "GET",
    path: "/api/catalog",
    inputSchemaId: "gui.empty/v1",
    outputSchemaId: GUI_CATALOG_SNAPSHOT_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "getCatalogSnapshot",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "gui.catalog.preset.read",
    phase: "W5-GUI-S3",
    method: "repo.gui.catalog.preset.read",
    requiresRepo: true,
    params: shape({
      repo: shape({ repoId: "string" }),
      payload: shape({
        presetId: "string",
        profileId: "string?",
        locale: "string?",
      }),
    }),
    guiBridgeMethod: "getCatalogPreset",
    httpMethod: "GET",
    path: "/api/catalog/presets/:presetId",
    inputSchemaId: "gui.catalog-preset/v1",
    outputSchemaId: GUI_CATALOG_PRESET_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "getCatalogPreset",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
  {
    id: "terminal.sessions.list",
    phase: "W5-GUI-S3",
    method: "repo.terminal.sessions.list",
    requiresRepo: true,
    params: shape({ repo: shape({ repoId: "string" }) }),
    guiBridgeMethod: "listTerminalSessions",
    httpMethod: "GET",
    path: "/api/terminal/sessions",
    inputSchemaId: "gui.empty/v1",
    outputSchemaId: TERMINAL_SESSION_LIST_SCHEMA.id,
    errorSchemaId: DAEMON_PROTOCOL_ERROR_SCHEMA.id,
    serviceMethod: "listTerminalSessions",
    auth: "local-session-token",
    commandClass: "repo-read",
  },
] as const satisfies ReadonlyArray<{
  readonly method: DaemonGuiRpcReadMethod;
  readonly [key: string]: unknown;
}>);
