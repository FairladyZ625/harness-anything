...
export function createJsonRpcProtocolServer(options: { readonly host: DaemonHost; readonly authContext: DaemonAuthenticationContext; readonly emit: (method: string, params: JsonObject) => Promise<void>; readonly recordRequest?: (entry: DaemonRequestLogEntry) => void }): JsonRpcProtocolServer {
  ...
  const one = async (request: JsonRpcRequest): Promise<JsonRpcResponse | undefined> => {
    ...
    if (request.method === "daemon.fleet.task.run") { try { if (options.authContext.transportKind !== "unix-socket" || options.authContext.assignmentBinding) throw Object.assign(new Error("This control is available only through the local session token."), { code: "local_transport_required" }); // Loaded lazily: schema-closure imports this module in a zero-dependency checkout, and the fleet edge stack reaches the kernel barrel.
