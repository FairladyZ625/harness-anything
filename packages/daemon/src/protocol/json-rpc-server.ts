...
import { validateRuntimeCredentialReceipt } from "../gui-s3-control.ts";
...
export function createJsonRpcProtocolServer(options: { readonly host: DaemonHost; readonly authContext: DaemonAuthenticationContext; readonly emit: (method: string, params: JsonObject) => Promise<void> }): JsonRpcProtocolServer {
  ...
  const one = async (request: JsonRpcRequest): Promise<JsonRpcResponse | undefined> => {
    ...
    if (request.method === "daemon.agentRuntime.credentials.bind") { try { const result = await options.host.bindRuntimeCredential(params, options.authContext), errors = validateRuntimeCredentialReceipt(result); if (errors.length) throw Object.assign(new Error(errors.join("; ")), { code: "invalid_credential_receipt" }); return reply(result as JsonObject); } catch (error) { return reply(daemonProtocolError(request.method, rpcServerErrorCode(error), error instanceof Error ? error.message : String(error)) as unknown as JsonObject); } }
