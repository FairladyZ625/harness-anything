...
/**
 * Durable routes all reach the same port, so the only real question is whether an Action is in
 * the durable inventory — and the Policy already owns that inventory. Asking it directly keeps
 * one list; a hand-kept copy here would silently drift every time an Action is added, and the
 * drift only surfaces at runtime as actor_unauthorized.
 */
export function authorizeDurableRepoCellAction(
  input: Parameters<typeof authorizeRepoCellAction>[0],
): AuthorizationDecision | null {
  return (durablePolicyActions as readonly string[]).includes(input.action.kind)
    ? authorizeRepoCellAction(input)
    : null;
}
