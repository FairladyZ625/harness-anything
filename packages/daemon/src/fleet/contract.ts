...
export type FleetFrameV1 =
  ...
  | Msg<"fleet.assignment.result/v1", { inReplyTo: string; assignmentId: string; repoId: string; taskId: string; executionId: string; paths: readonly string[]; baseLedgerSha: string; expiresAt: string; writerEpoch: number }>
  ...
  | Msg<"fleet.doc.submit/v1", { assignmentId: string; executionId: string | null; writerEpoch: number; baseLedgerSha: string; changes: readonly FleetDocChange[] }>
...
const cut = shape({ revision: uint, headDigest: (value) => typeof value === "string" && /^sha256:[0-9a-f]{64}$/u.test(value) }), mirrorBaseCutShape = cut;
...
const schemas: Readonly<Record<string, Check>> = {
  ...
  "fleet.assignment.get/v1": shape({ ...common, assignmentId: id }), "fleet.assignment.result/v1": shape({ ...reply, assignmentId: id, repoId: id, taskId: id, executionId: id, paths: array(logicalPath), baseLedgerSha: sha40, expiresAt: (value) => typeof value === "string" && !Number.isNaN(Date.parse(value)), writerEpoch: uint }), "fleet.receipt.get/v1": shape({ ...common, assignmentId: id, opId: id }), "fleet.receipt.result/v1": shape({ ...reply, opId: id, receipt: record }),
  ...
  "fleet.doc.submit/v1": shape({ ...common, assignmentId: id, executionId: nullable(id), writerEpoch: uint, baseLedgerSha: sha40, changes: array(docChange) }), "fleet.doc.result/v1": shape({ ...reply, outcome: one("applied", "pending", "op_rejected", "indeterminate"), opId: text, revision: nullable(uint), code: nullable(text) }), "fleet.task.command/v1": shape({ ...common, assignmentId: id, writerEpoch: uint, opId: id, repoId: id, taskId: nullable(id), action: taskAction, waitMs: uint, docChanges: nullable(array(docChange, 128)), mirrorBaseCut: nullable(mirrorBaseCutShape) }), "fleet.task.result/v1": shape({ ...reply, outcome: one("applied", "op_rejected", "wait_expired"), opId: id, revision: nullable(uint), code: nullable(text), receipt: nullable(record), lease: nullable(taskLease), queuePosition: nullable(uint) }), "fleet.replica.pull/v1": shape({ ...common, assignmentId: id }), "fleet.replica.current/v1": shape({ ...reply, repoId: id, viewId: id, cut, manifestDigest: sha64 }),
