...
export type FleetFrameV1 =
  ...
  | Msg<"fleet.runtime.read/v1", { assignmentId: string; repoId: string; method: "repo.agentRuntime.sessions.read"; payload: Readonly<Record<string, unknown>> }>
...
const schemas: Readonly<Record<string, Check>> = {
  ...
  "fleet.runtime.event/v1": shape({ ...common, assignmentId: id, writerEpoch: uint, repoId: id, opId: id, eventType: runtimeEventType, payload: record, result: nullable(descriptor) }), "fleet.runtime.event.result/v1": shape({ ...reply, event: record, receipt: record }), "fleet.runtime.archive/v1": shape({ ...common, assignmentId: id, writerEpoch: uint, repoId: id, archive: record }), "fleet.runtime.archive.result/v1": shape({ ...reply, receipt: record }), "fleet.runtime.read/v1": shape({ ...common, assignmentId: id, repoId: id, method: one("repo.agentRuntime.sessions.read"), payload: record }), "fleet.runtime.read.result/v1": shape({ ...reply, result: record }),
