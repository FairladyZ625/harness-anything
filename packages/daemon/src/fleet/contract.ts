...
const schemas: Readonly<Record<string, Check>> = {
  ...
  "fleet.assignment.get/v1": shape({ ...common, assignmentId: id }), "fleet.assignment.result/v1": shape({ ...reply, assignmentId: id, repoId: id, taskId: id, executionId: id, paths: array(logicalPath), baseLedgerSha: sha40, expiresAt: (value) => typeof value === "string" && !Number.isNaN(Date.parse(value)), writerEpoch: uint }),
