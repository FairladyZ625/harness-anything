...
export type FleetFrameV1 =
  ...
  | Msg<"fleet.doc.result/v1", { inReplyTo: string; outcome: "applied" | "pending" | "rejected" | "indeterminate"; opId: string; revision: number | null; code: string | null }>
  ...
  | Msg<"fleet.ack.result/v1", { inReplyTo: string; outcome: "applied" | "current" | "rejected"; viewId: string; ackCut: number; code: string | null }>
...
const schemas: Readonly<Record<string, Check>> = {
  ...
  "fleet.doc.submit/v1": shape({ ...common, assignmentId: id, baseLedgerSha: sha40, changes: array(docChange) }), "fleet.doc.result/v1": shape({ ...reply, outcome: one("applied", "pending", "rejected", "indeterminate"), opId: text, revision: nullable(uint), code: nullable(text) }), "fleet.replica.pull/v1": shape({ ...common, assignmentId: id }), "fleet.replica.current/v1": shape({ ...reply, repoId: id, viewId: id, cut, manifestDigest: sha64 }),
  ...
  "fleet.ack/v1": shape({ ...common, transferId: id, cut, manifestDigest: sha64 }), "fleet.ack.result/v1": shape({ ...reply, outcome: one("applied", "current", "rejected"), viewId: id, ackCut: uint, code: nullable(text) }), "fleet.error/v1": shape({ ...reply, code: text, retryable: (value) => typeof value === "boolean", resumeOffset: nullable(uint), nextAction: text })
};
