...
export function validateAgentRuntimeAttachEvent(value: unknown): readonly string[] { if (!isRuntimeStreamRecord(value) || value.schema !== "agent-runtime-attach-event/v1" || typeof value.runtimeSessionId !== "string" || !/^stream:\d+$/u.test(String(value.cursor)) || typeof value.occurredAt !== "string" || !["activity", "heartbeat", "exit", "error", "gap"].includes(String(value.type))) return ["agent runtime attach event is invalid"]; const field = value.type === "activity" ? "activity" : value.type === "exit" ? "outcome" : value.type === "error" ? "code" : value.type === "gap" ? "required" : null, allowed = value.type === "activity" ? ["thinking", "tool", "message"].includes(String(value.activity)) : value.type === "exit" ? ["succeeded", "failed", "unknown"].includes(String(value.outcome)) : value.type === "error" ? ["provider_disconnected", "provider_protocol_error"].includes(String(value.code)) : value.type === "gap" ? value.required === "snapshot" : true; return allowed && hasExactRuntimeStreamFields(value, ["schema", "type", "runtimeSessionId", "cursor", "occurredAt", ...(field ? [field] : [])]) ? [] : ["agent runtime attach event is invalid"]; }
