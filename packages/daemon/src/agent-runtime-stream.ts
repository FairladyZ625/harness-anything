...
export type AgentRuntimeNativeSignal =
  | { readonly type: "activity"; readonly activity: AgentRuntimeActivity }
  | { readonly type: "heartbeat" }
  | { readonly type: "exit"; readonly outcome: "succeeded" | "failed" | "unknown" }
...
function parseCursor(value: string): number { const match = /^stream:(\d+)$/u.exec(value), sequence = match ? Number(match[1]) : Number.NaN; if (!Number.isSafeInteger(sequence)) throw coded("invalid_cursor", `Invalid attach cursor: ${value}.`); return sequence; }
...
export function validateAgentRuntimeAttach(value: unknown): readonly string[] { if (!record(value) || typeof value.ok !== "boolean" || typeof value.runtimeSessionId !== "string") return ["agent runtime attach result is invalid"]; if (!value.ok) return exact(value, ["ok", "code", "runtimeSessionId", "hint"]) && value.code === "unsupported" && typeof value.hint === "string" ? [] : ["agent runtime unsupported result is invalid"]; return exact(value, ["ok", "status", "runtimeSessionId", "cursor", "events"]) && ["attached", "gap"].includes(String(value.status)) && /^stream:\d+$/u.test(String(value.cursor)) && Array.isArray(value.events) && value.events.every((event) => validateAgentRuntimeAttachEvent(event).length === 0) ? [] : ["agent runtime attach result is invalid"]; }
export function validateAgentRuntimeAttachEvent(value: unknown): readonly string[] { if (!record(value) || value.schema !== "agent-runtime-attach-event/v1" || typeof value.runtimeSessionId !== "string" || !/^stream:\d+$/u.test(String(value.cursor)) || typeof value.occurredAt !== "string" || !["activity", "heartbeat", "exit", "error", "gap"].includes(String(value.type))) return ["agent runtime attach event is invalid"]; const field = value.type === "activity" ? "activity" : value.type === "exit" ? "outcome" : value.type === "error" ? "code" : value.type === "gap" ? "required" : null, allowed = value.type === "activity" ? ["thinking", "tool", "message"].includes(String(value.activity)) : value.type === "exit" ? ["succeeded", "failed", "unknown"].includes(String(value.outcome)) : value.type === "error" ? ["provider_disconnected", "provider_protocol_error"].includes(String(value.code)) : value.type === "gap" ? value.required === "snapshot" : true; return allowed && exact(value, ["schema", "type", "runtimeSessionId", "cursor", "occurredAt", ...(field ? [field] : [])]) ? [] : ["agent runtime attach event is invalid"]; }
