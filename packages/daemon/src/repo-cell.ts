...
import { assertCurrentWriter, bindWriterGenerationToken, makeTaskEventStore, makeTaskProjection, normalizeCommandEnvelope,
  normalizeTaskLifecycleCommand, queryDecisionProjection, readRelationGraphProjection, buildRelationGraphProjection, type ActorIdentity, type CompleteTaskCommand, type EventPublicationKillpoint, type ProofFor,
...
export async function openRepoCell(input: { readonly repoId: WorkspaceId; readonly rootDir: CanonicalRoot; readonly ownerId: string;
  ...
  const factActions = makeFactActions({ store, projection, now, killpoint: input.killpoint });
  ...
  const readHandlers = { "repo.tasks.list": () => ({ ok: true, ...projection.list() }), "repo.triadic.relationGraph": () => { const cached = readRelationGraphProjection({ rootDir }), factGraph = projection.readFactGraph(), authored = buildRelationGraphProjection({ rootDir }, factGraph.factAnchors); return { ok: true as const, ...authored, edges: [...authored.edges, ...factGraph.edges], facts: factGraph.facts, warnings: cached.warnings }; },
