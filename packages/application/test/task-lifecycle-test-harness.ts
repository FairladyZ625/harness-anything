  ...
  return {
    ...
    create: (opId = "op-create") => {
      ...
      return service.execute({
        type: "CreateReplayTask", taskId: "task-1", title: "Replay task", graph: replayGraph,
        completionGateIds: [], actor: owner, opId, eventId: `event-${opId}`, workspaceRevision: next, occurredAt: at(next)
      }, { taskIdUnique: true, actorBinding: owner });
    },
    start: (executionId = `execution-${revision() + 1}`, opId = `op-start-${revision() + 1}`) => {
      const next = revision() + 1;
      return service.execute({
        type: "StartExecution", taskId: "task-1", executionId, actor: owner,
        opId, eventId: `event-${opId}`, workspaceRevision: next, occurredAt: at(next)
      }, {
        actorBinding: owner, expectedRevision: next - 1,
        reservation: { taskId: "task-1", executionId, credentialHash: `credential-${executionId}`, expiresAt: "2026-08-11T01:00:00.000Z", version: 0 }
      });
    },
    submit: (executionId: string, opId = `op-submit-${revision() + 1}`, claim = "implemented") => {
      const next = revision() + 1;
      return service.execute({
        type: "SubmitExecution", taskId: "task-1", executionId, actor: owner,
        opId, eventId: `event-${opId}`, workspaceRevision: next, occurredAt: at(next),
        submission: { claim, deliverables: [], evidenceRefs: [], verification: ["tests"], knownGaps: [], residualRisks: [], commitSha }
      }, { expectedRevision: next - 1, credentialHash: `credential-${executionId}`, sessionDisposition: "complete" });
    },
    review: async (executionId: string, kind: "anti_entropy" | "acceptance", verdict: "approved" | "changes_requested" | "dismissed", opId = `op-review-${revision() + 1}`) => {
      ...
      return service.execute({
        ...
        actor: reviewer, opId, eventId: `event-${opId}`, workspaceRevision: next, occurredAt: at(next)
      }, {
    ...
    complete: (executionId: string, opId = `op-complete-${revision() + 1}`) => {
      const next = revision() + 1;
      return service.execute({
        type: "CompleteTask", taskId: "task-1", executionId, actor: owner,
        opId, eventId: `event-${opId}`, workspaceRevision: next, occurredAt: at(next)
      }, {
