...
const command = <C extends Parameters<typeof normalizeTaskLifecycleCommand>[1]>(
  rootDir: string,
  intent: C,
  meta: { readonly eventId: string; readonly workspaceRevision: number; readonly occurredAt: string },
  expectedRevision = meta.workspaceRevision - 1,
) => ({
  ...normalizeTaskLifecycleCommand({ workspaceId: rootDir, actor, source: "local", expectedRevision }, intent),
  ...meta,
});
  ...
  try {
    ...
    const verified = reduceTaskEvent(snapshot, compiled.event),
      complete = command(
        ...
        { eventId: "event-complete", workspaceRevision: verified.revision + 1, occurredAt: "2026-08-11T00:11:00.000Z" },
        verified.revision,
      ),
      proof = {
        capability: "task-complete@v1" as const,
    ...
    const withFailedHistory = {
      ...verified,
      gateWitnesses: [
        {
          ...compiled.event.payload.witness,
          witnessId: "gate-op-ci-fail",
          receiptId: "op-ci-fail",
          result: "fail",
        } as unknown as typeof compiled.event.payload.witness,
        ...verified.gateWitnesses,
      ],
    };
    assert.deepEqual(
      withFailedHistory.gateWitnesses.map(({ result }) => result),
      ["fail", "pass"],
    );
    assert.doesNotThrow(() => applyTransition(withFailedHistory, complete, proof));
    assert.throws(
      () =>
        applyTransition(verified, complete, {
          ...proof,
          gateReceipts: [{ ...proof.gateReceipts[0]!, receiptRef: "event:forged" }],
        }),
      /L2-verified/u,
    );
    assert.throws(
      () => reduceTaskEvent(snapshot, { ...compiled.event, opId: "op-tampered" }),
...
test("transition service freezes targets and makes create/start idempotent by opId payload", async () => {
  ...
  try {
    ...
    const create = command(
      ...
      { eventId: "event-create", workspaceRevision: 1, occurredAt: "2026-08-11T00:00:00.000Z" },
    );
...
test("transition service publishes aggregate-authored task status idempotently", async () => {
  ...
  try {
    ...
    const first = await service.execute(block, {}),
      second = await service.execute(block, {});
    assert.equal(first.event?.type, "task_transitioned");
    assert.deepEqual(second, first);
    const unblock = command(
      rootDir,
      { type: "TransitionTask" as const, taskId: "task-1", status: "active" as const, reason: "", force: true },
      { eventId: "event-unblock", workspaceRevision: 3, occurredAt: "2026-08-11T00:02:00.000Z" },
    );
    const unblocked = await service.execute(unblock, {}),
      retried = await service.execute(unblock, {});
    assert.equal(unblocked.snapshot.task?.status, "active");
    assert.equal(unblocked.event?.type, "task_transitioned");
    assert.equal(unblocked.event?.payload.mutation.reason, "Explicit lifecycle transition to active");
    assert.deepEqual(retried, unblocked);
...
test("transition republishes a historical task without its retired longRunning metadata", async () => {
  ...
  try {
    initRepo(rootDir);
    const historical = oldTaskEvent(1),
      metadata = {
        idempotencyKey: null,
        parentTaskId: null,
    ...
    const receipt = await service.execute(block, {}),
      written = eventStore.readTaskEvent(block.opId);
    assert.equal(receipt.outcome, "applied");
    assert.equal(receipt.snapshot.task?.status, "blocked");
    assert.equal(Object.hasOwn(receipt.snapshot.task?.metadata ?? {}, "longRunning"), false);
    assert.equal(
      written?.schema === "task-event/v1" && Object.hasOwn(written.payload.task.metadata ?? {}, "longRunning"),
      false,
    );
    const contract = projection.readDocument("tasks/task-legacy-legacy-task/task-contract.json").document;
    assert.ok(contract);
    assert.equal(Object.hasOwn(JSON.parse(contract.body).metadata, "longRunning"), false);
  } finally {
    projection?.close();
    rmSync(rootDir, { recursive: true, force: true, maxRetries: 5, retryDelay: 20 });
  }
  ...
  try {
    const transition = (status: "planned" | "active" | "in_review" | "cancelled", reason: string, force = false) => {
      ...
      return harness.service.execute(
        command(
          harness.rootDir,
          { type: "TransitionTask" as const, taskId: "task-1", status, reason, force },
          {
            eventId: `event-reinstate-${revision}`,
...
test("terminal lifecycle states clear a prior task pin", async () => {
  ...
  try {
    ...
    const cancelled = applyTransition(
      { ...created.snapshot, task: { ...created.snapshot.task!, pinned: true } },
      command(
        harness.rootDir,
        {
          type: "TransitionTask" as const,
          ...
          reason: "No longer being worked",
          force: true,
        },
        {
          eventId: "event-cancel-pin",
          workspaceRevision: created.snapshot.revision + 1,
    ...
    const completed = applyTransition(
      { ...consented.snapshot, task: { ...consented.snapshot.task!, pinned: true } },
      command(
        harness.rootDir,
        {
        ...
        {
          eventId: "event-complete-pin",
          workspaceRevision: consented.snapshot.revision + 1,
          occurredAt: "2026-08-11T00:10:00.000Z",
        },
        consented.snapshot.revision,
      ),
      {
...
test("second distinct task create uses aggregate revision zero in a non-empty workspace", async () => {
  ...
  try {
    ...
    projection = makeTaskProjection({ rootDir, eventStore, now: () => "2026-08-11T00:30:00.000Z" });
    const service = makeTaskLifecycleService({ eventStore, projection });
    const create = (taskId: string, title: string, revision: number) =>
      command(
        rootDir,
        {
          type: "CreateReplayTask" as const,
        ...
        {
          eventId: `event-create-${revision}`,
          workspaceRevision: revision,
          occurredAt: `2026-08-11T00:0${revision}:00.000Z`,
        },
        0,
      );
    await service.execute(create("task-1", "First task", 1), { taskIdUnique: true, actorBinding: actor });
    const second = await service.execute(create("task-2", "Second task", 2), {
      taskIdUnique: true,
      actorBinding: actor,
    });
    ...
    await assert.rejects(() =>
      service.execute(create("task-2", "Duplicate task", 3), { taskIdUnique: true, actorBinding: actor }),
    );
  } finally {
    projection?.close();
...
test("pending without an event uses an honest receipt", async () => {
  ...
  const service = makeTaskLifecycleService({
    eventStore: {
      ...
      append: (candidate) => {
        appendCalls += 1;
        return { status: "applied" as const, event: candidate.event, revision: candidate.event.workspaceRevision };
      },
    },
    projection: {
      read: () => read,
  ...
  const start = command(
    "workspace",
    { type: "StartExecution" as const, taskId: "task-pending", executionId: "execution-pending" },
...
test("10,000 old events do not block a new write", async (context) => {
  ...
  try {
    ...
    const samples: Record<ReadMode, Awaited<ReturnType<typeof independentWrite>>[]> = {
        bounded: [],
        "whole-history": [],
      },
      ratios: number[] = [];
    ...
    for (let round = 1; round <= 5; round += 1) {
      ...
      let boundedArm: Awaited<ReturnType<typeof independentWrite>> | undefined,
        wholeHistoryArm: Awaited<ReturnType<typeof independentWrite>> | undefined;
      for (const mode of order) {
        revision += 1;
        const arm = await independentWrite(historyRoot, revision, `${round}-${mode}`, mode);
        samples[mode].push(arm);
        if (mode === "bounded") boundedArm = arm;
        else wholeHistoryArm = arm;
        context.diagnostic(
      ...
      assert.equal(boundedArm.fullHistoryReads, 0, "the bounded path must not read the whole event history");
      if (wholeHistoryArm.fullHistoryReads > 0)
        assert.equal(
          wholeHistoryArm.fullHistoryReads > boundedArm.fullHistoryReads,
          true,
    ...
    const describe = (values: readonly number[]) =>
      ...
      metric = (mode: ReadMode, key: "elapsedMs" | "storeInitMs" | "readMs" | "appendMs" | "applyMs") =>
        samples[mode].map((sample) => sample[key]);
    for (const mode of ["bounded", "whole-history"] as const)
      context.diagnostic(
        `independent-write-samples mode=${mode} samples=${samples[mode].length} total(${describe(metric(mode, "elapsedMs"))}) storeInit(${describe(metric(mode, "storeInitMs"))}) read(${describe(metric(mode, "readMs"))}) append(${describe(metric(mode, "appendMs"))}) apply(${describe(metric(mode, "applyMs"))})`,
      );
    const orderedRatios = [...ratios].sort((left, right) => left - right),
      ratio = median(ratios);
    context.diagnostic(
      `independent-write-ratio=paired-whole-history-over-bounded samples=${ratios.length} p50=${ratio.toFixed(3)}x min=${orderedRatios[0]!.toFixed(3)}x max=${orderedRatios.at(-1)!.toFixed(3)}x`,
    );
  } finally {
...
async function independentWrite(rootDir: string, revision: number, label: string, mode: ReadMode) {
  ...
  const service = makeTaskLifecycleService({
    eventStore: {
      ...
      append: (candidate) => {
        ...
        try {
          return eventStore.append(candidate);
        } finally {
          phase.appendMs += performance.now() - phaseStarted;
        }
      },
    },
    projection: {
  ...
  const create = command(
    ...
    {
      type: "CreateReplayTask" as const,
      taskId: `task-new-${label}`,
      title: "New independent task",
      taskClass: "standard" as const,
      graph: replayGraph,
      completionGateIds: [],
      presetSnapshotDigest: null,
    },
    { eventId: `event-new-${label}`, workspaceRevision: revision, occurredAt: "2026-08-12T00:00:00.000Z" },
    0,
  );
  const receipt = await service.execute(create, { taskIdUnique: true, actorBinding: actor });
  projection.close();
...
test("transition service replays reject through a new Execution before completion", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      completed.snapshot.executions.map((execution) => execution.state),
...
test("second lifecycle claim uses monotonic lease CAS", async () => {
  ...
  try {
    ...
    if (first.type !== "execution_started" || second.type !== "execution_started")
      throw new Error("fixture requires claim events");
    assert.equal(second.payload.lease.version > first.payload.lease.version, true);
    assert.deepEqual(second.payload.previousHolder, {
      taskId: first.taskId,
      executionId: first.payload.execution.executionId,
      actor: first.actor,
      source: first.source,
    });
...
test("SQLite/response killpoints reconstruct the exact applied receipt by opId without another commit", async () => {
  for (const point of ["after_sqlite_commit", "before_response_write", "after_response_write"] as const) {
    ...
    try {
      ...
      projection = makeTaskProjection({ rootDir, eventStore, now: () => "2026-08-11T00:30:00.000Z" });
      let armed = true;
      const interrupted = makeTaskLifecycleService({
        eventStore,
        projection,
        killpoint: (candidate) => {
...
test("lease broker capacity ceiling rejects concurrent exhaustion and release restores availability", async () => {
  ...
  try {
    ...
    assert.throws(
      () => projection.reserveLease(lease("overflow"), "2026-08-11T00:00:00.000Z"),
      /capacity.*exhausted/iu,
    );
