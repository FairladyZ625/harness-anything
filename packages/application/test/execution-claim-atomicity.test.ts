...
test("claim leaves no lease before publication and converges from a committed event after response loss", async () => {
  ...
  try {
    ...
    assert.equal(harness.projection.currentLease("task-1"), null);
