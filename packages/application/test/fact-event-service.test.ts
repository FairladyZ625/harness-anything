...
test("Decision transition matrix, independent arbiter, claims, relation retirement, and replay are canonical", () => {
  ...
  for (const terminal of ["decision_rejected", "decision_deferred"] as const) withDecisionFixture(({ service, projection }) => { recordDecision(service, projection, decisionEvent(1, "decision_proposed")); assert.equal(recordDecision(service, projection, decisionEvent(2, terminal)).decision.state, terminal.slice(9)); });
});
