import { canonicalEventWritePlan, serializeCanonicalEvent, type CanonicalEventStore, type DecisionEventV1, type DecisionProjectionRow, type DecisionSearchFilters, type FactEventV1, type FactProjectionRow, type FactSearchFilters, type TaskProjection } from "../../kernel/src/index.ts";
...
export function makeFactService(options: { readonly eventStore: Pick<CanonicalEventStore, "append" | "readEvent">; readonly projection: Pick<TaskProjection, "admitFact" | "apply" | "readFact" | "searchFacts"> }) {
  const record = (event: FactEventV1): FactRecordResult => {
    ...
    const appended = options.eventStore.append({ event, plan: canonicalEventWritePlan(event, "fact/v1", event.taskId), blobs: [] });
    if (existing === null) options.projection.apply(event);
    ...
    return { status: "ready", fact: read.fact, revision: appended.revision, watermark: read.watermark };
  };
