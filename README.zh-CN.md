...
**你不需要先装好任何东西。** 把下面这段话交给一个在待接入项目里工作的编码 agent：

> 读取 https://github.com/FairladyZ625/harness-anything 的
> `skills/harness-install/SKILL.md`，按它把 Harness Anything 装进本项目。
> skill 里说要问我的每个决定，都先问我。

该 skill 会先读你的项目——什么语言、什么结构、有没有 CI、是不是已经有 `AGENTS.md`——
再据此初始化台账、挂上 daemon，并带着你把一条真实任务一路推到 `done`：装完是被证明的，
不是被假定的。凡是不好回头的地方它都会停下来问你：即将写入的 person id，以及任何需要与你
自己的文件合并的地方。

如果项目里**已经有**上一代的 `harness/` 台账，请改用下面的[迁移 skill](#破坏性变更已有仓库需要迁移)。
