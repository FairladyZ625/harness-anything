...
用户询问进度或追问原因时，先回答，再继续此前已授权的工作，除非用户明确暂停或改变目标。独立工作运行期间推进关键路径；对本轮选定且可执行的动作，实际执行或派发并核实启动，不能以“下一步会做”结束。使用受支持的完成事件或有界状态检查来回收、诊断、重派、集成或裁决。结束回合前区分已实际执行、在飞且有接手人、等待具体条件；计划文字、任务创建和派发受理均不单独证明已经启动。按[长程监管](references/supervision.md)执行每轮回收、验收、推进和交接。会话断开、重复失败、无 Git 边缘节点等情况见[派工与恢复](references/dispatch.md)。
