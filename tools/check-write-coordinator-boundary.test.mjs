...
test("W3 write authority accepts RepoCell as the sole production event-store composition root", () =>
  withFixture((root) => {
    assert.deepEqual(findW3WriteAuthorityViolations(root), []);
  }));

test("W3 write authority rejects restoration of a journal coordinator", () =>
  withFixture((root) => {
...
test("W3 write authority rejects a second event-store composition root", () =>
  withFixture((root) => {
...
test("W3 write authority rejects worktree mutation capability in the object/ref publisher", () =>
  withFixture((root) => {
...
test("W3 write authority rejects moving a coordinated-commit primitive outside its named function", () =>
  withFixture((root) => {
    write(
      root,
...
`,
    ...
    assert.match(
      findW3WriteAuthorityViolations(root).join("\n"),
      /prepareCommit must execute gitObjects\.importCommit/u,
    );
  }));

test("W3 write authority rejects an additional coordinated-commit primitive caller", () =>
  withFixture((root) => {
    write(
      root,
      "packages/kernel/src/store/alternate-commit.ts",
      "export function alternateCommitter() { gitObjects.importCommit(); }\n",
    );
    assert.match(findW3WriteAuthorityViolations(root).join("\n"), /primitive must belong only to prepareCommit/u);
  }));

test("W3 write authority rejects thin CLI domain imports by relative source path", () =>
  withFixture((root) => {
    write(
      root,
      "packages/cli/src/domain-import.ts",
      'import { service } from "../../../application/src/task-lifecycle-service.ts";\nexport { service };\n',
    );
    assert.match(findW3WriteAuthorityViolations(root).join("\n"), /thin CLI must not import kernel\/application/u);
  }));

test("W3 write authority rejects thin CLI domain imports by workspace package identity", () =>
  withFixture((root) => {
    write(
      root,
      "packages/cli/src/domain-import.ts",
      'import { service } from "@harness-anything/application/internal/task-lifecycle-service";\nexport { service };\n',
    );
    assert.match(findW3WriteAuthorityViolations(root).join("\n"), /thin CLI must not import kernel\/application/u);
  }));

function withFixture(run) {
  const root = mkdtempSync(path.join(tmpdir(), "w3-write-authority-"));
  try {
    write(
      root,
...
function publisherFixture(extra = "") {
  return `
