...
test("bypass write boundary governs daemon fs writes", () => {
  const root = makeFixtureRoot();
  const policyRoot = mkdtempSync(path.join(tmpdir(), "ha-w8-policy-"));
  try {
    writeFileSync(path.join(root, "packages/daemon/src/fixture.ts"), [
      "import { writeFileSync } from 'node:fs';",
      "export function bypass() {",
      "  writeFileSync('daemon-state.json', '{}', 'utf8');",
      "}"
    ].join("\n"), "utf8");
    writeAllowlist(policyRoot);
    ...
    const result = runChecker(root, policyRoot);
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /packages\/daemon\/src\/fixture\.ts#writeFileSync@1/u);
  } finally {
    rmSync(root, { recursive: true, force: true });
    rmSync(policyRoot, { recursive: true, force: true });
  }
