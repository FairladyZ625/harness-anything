...
const requiredCompilerOptions = Object.fromEntries(
  allowlist.requiredCompilerOptions.map((entry) => [entry.option, entry.value]),
);
...
if (rootPackage.dependencies?.effect !== "3.21.4")
  record("effect version must remain 3.21.4 after task_01KWNFNGG9H41724SADGFSMEZ3");
if (cliPackage.dependencies?.["@effect/platform"] !== "0.96.2")
  record("@effect/platform version must remain 0.96.2 after task_01KWNFNGG9H41724SADGFSMEZ3");
if (rootPackage.devDependencies?.typescript !== "5.9.3")
  record("typescript version must remain 5.9.3 until an explicit upgrade task");
...
if (!existsSync(path.join(root, "package-lock.json")))
  record("package-lock.json is required; npm is the package manager");
...
for (const forbiddenLockfile of forbiddenLockfiles) {
  if (existsSync(path.join(root, forbiddenLockfile)))
    record(`${forbiddenLockfile} is not allowed in this npm workspace`);
}
...
const hasPublishImplementation = files.some((file) =>
  /packages\/(?:kernel|cli|gui)\/src\/.*publish/i.test(relative(file)),
);
const hasLocalLifecycleImplementation =
  files.some((file) => relative(file) === "packages/daemon/src/repo-cell.ts") &&
  files.some((file) => relative(file) === "packages/cli/src/cli/thin-command.ts");
const hasTaskProjectionImplementation = files.some(
  (file) => relative(file) === "packages/kernel/src/projection/rebuildable-task-projection.ts",
);
const hasMulticaAdapterImplementation =
  files.some((file) => relative(file) === "packages/adapters/multica/src/index.ts") &&
  !readFileSync(path.join(root, "packages/adapters/multica/src/index.ts"), "utf8").trim().startsWith("export {}");
const hasExtensionModelImplementation = files.some(
  (file) => relative(file) === "packages/preset/src/preset-extension-model.ts",
);
for (const [kind, active] of Object.entries({
  gui: hasGuiImplementation,
  store: hasStoreImplementation,
  publish: hasPublishImplementation,
})) {
  if (!active) continue;
  for (const requiredPath of expectedRuntimeTestFiles[kind]) {
    if (!existsSync(path.join(root, requiredPath)))
      record(`${kind} implementation requires contract test: ${requiredPath}`);
  }
}
...
if (hasStoreImplementation) {
  ...
  const availability = cellText.slice(
    cellText.indexOf('state: RepoCellStatus["state"] ='),
    cellText.indexOf("let lastError"),
  );
  if (/elapsedMs|Date\.now|performance\.now/u.test(availability))
    record("RepoCell availability must depend on recovery status alone, never on elapsed time");
}
...
if (hasMulticaAdapterImplementation) {
  ...
  for (const requiredSnippet of multicaRequiredSnippets) {
    if (!multicaText.includes(requiredSnippet))
      record(`Multica readonly adapter implementation must include ${requiredSnippet}`);
  }
  for (const forbiddenVerb of multicaForbiddenVerbs) {
    const exposedVerb = new RegExp(`(?:readonly\\s+)?${forbiddenVerb}\\s*[?:=(:]`, "u");
    if (exposedVerb.test(multicaText))
      record(`Multica readonly adapter must not expose external write verb: ${forbiddenVerb}`);
  }
...
if (hasExtensionModelImplementation) {
  const extensionModelText = readFileSync(path.join(root, "packages/preset/src/preset-extension-model.ts"), "utf8");
  const extensionTestPath = "packages/preset/test/preset-extension-model.test.ts";
  if (!existsSync(path.join(root, extensionTestPath)))
    record(`extension model implementation requires contract test: ${extensionTestPath}`);
  ...
  if (/from\s+["']effect["']/.test(extensionModelText) || /Effect\./.test(extensionModelText)) {
    record("preset extension helpers must remain pure and must not import or run Effect");
  }
...
for (const file of files) {
  ...
  if (rel.startsWith("packages/") && rel.includes("/src/") && rel !== "packages/kernel/src/layout/index.ts") {
    if (
      /planning\/tasks|planningRoot|\{\{paths\.authoredRoot\}\}\/planning|path\.join\([^)]*["']planning["']/.test(text)
    ) {
      record(
        `${rel}: authored planning roots must use layout root fields; planning/tasks, planningRoot, and authoredRoot/planning path concatenation are no longer valid production roots`,
      );
    }
    if (
      /path\.join\([^)]*(?:authoredRoot|context\.paths\.authoredRoot)[^)]*["'](?:decisions|sessions|adr)["']/.test(
        text,
      ) ||
      /\{\{paths\.authoredRoot\}\}\/(?:decisions|sessions|adr)\b/.test(text)
    ) {
      record(
        `${rel}: decision/session/adr roots must come from HarnessLayout or ScriptHost context path tokens, not authoredRoot string concatenation`,
      );
    }
  ...
  if (rel.startsWith("packages/kernel/src/domain/")) {
    if (
      /\bfrom\s+["']effect["']|\bimport\s*\(\s*["']effect["']\s*\)|\b(?:Effect|Context|Layer|Queue|Semaphore)\b/.test(
        text,
      )
    ) {
  ...
  if (
    ...
    /function\s+validateRelativeDocumentPath|path\.isAbsolute\s*\(\s*documentPath|path\.normalize\s*\(\s*documentPath/.test(
      text,
    )
  ) {
    record(
      `${rel}: controller document paths must use kernel normalizeRelativeDocumentPath instead of a local validator`,
    );
  }

  if (rel === "packages/gui/src/api/service-bridge.ts" && !text.includes("daemon")) {
  ...
  if (
    ...
    !isTestOrFixture &&
    /\.(?:writeDocument|archivePackage)\s*\(/.test(text)
  ) {
  ...
  if (
    rel.startsWith("packages/kernel/src/store/") &&
    /\bfrom\s+["'][^"']*(?:packages\/adapters|@harness-anything\/adapter-)[^"']*["']/.test(text)
  ) {
    record(`${rel}: store must not import engine adapter implementations`);
  }
  const allowedGitProcessImplementations = new Set(["packages/kernel/src/store/local-version-control-system.ts"]);
  if (
    rel.startsWith("packages/kernel/src/store/") &&
  ...
  if (rel.startsWith("packages/adapters/") && !isTestOrFixture) {
    if (/\bcoordinator\.(?:enqueue|flush)\s*\(/.test(text)) {
      record(
        `${rel}: adapters must use kernel store write helpers instead of directly calling WriteCoordinator.enqueue/flush`,
      );
    }
  ...
  if (rel.startsWith("packages/gui/")) {
    ...
    if (/cors\s*\([^)]*(?:origin\s*:\s*["']\*["']|\*)/s.test(text))
      record(`${rel}: local API must not use wildcard CORS`);
    if (/\.listen\s*\(\s*["'](?:0\.0\.0\.0|::)["']/.test(text)) record(`${rel}: local API must bind to 127.0.0.1 only`);
    if (
      /\blisten\s*\([^)]*["']0\.0\.0\.0["']/.test(text) ||
      /\blisten\s*\([^)]*host\s*:\s*["']0\.0\.0\.0["']/.test(text)
    )
      record(`${rel}: local API must bind to 127.0.0.1 only`);
    if (/\bcors\s*\(\s*\)/.test(text)) record(`${rel}: local API must not use default wildcard CORS`);
    if (
      /\bcontextBridge\.exposeInMainWorld\s*\(/.test(text) &&
      !/allowedPreloadApi|preloadAllowlist|HARNESS_PRELOAD_API/.test(text)
    ) {
    ...
    if (/from\s+["'][^"']*(?:@harness-anything\/adapter-|packages\/adapters)[^"']*["']/.test(text)) {
      record(
        `${rel}: GUI must read cached projections/application services, not call external adapter implementations`,
      );
    }
    if (
      /terminal[\s\S]{0,120}(?:projection|mutate|ingest|parse output|appendProgress|saveEvidence|TaskService)/i.test(
        text,
      )
    ) {
  ...
  if (rel.startsWith("packages/daemon/src/")) {
    if (
      /from\s+["'][^"']*(?:packages\/kernel\/src\/store|packages\/adapters|@harness-anything\/adapter-)[^"']*["']/.test(
        text,
      )
    ) {
    ...
    if (
      /\bWriteCoordinator\.(?:enqueue|flush)\s*\(|\bcoordinator\.(?:enqueue|flush)\s*\(|\.(?:writeDocument|archivePackage)\s*\(/.test(
        text,
      )
    ) {
