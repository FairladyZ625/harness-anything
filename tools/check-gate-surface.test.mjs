      ...
      workflow(workflow) {
        return workflow.replace(
          ...
          [
            ...
            "      - run: xvfb-run --auto-servernum node tools/run-manifest-gates.mjs --workflow-job gui-e2e 2>&1 | tee artifacts/gui-e2e/gui-e2e.log",
            "  boundaries:",
            "",
          ].join("\n"),
        );
...
test("gate surface check rejects boundary allowlists embedded in the checker file", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /check-import-boundaries allowlistPolicy\.location must be outside the checker file tools\/check-import-boundaries\.mjs/,
    );
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
