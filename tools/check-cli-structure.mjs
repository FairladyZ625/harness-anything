...
const allowedStaticGraph = new Set([
  ...
  "packages/daemon/src/protocol/daemon-protocol-commands-event.ts",
  ...
  "packages/daemon/src/protocol/daemon-protocol-observe-tail-read.ts",
...
function checkThinCliSurface() {
  ...
  const expectedDynamic = ["./daemon/control.ts"];
  if (JSON.stringify(dynamic) !== JSON.stringify(expectedDynamic)) {
    violations.push("packages/cli/src/index.ts: daemon control must be the only dynamic entry module");
  }
}
