...
function args(argv) {
  ...
  if (!Number.isInteger(result.entities) || result.entities < 1)
    throw new Error("--entities must be a positive integer");
  if (!Number.isFinite(result.eventsPerTask) || result.eventsPerTask < 0)
    throw new Error("--events-per-task must be non-negative");
  return result;
}
...
function iso(index) {
  return new Date(Date.UTC(2024, 0, 1) + index * 3600_000).toISOString();
}
...
function factPackage(root, index, random) {
  const id = `F-${pad(index, 8)}`;
  const path = join(root, "harness", "facts", `${id}.md`);
...
function eventFile(root, sequence, event, random) {
  ...
  write(
    ...
    JSON.stringify({
      ...
      eventId: `evt_scale_${pad(sequence, 10)}`,
      workspaceRevision: sequence + 1,
      occurredAt: iso(sequence),
      type: event.type,
      taskId: event.taskId,
      payload: { ...event.payload, note: body(random, event.type, int(random, 80, 440)) },
    }) + "\n",
  );
}
...
function main() {
  ...
  for (let index = 0; index < options.entities; index += 1) {
    ...
    const factId = `F-${pad(index % facts, 8)}`;
    tasks.push(taskPackage(output, index, random, decisionId, factId));
  }
  for (let index = 0; index < facts; index += 1) factPackage(output, index, random);
  for (let index = 0; index < decisions; index += 1)
    decisionPackage(output, index, random, tasks[index % tasks.length]);
  const eventTypes = [
    "task_created",
    "task_progress",
    "fact_recorded",
  ...
  for (let sequence = 0; sequence < events; sequence += 1) {
    ...
    eventFile(
      ...
      sequence,
      {
        type: eventTypes[sequence % eventTypes.length],
        taskId: task.id,
        payload: {
          decisionId: task.decisionId,
          factId: task.factId,
          relation: sequence % 3 === 0 ? `decision/${task.decisionId}` : null,
        },
      },
      random,
    );
  ...
  const metadata = {
    schema: "scale-fixture/v1",
    seed: options.seed,
    primaryTaskCount: options.entities,
    factCount: facts,
    decisionCount: decisions,
    eventCount: events,
    eventsPerTask: options.eventsPerTask,
    eventDirectoryCount: EVENT_DIRS,
    model: { taskRatio: 1, factRatio: 0.45, decisionRatio: 0.48, docsPerTaskPackage: 8 },
  };
