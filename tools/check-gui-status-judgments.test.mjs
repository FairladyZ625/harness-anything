...
import { checkGuiStatusJudgments, scanGuiStatusJudgments } from "./check-gui-status-judgments.mjs";
      ...
      const register = [
        {
          ...
          entity: "Decision",
          field: "state",
          module: "packages/kernel/src/domain/decision.ts",
          anchor: "decisionStates",
          words: ["proposed", "rejected"],
        },
        {
        ...
        },
        {
          ...
          entity: "WriteReceipt",
          field: "outcome",
          module: "packages/kernel/src/domain/write.ts",
          anchor: "outcomes",
          words: ["applied", "pending", "op_rejected"],
        },
        {
          id: "gui.decision.state",
          entity: "GuiAdapter",
          field: "state",
          module: "packages/gui/src/model/types.ts",
      ...
      const register = [
        {
          ...
          field: "liveness",
          module: "packages/kernel/src/domain/runtime.ts",
          anchor: "runtimeLiveness",
          words: ["live", "exited"],
        },
        {
          id: "receipt.outcome",
          ...
          field: "outcome",
          module: "packages/kernel/src/domain/write.ts",
          anchor: "receiptOutcomes",
          words: ["applied", "pending"],
        },
        {
          id: "task.gate-witness",
          entity: "Task",
...
test("source identity survives formatting, responsibility split, and file rename", () => {
  ...
  withFixture(
    ...
    (root) => {
      ...
      assert.ok(
        semanticChange.some((finding) => finding.includes("baseline freezes")),
        semanticChange.join("\n"),
      );
    },
  );
  ...
  withFixture(
    ...
    (root) => {
      ...
      assert.equal(
        marked.length,
        0,
        "a call expression between the marker and the comparison must not pass the identity through",
      );
    },
  );
});
