...
const forbiddenApiPatterns = [/\brequestTransition\b/u, /\bruntimeQueue\b/u, /\bproviderNeutralTransition\b/u];
...
const activeCutoverGuidancePatterns = [/\bcoding vertical cutover\b/iu, /\bcutover readiness\b/iu];
...
function isAllowedCliPackageBuildScript(value) {
  return (
    value === "tsc -p tsconfig.build.json" || value === "tsc -p tsconfig.build.json && node scripts/copy-assets.mjs"
  );
}
...
function checkBehaviorCorpusReport(root, violations) {
  ...
  if (!existsSync(dataPath)) {
    violations.push(
      "tools/legacy-intake/behavior-corpus-classification.json: missing machine-checkable behavior corpus classification input",
    );
    return;
  }
  ...
  for (const required of ["preserve", "intentional-change", "old-bug", "unsupported-input", "needs-decision"]) {
    if (!Number.isInteger(categories[required]) || categories[required] < 0) {
      violations.push(
        `tools/legacy-intake/behavior-corpus-classification.json: missing non-negative integer category ${required}`,
      );
    }
  }
  if (Array.isArray(data?.items)) {
    ...
    for (const [category, expectedCount] of Object.entries(categories)) {
      if (actualCounts[category] !== expectedCount) {
        violations.push(
          `tools/legacy-intake/behavior-corpus-classification.json: category ${category} count ${expectedCount} does not match ${actualCounts[category]} item(s)`,
        );
      }
    }
    if (data.items.length < minBehaviorCorpusItems) {
      violations.push(
        `tools/legacy-intake/behavior-corpus-classification.json: behavior corpus must include at least ${minBehaviorCorpusItems} classified items`,
      );
    }
  } else {
  ...
  if (categories["needs-decision"] !== 0) {
    violations.push(
      "tools/legacy-intake/behavior-corpus-classification.json: unresolved needs-decision differences remain",
    );
  }
  ...
  if (!existsSync(reportPath)) {
    violations.push(
      "tools/legacy-intake/behavior-corpus-classification.md: missing behavior corpus classification report",
    );
    return;
  }
  ...
  if (!text.includes("behavior-corpus-classification.json")) {
    violations.push(
      "tools/legacy-intake/behavior-corpus-classification.md: report must reference machine-checkable JSON input",
    );
  }
  for (const required of ["preserve", "intentional-change", "old-bug", "unsupported-input"]) {
    if (!text.includes(required)) {
      violations.push(
        `tools/legacy-intake/behavior-corpus-classification.md: missing ${required} classification category`,
      );
    }
  }
  for (const [category, expectedCount] of Object.entries(categories)) {
    const row = text
      .split(/\r?\n/u)
      .find((line) => new RegExp(`^\\|\\s*${escapeRegExp(category)}\\s*\\|`, "u").test(line));
    const markdownCount = row?.split("|")[2]?.trim();
    if (markdownCount !== String(expectedCount)) {
      violations.push(
        `tools/legacy-intake/behavior-corpus-classification.md: Markdown category ${category} count must match JSON count ${expectedCount}`,
      );
    }
  }
  ...
  if (needsDecisionCount !== "0") {
    violations.push(
      "tools/legacy-intake/behavior-corpus-classification.md: unresolved needs-decision differences remain",
    );
  }
}
...
async function collectFiles(directory) {
  ...
  while (directories.length > 0) {
    ...
    const ignoredDirectories = collectGitIgnoredFiles(
      root,
      nestedDirectories.map((full) => relative(root, full)),
    );
...
function isTestFixtureOrGenerated(rel) {
  return (
    /(?:^|\/)(?:test|tests|fixtures|__fixtures__)\//u.test(rel) ||
