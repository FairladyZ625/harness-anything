...
test("thin help contract derives route flags from command paths", () => withFixture(({ rootDir, commandPath }) => {
...
test("thin help contract rejects vacuous and duplicate directories", () => withFixture(({ rootDir, commandPath }) => {
  writeFileSync(commandPath, commandSource("ha daemon status", "Status.").replace("]);", ", { path: [\"daemon\", \"status\"], usage: \"ha daemon status\", summary: \"Again.\" }]);"));
...
function commandSource(usage, summary, commandPath = ["task", "show"]) { return `export const thinCliCommands = Object.freeze([{ path: ${JSON.stringify(commandPath)}, usage: "${usage}", summary: "${summary}" }]);\nexport function renderThinHelp() { return [...thinCliCommands.map(() => "")]; }\n`; }
