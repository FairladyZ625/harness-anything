...
async function renderProjection(typeName, constName, value) {
  return format(
    [
      "// Generated by tools/generate-task-action-protocol.mjs. Do not edit.",
      `import type { ${typeName} } from "./preset-command-contract-support.ts";`,
      `export const ${constName} = ${readableJson(value)} as const satisfies ${typeName};`,
    ].join("\n"),
    { ...prettierConfig, parser: "typescript" },
  );
}

/** Lifecycle Task Actions and the write receipt they share. */
export async function renderTaskActionProtocolProjection() {
  const { writeReceiptFields, actions } = projectTaskActions();
  return renderProjection("GeneratedTaskActionProtocolProjection", "taskActionDescriptorProjection", {
    writeReceiptFields,
    actions: actions.filter(({ id }) => id !== "create"),
  });
}

/** Task creation input and its result fields. */
export async function renderTaskCreateProjection() {
  const { taskCreateResultFields, actions } = projectTaskActions();
  return renderProjection("GeneratedTaskCreateProjection", "taskCreateDescriptorProjection", {
    taskCreateResultFields,
    action: actions.find(({ id }) => id === "create"),
  });
}

export async function generateTaskActionProtocolProjection(check = false) {
  for (const [file, expected] of [
    [target, await renderTaskActionProtocolProjection()],
    [createTarget, await renderTaskCreateProjection()],
  ]) {
    if (!check) {
    ...
    if (normalizeProjectionLineEndings(readFileSync(file, "utf8")) !== normalizeProjectionLineEndings(expected))
