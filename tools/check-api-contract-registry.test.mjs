...
function validServer() {
  return `jsonRpcMethodContracts.some(() => true); request.method === "protocol.hello"; if (!handshaken) fail(); request.method === "daemon.status"; request.method === "daemon.stop"; request.method === "daemon.repo.bootstrap"; case "daemon.repo.register": case "daemon.repo.unbind": case "daemon.repo.purge": case "daemon.connection.probe": options.host.run(repo, action, options.authContext);\n`;
}
