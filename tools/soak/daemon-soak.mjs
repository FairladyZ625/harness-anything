...
} from "../../packages/daemon/src/client/local-json-rpc-client.ts";
...
const runtimeDefinition = Object.freeze({
  ...
  configVersion: 1,
  instanceId: "instance-soak",
  installationId: "installation-soak",
  kindId: "codex",
  providerId: "openai",
  model: "gpt-5.6-sol",
...
export function createSoakEvents({ taskCount, eventCount }) {
  ...
  if (!Number.isInteger(eventCount) || eventCount < taskCount + 3)
    throw new Error("eventCount must leave room for every task and the runtime stream fixture");
  const events = [],
    tasks = [];
  const append = (schema, type, payload, taskId) => {
    const revision = events.length + 1,
      suffix = String(revision).padStart(8, "0");
    events.push({
      schema,
      eventId: `event-soak-${suffix}`,
      ...
      actor: soakActor,
      source: "local",
      occurredAt: new Date(Date.UTC(2026, 0, 1) + revision * 1_000).toISOString(),
      payload,
    });
  };
  for (let index = 0; index < taskCount; index += 1) {
    ...
    const task = {
      schema: "task/v2",
      taskId,
      title: `Nightly soak task ${String(index).padStart(6, "0")}`,
      taskClass: "standard",
      status: "planned",
      ...
      metadata: {
        idempotencyKey: null,
        parentTaskId: index === 0 ? null : `task_soak_${String(index - 1).padStart(6, "0")}`,
        workKind: "test",
        riskTier: "medium",
        urgency: "medium",
  ...
  append("agent-runtime-event/v1", "runtime_installation_observed", {
    installationId: "installation-soak",
    kindId: "codex",
  ...
  append("agent-runtime-event/v1", "runtime_dispatch_requested", {
    dispatchId: "dispatch-soak",
    runtimeSessionId: "runtime-session-soak",
    instanceId: runtimeDefinition.instanceId,
    installationId: runtimeDefinition.installationId,
    kindId: runtimeDefinition.kindId,
    idempotencyKey: "nightly-soak",
    definitionSnapshotRef: "artifact:runtime-definition/nightly-soak",
    definitionSnapshot: runtimeDefinition,
  });
  ...
  for (let index = 0; events.length < eventCount; index += 1) {
    ...
    append(
      ...
      {
        task,
        mutation: { command: "transition", reason: "nightly soak scale filler", fields: [] },
        documentClaims: [],
      },
      task.taskId,
    );
...
export function assessConnections({ normal, fault, maxFaultConnections }) {
  const failures = [];
  if (normal.connections.stillOpen !== 0)
    failures.push(`${normal.connections.stillOpen} normal-load connection(s) remained open`);
  if (normal.growth.startedMinute !== null)
    failures.push(`active connections entered a sustained climb at ${normal.growth.startedMinute}`);
  if (fault.connections.stillOpen !== 0)
    failures.push(`${fault.connections.stillOpen} fault-window connection(s) remained open`);
  if (fault.connections.opened > maxFaultConnections)
    failures.push(`fault window opened ${fault.connections.opened} > ${maxFaultConnections}`);
...
export function assessHelloLatency({ clientSamplesMs, daemon, maxP99Ms }) {
  ...
  const daemonP99Ms =
    daemon.methods.find(({ method }) => method === "protocol.hello")?.p99Ms ?? Number.POSITIVE_INFINITY;
  const failures = [];
...
export function assessRssTrend({ samples, maxGrowthBytes, maxSlopeBytesPerMinute }) {
  if (samples.length < 4)
    return { ok: false, message: `FAIL RSS trend: only ${samples.length} samples; need at least 4` };
  ...
  if (growthBytes > maxGrowthBytes)
    failures.push(`last-window median grew ${formatBytes(growthBytes)} > ${formatBytes(maxGrowthBytes)}`);
  if (slopeBytesPerMinute > maxSlopeBytesPerMinute)
    failures.push(`slope ${formatBytes(slopeBytesPerMinute)}/min > ${formatBytes(maxSlopeBytesPerMinute)}/min`);
  const prefix = failures.length === 0 ? "PASS" : "FAIL";
  return {
    ok: failures.length === 0,
    firstMedianBytes,
...
export function assessWorkload({ requests, failures, failureEvidence, failureEvidenceDropped, failureEvidenceLimit }) {
  ...
  const evidence =
    failures === 0
      ? ""
      : `; ${failureEvidence.join("; ")}${failureEvidenceDropped === 0 ? "" : `${failureEvidence.length > 0 ? "; " : ""}failure evidence truncated: ${failureEvidenceDropped} additional failure(s) omitted after ${failureEvidenceLimit} record(s)`}`;
  return {
    ok: failures === 0,
    message: `${prefix} workload requests: ${requests} completed, ${failures} failed${evidence}`,
  };
}
...
async function runSoak(config = readConfig()) {
  const resultsDir = path.resolve(config.resultsDir),
    parent = mkdtempSync(path.join(tmpdir(), "harness-nightly-soak-"));
  const rootDir = path.join(parent, "repo"),
    userRoot = path.join(parent, "user"),
    home = path.join(parent, "home"),
    daemonId = `nightly-soak-${process.pid}`,
    repoId = "nightly-soak",
  ...
  try {
    ...
    const taskIds = Array.from(
      { length: config.taskCount },
      (_, index) => `task_soak_${String(index).padStart(6, "0")}`,
    );
    ...
    const load = await runLoadPhase({
      ...
      requestIntervalMs: config.requestIntervalMs,
      helloProbeIntervalMs: config.helloProbeIntervalMs,
      sampleRssPid: child.pid,
      workloadMethodOverride: config.workloadMethodOverride,
    });
    console.log(
      `[soak] load ${config.durationMs}ms requests=${load.requests} failures=${load.failures} hello-probes=${load.helloSamplesMs.length} rss-samples=${load.rssSamples.length}`,
    );

    let streamAttached = false,
      streamLost = null;
    detach = await streamAgentRuntimeAt({
      socketPath: endpoint,
      repoId,
      payload: { runtimeSessionId: "runtime-session-soak", afterCursor: "stream:0" },
      onValue: (value) => {
        if (value && typeof value === "object" && "ok" in value && value.ok === true) streamAttached = true;
      },
      onClosed: (failure) => {
        streamLost = failure;
      },
    ...
    rejectServer = undefined;
    await faultLog.settle();
    console.log(
      `[soak] fault window ${config.faultDurationMs}ms complete; terminal report=${streamLost ? JSON.stringify(streamLost) : "none"}`,
    );

    const normal = timelineFor({ userRoot, daemonId }),
      fault = timelineFor({ userRoot, daemonId: `${daemonId}-fault` });
    const connections = assessConnections({ normal, fault, maxFaultConnections: config.maxFaultConnections });
    const hello = assessHelloLatency({
      clientSamplesMs: load.helloSamplesMs,
      daemon: normal,
      maxP99Ms: config.maxHelloP99Ms,
    });
    const rss = assessRssTrend({
      samples: load.rssSamples,
      maxGrowthBytes: config.maxRssGrowthBytes,
      maxSlopeBytesPerMinute: config.maxRssSlopeBytesPerMinute,
    });
    const workload = {
      ...load,
      helloSamplesMs: undefined,
    ...
    const report = {
      ...
      workload,
      assertions,
      rssSamples: load.rssSamples,
      helloSamplesMs: load.helloSamplesMs,
      normalTimeline: normal,
      faultTimeline: fault,
      streamLost,
    };
...
function initializeFixture({ rootDir, userRoot, repoId, events }) {
  ...
  for (const event of events) {
    ...
    writeFileSync(target, serializeCanonicalEvent(event), "utf8");
  }
  const last = events.at(-1),
    eventBody = serializeCanonicalEvent(last);
  writeFileSync(
  ...
  git(rootDir, "config", "user.name", "Nightly Soak");
...
async function runLoadPhase({
  ...
  concurrency,
  requestIntervalMs,
  helloProbeIntervalMs,
  sampleRssPid,
  workloadMethodOverride = null,
}) {
  const startedAt = performance.now(),
    ...
    counts = {
      ...
      failures: 0,
      failureEvidence: [],
      failureEvidenceDropped: 0,
      failureEvidenceLimit: workloadFailureEvidenceLimit,
    },
    methods = new Map(),
    helloSamplesMs = [],
    rssSamples = [];
...
async function workloadClient({
  ...
  while (Date.now() < stopAt) {
    ...
    const taskBatch = Array.from(
      { length: Math.min(50, taskIds.length) },
      (_, offset) => taskIds[(batchStart + offset) % taskIds.length],
    );
    const options = [
    ...
    const [method, params] =
      workloadMethodOverride === null ? options[(worker + iteration) % options.length] : [workloadMethodOverride, {}];
    try {
      const result = await requestDaemonJsonRpcAt(endpoint, method, params, 1_000, 5_000);
      if (result.ok !== true) {
        counts.failures += 1;
        recordLoadFailure({ counts, method, startedAt, failure: result });
      }
    } catch (error) {
      counts.failures += 1;
      recordLoadFailure({ counts, method, startedAt, failure: error });
    }
    counts.requests += 1;
...
async function helloProbeLoop({ endpoint, stopAt, startedAt, samples, counts, helloProbeIntervalMs }) {
  while (Date.now() < stopAt) {
    ...
    try {
      ...
      if (result.ok !== true) {
        counts.failures += 1;
        recordLoadFailure({ counts, method: "protocol.hello", startedAt, failure: result });
      }
      client.close();
    } catch (error) {
      counts.failures += 1;
      recordLoadFailure({ counts, method: "protocol.hello", startedAt, failure: error });
      socket?.destroy();
    }
...
export function recordLoadFailure({ counts, method, startedAt, failure }) {
  if (counts.failureEvidence.length >= counts.failureEvidenceLimit) {
    counts.failureEvidenceDropped += 1;
    return;
  }
  counts.failureEvidence.push(
    `t+${Math.round(performance.now() - startedAt)}ms ${method} failed: ${describeFailure(failure)}`,
  );
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try {
    await runSoak(config);
  } catch (error) {
    const message = error instanceof Error ? (error.stack ?? error.message) : String(error),
      resultsDir = path.resolve(config.resultsDir);
    mkdirSync(resultsDir, { recursive: true });
    writeFileSync(path.join(resultsDir, "failure.txt"), `${message}\n`, "utf8");
    console.error(`[soak] FAIL ${message}`);
    process.exitCode = 1;
  }
}
