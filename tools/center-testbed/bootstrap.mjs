...
async function main() {
  ...
  if (!token)
    fail("env", "GITLAB_TOKEN is required (export GITLAB_TOKEN=$(cat ~/.harness-secrets-center-testbed-token)).");
  resetState();
...
function resetState() {
  ...
  if (existsSync(workspace))
    for (const entry of readdirSync(workspace)) rmSync(path.join(workspace, entry), { recursive: true, force: true });
...
function seedLedger() {
  ...
  const packet = JSON.stringify({
    ...
    rejected: [
      {
        id: "RJ1",
        text: "Edges write directly to the GitLab remote",
        whyNot: "M2 forbids edge direct writes to the canonical ledger.",
      },
    ],
    claims: [],
    fulfillments: [],
  ...
  const decision = ha("decision-propose", env, [
    ...root,
    "decision",
    "propose",
    "--json-input",
  ...
  ha("fact-record", env, [
    ...
    task.taskId,
    "--statement",
    "The testbed bootstrap wrote a task, a decision, and a fact into the canonical center ledger before pushing it to GitLab.",
    "--source",
    "tools/center-testbed/bootstrap.mjs",
    "--confidence",
    "high",
  ]);
  ha("progress-append", env, [
    ...root,
    "task",
    "progress",
    "append",
    task.taskId,
    "--text",
    "Seed bootstrap finished: ledger content ready for the center daemon to attach and republish.",
  ]);
  ...
  writeFileSync(
    path.join(workspace, "harness", "context", "shared-notes.md"),
  ...
  mustRun(
    ...
    [
      ...
      workspace,
      "-c",
      `user.name=${TESTBED.gitAuthor.name}`,
      "-c",
      `user.email=${TESTBED.gitAuthor.email}`,
      "commit",
      "-q",
      "-m",
      "Testbed project wrapper",
    ],
    { env: harnessEnv() },
  );
  const ledger = path.join(workspace, "harness");
  const canonicalHead = mustRun("revision", "git", ["-C", ledger, "rev-parse", "refs/ha/canonical"], {
    env: harnessEnv(),
  }).trim();
...
async function ensureGitLabProject(token) {
  ...
  if (response.status === 301) {
    ...
    if (!tombstone?.marked_for_deletion_on)
      fail(
        "gitlab",
        `path root/${TESTBED.gitlabProject} is taken by ${tombstone?.path_with_namespace ?? "an unknown project"} that is not a restorable tombstone.`,
      );
  ...
  const created = await gitlab("projects", {
    method: "POST",
    headers,
    body: JSON.stringify({
      name: TESTBED.gitlabProject,
      path: TESTBED.gitlabProject,
      visibility: "private",
      default_branch: "main",
    }),
  });
  if (created.status !== 201)
    fail("gitlab", `project creation failed: HTTP ${created.status} ${(await created.text()).slice(0, 500)}`);
...
async function allowForcePush(projectId, headers) {
  ...
  if (removed.status !== 204 && removed.status !== 404)
    fail("gitlab", `could not unprotect main for reseeding: HTTP ${removed.status}`);
...
function pushLedgerToGitLab(project, token) {
  ...
  mustRun(
    ...
    [
      ...
      "refs/ha/canonical:refs/ha/canonical",
    ],
    { env: harnessEnv({ GITLAB_TOKEN: token }) },
  );
...
function mintFleetMaterial(seed) {
  ...
  const key = path.join(sharedFleet, "fleet.key"),
    cert = path.join(sharedFleet, "fleet.crt");
  const keyStaging = `${key}.staging`,
    certStaging = `${cert}.staging`;
  mustRun("tls", "openssl", [
    "req",
    "-x509",
    "-newkey",
    ...
    "30",
    "-addext",
    "subjectAltName=DNS:center,DNS:localhost,IP:127.0.0.1",
  ]);
  ...
  const roster = {
    ...
    assignments: edges.map((nodeId) => ({
      ...
      viewId: `${nodeId}-view`,
      personId: `testbed-${nodeId}`,
      executorId: `testbed-agent-${nodeId}`,
      expiresAt,
      scope: {
        kind: "task",
        taskId: seed.taskId,
...
function writeStateFile(seed, project, fleet) {
  const state = {
    ...
    fleet: {
      port: TESTBED.fleetPort,
      quotaBytes: TESTBED.fleetQuotaBytes,
      rosterPath: path.join(sharedFleet, "roster.json"),
      certPath: path.join(sharedFleet, "fleet.crt"),
      nodes: fleet.roster.nodes.map(({ nodeId }) => nodeId),
...
async function gitlab(pathname, init = {}) {
  try {
    return await fetch(new URL(pathname, `${TESTBED.gitlabUrl}/api/v4/`), {
      ...init,
