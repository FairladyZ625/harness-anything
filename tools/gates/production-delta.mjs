...
export function parseProductionDeclaration(prBody) {
  ...
  if (matches.length !== 1) {
    return { declaration: null, errors: [`PR body must contain exactly one Production-Delta: +N/-M line; found ${matches.length}`] };
  }
  return {
...
export function parseRetainedPaths(prBody) {
  ...
  const errors = retainedLines.length === declarations.length
    ? []
...
function validateRetainedPath({ declaration, rootDir, base, receipts, now }) {
  ...
  if (Number.isNaN(endOfExpiryDay) || endOfExpiryDay <= now.getTime()) errors.push(`Retained-Path expiry must be in the future: ${declaration.until}`);

  const matchingReceipt = receipts.find(({ receipt }) => verifyReceipt(receipt, {
  ...
  if (matchingReceipt === undefined) {
    errors.push(`Retained-Path ${normalized} lacks a valid receipt for ${declaration.decisionId} through ${declaration.until}`);
  }
  return errors;
}
...
export function evaluateProductionDelta({
  ...
  for (const filePath of computed.unclassified) errors.push(`production source is not classified by module-policy: ${filePath}`);
  if (parsedDelta.declaration !== null
    ...
    errors.push(`declared Production-Delta +${parsedDelta.declaration.added}/-${parsedDelta.declaration.deleted} does not match computed +${computed.added}/-${computed.deleted}`);
  }
  ...
  return { ok: errors.length === 0, errors, computed, declaration: parsedDelta.declaration, retainedPaths: parsedRetained.declarations };
}
...
function parseArgs(argv) {
  ...
  if (base === null) throw new Error("usage: node tools/gates/production-delta.mjs --base <sha> [--pr-body-file <path>]");
