...
test("G29 doc publication rejects extra, missing, and late targets before Git or SQLite mutation", () => {
  ...
  try { git(rootDir, "init", "-q"); git(rootDir, "config", "user.name", "G29"); git(rootDir, "config", "user.email", "g29@example.invalid"); git(rootDir, "commit", "--allow-empty", "-qm", "base");
    ...
    for (const invalid of [extra, missing]) { assert.throws(() => store.append({ event, plan: invalid, blobs: [blob] }), /write plan/iu); assert.deepEqual(store.currentCommit(), baseCommit); }
    assert.throws(() => plan.targets.push(extra.targets.at(-1))); assert.deepEqual(store.currentCommit(), baseCommit); const receipt = store.append({ event, plan, blobs: [blob] }); assert.deepEqual(projection.apply(event, plan).metrics, { sqliteTransactions: 1, reducedItems: 1 });
