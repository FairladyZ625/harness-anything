...
import {
  assertPhaseAppendOnly,
  discoverContractFiles,
  loadContracts,
  validateDerivedContracts,
} from "../derived-contracts.mjs";
...
test("G11 accepts distinct schema ids and rejects a duplicate across contracts", () => {
  const contract = (file, id, schemaId) => ({
    file,
    declaration: { id, phases: ["P2"], commands: [], gates: [], guards: [], schemas: [{ id: schemaId }] },
  });
  const distinct = [
    contract("one.contract.mjs", "one", "example/one"),
    contract("two.contract.mjs", "two", "example/two"),
  ];
  assert.deepEqual(validateDerivedContracts("/not-a-repo", distinct), []);
  assert.match(
    validateDerivedContracts("/not-a-repo", [...distinct, contract("three.contract.mjs", "three", "example/one")]).join(
      "\n",
    ),
...
test("G11 closes every structured CLI input facet and its compatibility projection", () => {
  ...
  const declaration = {
    ...
    phases: ["P2"],
    commands: [
      { id: "run", phase: "P2", path: ["run"], usage: "ha run --mode <safe>", inputs: [input], flags: [input] },
    ],
    gates: [],
    guards: [],
  };
  assert.deepEqual(validateDerivedContracts("/not-a-repo", [{ file: "cli.contract.mjs", declaration }]), []);
  const { required: _required, ...withoutRequired } = input,
    mutated = {
      ...declaration,
      commands: [{ ...declaration.commands[0], inputs: [withoutRequired], flags: [withoutRequired] }],
    };
  assert.match(
    validateDerivedContracts("/not-a-repo", [{ file: "cli.contract.mjs", declaration: mutated }]).join("\n"),
    /invalid input facet/u,
  );
  const staleProse = { ...input, error: { ...input.error, nextAction: "Use safe mode." } },
    withStaleProse = {
      ...declaration,
      commands: [{ ...declaration.commands[0], inputs: [staleProse], flags: [staleProse] }],
    };
  assert.match(
    validateDerivedContracts("/not-a-repo", [{ file: "cli.contract.mjs", declaration: withStaleProse }]).join("\n"),
    /invalid input facet/u,
  );
  const invalidJsonEnums = { ...input, jsonEnums: { verdict: [] } },
    withInvalidJsonEnums = {
      ...declaration,
      commands: [{ ...declaration.commands[0], inputs: [invalidJsonEnums], flags: [invalidJsonEnums] }],
    };
  assert.match(
    validateDerivedContracts("/not-a-repo", [{ file: "cli.contract.mjs", declaration: withInvalidJsonEnums }]).join(
      "\n",
    ),
    /JSON enums are invalid/u,
  );
  const divergent = {
    ...declaration,
    commands: [{ ...declaration.commands[0], flags: [{ ...input, enum: undefined }] }],
  };
  assert.match(
    validateDerivedContracts("/not-a-repo", [{ file: "cli.contract.mjs", declaration: divergent }]).join("\n"),
...
test("G11 requires an exact command, gate, guard, schema, and phase catalog projection", () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      gates: ["G11"],
      guards: ["safe"],
      schemas: ["manual-copy/v1"],
      phases: ["P2"],
    }),
  );
  assert.match(
    validateDerivedContracts(rootDir, [{ file: "example.contract.mjs", declaration }]).join("\n"),
