...
function lint(source, filename = "packages/cli/src/cli/manual-flags.ts") {
...
test("G12 permits contract declarations and SoT-derived projections", () => {
  ...
  assert.deepEqual(lint("const flags = new Set(command.inputs.map(({ name }) => name));"), []);
  assert.deepEqual(lint("const flags = new Set(command.inputs.filter(Boolean).map(({ name }) => name));"), []);
});
test("G12 rejects manual registries and registration calls outside contracts", () => {
  ...
  assert.match(lint('const names = ["--manual"]; const flags = new Set(names);')[0].message, /Do not handwrite a CLI flag directory/u);
  assert.match(lint('const names = ["--manual"]; const alias = names; const flags = new Set(alias);')[0].message, /Do not handwrite a CLI flag directory/u);
  assert.match(lint('const directory = { flags: ["--manual"] }; const flags = new Set(directory.flags);')[0].message, /Do not handwrite a CLI flag directory/u);
  assert.match(lint('function make(values) { return new Set(values); } const flags = make(["--manual"]);')[0].message, /Do not handwrite a CLI flag directory/u);
});
