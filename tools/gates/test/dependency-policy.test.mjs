...
test("G31 rejects lock drift and undeclared dependency changes", () => {
  const { rootDir } = makeRepo({
    ...
    "package-lock.json": JSON.stringify({ lockfileVersion: 3, packages: { "": { dependencies: { effect: "1.0.0" } } } })
  });
  assert.match(validateLockfile(rootDir).join("\n"), /effect.*2\.0\.0.*1\.0\.0/u);
  assert.match(validateDependencyDeclaration(["package-lock.json"], "No declaration.").join("\n"), /Dependency-Change/u);
