...
| Remote/Fleet daemon mode                          | Experimental | The CLI stdio relay and Windows GUI SSH transport can connect to an already-running daemon through a user-managed local endpoint. UU, FRP, VPN, public relays, and SSH `-L` are infrastructure outside Harness; fleet wire, daemon bootstrap, and general remote product support are not shipped. Evidence: `packages/cli/src/daemon/stdio-bridge.ts` and `packages/gui/src/main/remote-composition-root.ts`. |
...
| Code-doc reconciliation gate | Mechanism-complete | When the resolved preset/profile declares `code-doc-reconciliation`, repository deliverables require an execution-bound verified path witness. A reviewed submission whose non-empty deliverable list contains only task-package artifacts completes without fabricating a repository anchor; empty or ambiguous lists remain fail-closed. The gate is contract-derived and execution-applicable, not universal (ADR-0027 D7). Evidence: canon 1.4. |
...
- source-checkout `ha gui` entry with canonical build inputs and CLI-owned daemon acquisition;
- unsigned macOS arm64 DMG packaging candidate, retained for build validation but not as a supported launch entry;
...
- no complete GUI runtime control plane; agent runtime remains CLI-driven;
- no working attach-token transport, TCP listener, HTTP API, WebSocket server,
  live notification subscription, or enforced RBAC when no
  `harness/people.yaml` roster exists;
- GUI SSH transport consumes a user-managed local endpoint and does not create
  UU, FRP, VPN, or SSH `-L` channels.
