...
| 范围                      | 状态         | 边界和证据                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 源码 CLI 写路径           | Shipped      | CLI 写命令需要 daemon 认证的 human 归属与 git author。`HARNESS_ACTOR` 继续用于 `agent:<id>` 与 `system:<id>`；journal 会记录 `env` 或 `daemon` source。示例使用 `HARNESS_GIT_AUTHOR_NAME` / `HARNESS_GIT_AUTHOR_EMAIL`，也可 fallback 到 Git author 变量。 |
...
| 远端纯展示 daemon 模式    | Shipped      | `remote-proxy` 让本机 GUI 通过用户管理的本地 endpoint tunnel 显示服务器仓库；本机没有工作区，开发必须 SSH 到服务器。使用 `ha daemon connection add` / `probe` 后，以 `ha daemon repo register --mode remote-proxy --connection <connection>` 注册。证据：`docs-release/operations-server-daemon.zh-CN.md`。                        |
...
| Code-doc reconciliation gate | Mechanism-complete | 解析出的 preset/profile 声明 `code-doc-reconciliation` 时，公开仓交付物必须有绑定 Execution 的已验证路径 witness；已复核 submission 的非空 deliverable 列表若全是任务包工件，可直接完成而不伪造公开仓锚点，空列表或歧义声明仍 fail-closed。该门由契约派生并按 Execution 判断适用性，并非全局门（ADR-0027 D7）。证据：canon 1.4。 |
...
- 本地 daemon 仓库注册与多仓路由；
- source-checkout `ha gui` 入口，使用 canonical build 输入与 CLI-owned daemon acquisition；
- 未签名的 macOS arm64 DMG packaging candidate，只保留 build 验证，不作为受支持启动入口；
- 首次启动的仓库选择、身份 bootstrap 与后续引导；
...
| 界面                                      | 尚未发布                                                                                           | 不能意外继承为状态事实                                                                                    |
| ----------------------------------------- | -------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| 已 ship 与 mechanism-complete 的 CLI 界面 | 工作流证明与完整公开文档。                                                                         | 把已 ship 的层级能力继续写成 planned 的旧文档，或隐藏写命令归属要求的文档。       |
...
状态：macOS Local v1 发布候选版。运行时与 package 检查可执行；
未签名的 0.0.1 arm64 产物已准备好发布。
...
这只证明 renderer bundle 可以编译。Release build 还会执行
`npm run package:mac:arm64`，并在干净位置验证安装后的 DMG。

### GUI 分发与更新边界

Harness Anything GUI 0.0.1 仍保留手动、未签名的 macOS arm64 packaging
candidate，但 source installation 的 `ha gui` 是唯一受支持的启动入口。直接启动
DMG application、签名、notarization、其他操作系统和 update feed 属于后续发布任务。
