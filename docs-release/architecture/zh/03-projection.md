...
canonical 数据库，`.harness/cache/task.sqlite` 是派生读取缓存。只有后者可在不丢失已接受命令的前提下删除重建。

`makeTaskProjection`(`packages/kernel/src/projection/rebuildable-task-projection-factory.ts`)
读取 canonical store 的有限事件批次及必需对象。Catch-up 推进完整
...
读取投影损坏或与 schema 不匹配时，daemon 在打开时丢弃缓存并对 canonical 事件做 cold rebuild；
操作者也可以直接删除 `.harness/cache/task.sqlite`，下次打开即重建。保留 canonical 数据库及
