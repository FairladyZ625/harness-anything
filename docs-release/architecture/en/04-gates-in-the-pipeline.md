...
| CI gate axis | the resolved contract declares `ci` only when repository settings configure CI witness workflows (`settings.ci.workflows`); a repository that configures none — the default — witnesses no CI runs at all. Opt in with `ha settings update --ci-workflows <workflow>...` and clear back to none with `ha settings update --ci-workflows none`; with the gate declared, a green run of a configured workflow on `main` — imported through `ha ci observe pull` — must be witnessed for the submitted cut, the run conclusion being the verdict and uploaded `ci-observation-*` artifacts being optional detail | `ci_missing` |
